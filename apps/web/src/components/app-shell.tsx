    ...
    <div className="min-h-screen bg-[#f5f7f9] text-slate-900">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 border-r border-slate-200 bg-[#0b1f2a] text-white lg:block">
        <div className="border-b border-white/10 px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold tracking-wide">Maritime Claims</div>
              <div className="mt-0.5 text-xs text-slate-300">Risk Intelligence Platform</div>
            </div>
          </div>
        </div>
          ...
          {nav.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`mb-1 flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${active ? "bg-white/12 text-white" : "text-slate-300 hover:bg-white/7 hover:text-white"}`}>
                <span className={`grid h-7 w-7 place-items-center rounded-md text-xs font-bold ${active ? "bg-cyan-400 text-slate-950" : "bg-white/8 text-slate-300"}`}>{item.mark}</span>
                {item.label}
              </Link>
            );
          })}
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-5 backdrop-blur md:px-8">
          ...
          <div className="flex items-center gap-3">
            <span className="hidden text-sm text-slate-500 sm:inline">{user.role.replaceAll("_", " ")}</span>
            <div className="grid h-9 w-9 place-items-center rounded-full bg-slate-900 text-xs font-bold text-white">{user.full_name.split(" ").slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</div>
          </div>
        </header>
