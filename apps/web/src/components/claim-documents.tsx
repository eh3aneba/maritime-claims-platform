...
function malwareStatus(status: DocumentMalwareScanStatus) {
  if (status === "clean") return { label: "Malware scan · Clean", className: "text-emerald-700" };
  if (status === "infected_quarantined") {
    return { label: "Malware detected · Blocked", className: "text-red-700" };
  }
  if (status === "scan_error") {
    return { label: "Scanner error · Blocked", className: "text-amber-700" };
  }
  return { label: "Legacy evidence · Not scanned", className: "text-amber-700" };
}
...
export function ClaimDocuments({ claimId }: { claimId: string }) {
  ...
  async function uploadFiles(files: File[]) {
    ...
    if (validFiles.length !== files.length) {
      setError("Some files were skipped. Allowed: PDF, JPG, PNG, DOCX, XLSX.");
    }
    ...
    for (let index = 0; index < validFiles.length; index += 1) {
      ...
      try {
        ...
        setUploads((current) => current.map((item, i) => i === index ? {
          ...
          progress: 100,
          status: "done",
          name: uploaded.malware_scan_status === "clean"
            ? `${item.name} · malware scan clean`
            : item.name,
        } : item));
      } catch (e) {
  ...
  async function queueLegacyRescan() {
    if (!window.confirm(`Queue malware rescans for up to ${Math.min(legacyCount, 25)} legacy files?`)) return;
    setError("");
    setOperationMessage("");
    setOperationState((current) => ({ ...current, rescan: "Queueing…" }));
    try {
      const result = await queueLegacyEvidenceRescan(claimId, Math.min(legacyCount, 25));
      setOperationMessage(
        `${result.queued_count} legacy evidence rescan job(s) queued. The worker will update each verdict.`,
      );
      window.setTimeout(() => void refresh(), 2500);
    } catch (e) {
      setError(e instanceof ApiError ? e.detail : "Legacy evidence rescans could not be queued.");
    } finally {
  ...
  async function retryQuarantine(upload: QuarantinedUpload) {
    setError("");
    setOperationMessage("");
    setOperationState((current) => ({ ...current, [upload.id]: "Retrying scan…" }));
    try {
      const result = await retryQuarantinedUpload(claimId, upload.id);
      if (result.status === "released") {
        setOperationMessage("A clean retry released the evidence into the active claim file.");
      } else if (result.status === "infected") {
        setOperationMessage("The retry detected malware. The evidence remains quarantined.");
      } else {
        setOperationMessage("The scanner is still unavailable. The evidence remains quarantined.");
      }
      await refresh();
    } catch (e) {
      setError(e instanceof ApiError ? e.detail : "Quarantine retry failed.");
    } finally {
  ...
  async function purgeQuarantine(upload: QuarantinedUpload) {
    const reason = window.prompt(
      "Administrative purge permanently removes the retained bytes. Enter a reason of at least 20 characters:",
    );
    if (!reason) return;
    if (reason.trim().length < 20) {
      setError("A purge reason of at least 20 characters is required.");
      return;
    }
    if (!window.confirm(`Permanently purge retained bytes for ${upload.original_filename}?`)) return;
    setError("");
    setOperationMessage("");
    setOperationState((current) => ({ ...current, [upload.id]: "Purging…" }));
    try {
      await purgeQuarantinedUpload(claimId, upload.id, reason.trim());
      setOperationMessage("Quarantined bytes were purged; provenance and audit metadata were retained.");
      await refresh();
    } catch (e) {
      setError(e instanceof ApiError ? e.detail : "Quarantined bytes could not be purged.");
    } finally {
      setOperationState((current) => ({ ...current, [upload.id]: "" }));
    }
  }
  ...
  async function download(document: ClaimDocument) {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.detail : "Document could not be downloaded.");
    }
  }
  ...
  async function analyzeDocument(document: ClaimDocument) {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.detail : "Document intelligence could not be queued.");
    }
  }
  ...
  return (
    <section className="panel p-6">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-[1fr_200px]">
        <label><span className="label">Document type</span><select value={documentType} onChange={(event) => setDocumentType(event.target.value)} className="field">{documentTypes.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
        <label><span className="label">Confidentiality</span><select value={confidentiality} onChange={(event) => setConfidentiality(event.target.value as ConfidentialityLevel)} className="field"><option value="internal">Internal</option><option value="confidential">Confidential</option><option value="restricted">Restricted</option></select></label>
      </div>
