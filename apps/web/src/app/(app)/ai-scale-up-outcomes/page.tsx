...
type ScaleRun = { id: string; task_type: string; status: string };
type ScaleAuthorization = { id: string; authorization_key: string; status: string; rollout_percentage: number; runs: ScaleRun[] };
type Observation = { scale_up_run_id: string; usefulness_rating: number; review_seconds: number };
type Review = { review_role: string; action: string };
type Assessment = {
  id: string; scale_up_authorization_id: string; assessment_key: string; attempt_number: number;
  ...
  summary: { broader_production_stage_recommended: boolean; production_wide_authorized: boolean; rollout_increase_authorized: boolean };
};
type OutcomeDashboard = { assessments: Assessment[] };
type ScaleDashboard = { authorizations: ScaleAuthorization[] };
...
export default function AIScaleUpOutcomesPage() {
  const [outcomes, setOutcomes] = useState<OutcomeDashboard | null>(null);
  const [scale, setScale] = useState<ScaleDashboard | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const [a, b] = await Promise.all([
        request<OutcomeDashboard>("/ai-scale-up-outcomes"),
        request<ScaleDashboard>("/ai-scale-up"),
      ]);
      setOutcomes(a); setScale(b); setError(null);
    } catch (err) {
      setError(err instanceof ApiError ? err.detail : "Could not load Sprint 11H.");
    }
  }, []);
  useEffect(() => { void load(); }, [load]);

  const completed = scale?.authorizations.find((item) => item.status === "completed") ?? null;
  const current = outcomes?.assessments[0] ?? null;
  const observed = useMemo(() => new Set(current?.observations.map((item) => item.scale_up_run_id) ?? []), [current]);
  ...
  async function act(action: () => Promise<unknown>, success: string) {
    ...
    catch (err) { setError(err instanceof ApiError ? err.detail : "Sprint 11H action failed."); }
    finally { setBusy(false); }
  }

  async function create() {
    if (!completed) return setError("A completed Sprint 11G controlled scale-up is required.");
    await act(() => request("/ai-scale-up-outcomes/assessments", {
      method: "POST", body: JSON.stringify({
        scale_up_authorization_id: completed.id,
        assessment_key: `scale-up-readiness-${crypto.randomUUID()}`,
        confirm_content_free_assessment: true,
      }),
    }), "Sprint 11H readiness assessment created.");
  }

  async function observe() {
    if (!current || !runId) return;
    await act(() => request(`/ai-scale-up-outcomes/assessments/${current.id}/observations`, {
      method: "POST", body: JSON.stringify({
        scale_up_run_id: runId, usefulness_rating: usefulness, review_seconds: reviewSeconds,
        workflow_completed: true,
        evidence_reference: `artifact://ai-scale-up-outcomes/run-${runId}`,
        note: "Content-free usefulness and operator-effort evidence for the immutable Sprint 11G run.",
  ...
  async function review(role: "product" | "quality" | "risk" | "operations" | "security") {
    if (!current) return;
    await act(() => request(`/ai-scale-up-outcomes/assessments/${current.id}/reviews`, {
      method: "POST", body: JSON.stringify({ review_role: role, action: "approve",
        evidence_reference: `artifact://ai-scale-up-outcomes/${role}-review`,
        note: `Independent ${role} reviewer reproduced the frozen Sprint 11H scorecard.` }),
    }), `${role} review recorded.`);
  }

  async function decide(outcome: "recommend_broader_production_stage" | "extend_controlled_scale_up" | "stop_ai_progression") {
    if (!current) return;
    await act(() => request(`/ai-scale-up-outcomes/assessments/${current.id}/decision`, {
      method: "POST", body: JSON.stringify({ outcome, confirm_recommendation_only: true,
        note: outcome === "recommend_broader_production_stage"
          ? "Recommend designing a separately authorized broader-production stage; no rollout changes here."
          : outcome === "extend_controlled_scale_up"
            ? "Require another controlled scale-up cohort before broader-production design."
            : "Stop AI progression without granting broader permissions." }),
    }), "Recommendation-only Sprint 11H decision recorded.");
  }

  const metrics = current?.metrics ?? {};
  const roles = new Set(current?.reviews.map((review) => review.review_role) ?? []);
  const canCreate = completed && (!current || current.scale_up_authorization_id !== completed.id || ["review_rejected", "extended", "stopped"].includes(current.status));

  return <div className="space-y-7">
    <section className="rounded-2xl bg-[#0b1f2a] p-7 text-white shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300">Sprint 11H · controlled scale-up outcome</p>
      <h1 className="mt-3 text-3xl font-semibold">Production-readiness recommendation gate</h1>
      <p className="mt-3 max-w-4xl text-sm leading-6 text-slate-300">Measure the completed 11G cohort using immutable run, monitor and incident evidence. Five independent reviews are required. A passing result still grants no rollout increase, Production-wide authorization, Restricted documents or new document classes.</p>
    </section>
    ...
    <section className="grid gap-4 md:grid-cols-4">
      {[["Assessment", current?.status.replaceAll("_", " ") ?? "not created"],
        ...
        ["Broader rollout", "Not authorized"]].map(([label, value]) =>
    ...
    {canCreate && <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="text-lg font-semibold">Start Sprint 11H</h2>
      <p className="mt-2 text-sm text-slate-600">Anchor the completed 11G authorization and freeze its exact decision hash plus inherited 11F hashes.</p>
    ...
    {current && <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-5">
        <button disabled={busy || current.status !== "collecting"} onClick={() => void act(() => request(`/ai-scale-up-outcomes/assessments/${current.id}/finalize`, { method: "POST", body: JSON.stringify({ confirm_finalize: true, note: "Freeze the complete Sprint 11G cohort, monitoring, incident and recovery evidence." }) }), "Sprint 11H scorecard finalized.")} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">Finalize scorecard</button>
        {(["product", "quality", "risk", "operations", "security"] as const).map((role) => <button key={role} disabled={busy || roles.has(role) || !["review_ready", "decision_ready"].includes(current.status)} onClick={() => void review(role)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold capitalize disabled:opacity-40">{role} approve</button>)}
        <button disabled={busy || current.status !== "decision_ready"} onClick={() => void decide("recommend_broader_production_stage")} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">Recommend broader-stage design</button>
        <button disabled={busy || current.status !== "decision_ready"} onClick={() => void decide("extend_controlled_scale_up")} className="rounded-lg border border-amber-300 px-4 py-2 text-sm font-semibold text-amber-800 disabled:opacity-40">Extend controlled scale-up</button>
