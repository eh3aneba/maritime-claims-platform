...
from tests.test_adjustment_controls import _edit_invoice_amount, _seed_cost_schedule
...
def _approved_adjustment() -> tuple[str, dict, dict]:
    claim_id, _, refs = _seed_cost_schedule()
    statement_response = client.post(f"/api/v1/claims/{claim_id}/adjustments", json={"currency": "USD"})
    ...
    approved = client.post(
        ...
        json={"note": "Approved source-bound adjustment for settlement control."},
    )
    assert approved.status_code == 200, approved.text
    assert approved.json()["source_state_status"] == "current"
    return claim_id, approved.json(), refs
...
def test_settlement_and_payment_require_human_separation_and_preserve_hashes() -> None:
    ...
    created = client.post(
        ...
        json={
            ...
            "title": "Without Prejudice Final Settlement",
    ...
    assert client.post(
        f"/api/v1/claims/{claim_id}/settlement-ledger/settlements/{settlement['id']}/submit"
    ).status_code == 200
    ...
    payment = client.post(
        ...
        json={
            ...
            "amount": "700.00",
            "purpose": "First controlled settlement instalment.",
        },
    )
    ...
    over_cap = client.post(
        ...
        json={
            ...
            "payee": "Orion Shipowning Ltd",
            "amount": "400.00",
            "purpose": "Would exceed settlement cap.",
        },
    )
    ...
    with TestingSessionLocal() as db:
        ...
        second = User(
            ...
            email="alpha-manager-two@example.com",
            full_name="Alpha Manager Two",
    ...
    client.cookies.clear()
    login("alpha", "alpha-manager-two@example.com")
    second = client.post(
        f"/api/v1/claims/{claim_id}/settlement-ledger/payments/{payment_id}/approve",
    ...
    unconfirmed = client.post(
        ...
        json={
            "confirm_paid_externally": False,
            "channel": "bank_transfer",
            "external_reference": "BANK-2026-001",
    ...
    with TestingSessionLocal() as db:
        ...
        assert {
            ...
            "FIRST_APPROVE_PAYMENT_AUTHORIZATION",
            "SECOND_APPROVE_PAYMENT_AUTHORIZATION",
            "RECORD_PAYMENT_PAID_EXTERNALLY",
        }.issubset(actions)


def test_new_settlement_is_blocked_after_adjustment_source_becomes_stale_but_historical_ledger_remains() -> None:
    claim_id, adjustment, refs = _approved_adjustment()
    client.cookies.clear()
    login("alpha", "alpha-admin@example.com")
    existing = client.post(
        ...
        json={
            "adjustment_statement_id": adjustment["id"],
            "title": "Historical proposal before source correction",
            "settlement_type": "interim",
            "amount": "500.00",
            "terms": "Recorded before later source evidence correction.",
        },
    )
    ...
    blocked = client.post(
        ...
        json={
            "adjustment_statement_id": adjustment["id"],
            "title": "Must not use stale adjustment",
            "settlement_type": "final",
            "amount": "400.00",
            "terms": "This proposal must be blocked until the Adjustment is rebased and re-approved.",
        },
    )
    assert blocked.status_code == 409
    assert "historical against the current financial evidence" in blocked.json()["detail"]

    ledger = client.get(f"/api/v1/claims/{claim_id}/settlement-ledger")
    assert ledger.status_code == 200
    assert any(row["id"] == existing_id for row in ledger.json()["settlements"])
...
def test_settlement_amount_and_tenant_scope_are_enforced() -> None:
    ...
    excessive = client.post(
        ...
        json={
            "adjustment_statement_id": adjustment["id"],
            "title": "Excess proposal",
            "settlement_type": "final",
            "amount": "99999.00",
            "terms": "Invalid excess.",
        },
    )
