"""Add Phase AF independent health qualification for completed AE transitions.

Revision ID: 0145_write_ownership_transition_health
Revises: 0144_recovery_write_ownership_transition_execution
"""
...
revision = "0145_write_ownership_transition_health"
down_revision = "0144_recovery_write_ownership_transition_execution"
branch_labels = None
depends_on = None

QUAL = "evidence_recovery_write_ownership_transition_health_qualifications"
RECEIPT = "evidence_recovery_write_ownership_transition_health_receipts"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("canary_lease_hash", sa.String(64), nullable=False),
        ...
        *_safety_columns(),
        ...
        sa.ForeignKeyConstraint(["activation_receipt_id"], ["evidence_recovery_write_ownership_transition_receipts.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["terminal_receipt_id"], ["evidence_recovery_write_ownership_transition_receipts.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["phase_ae_activated_by_id"], ["users.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["phase_ad_requested_by_id"], ["users.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["phase_ad_approved_by_id"], ["users.id"], ondelete="RESTRICT"),
        ...
        sa.UniqueConstraint("transition_lease_id", name="uq_wr_own_health_lease"),
        sa.UniqueConstraint("organization_id", "health_qualification_hash", name="uq_wr_own_health_org_hash"),
        sa.CheckConstraint("status IN ('pending_second_approval','qualified','rejected','expired','invalidated')", name="ck_wr_own_health_status"),
        sa.CheckConstraint("health_state = 'healthy'", name="ck_wr_own_health_healthy"),
        sa.CheckConstraint("requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_four_eyes"),
        sa.CheckConstraint("phase_ae_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_ae_split"),
        sa.CheckConstraint("phase_ad_requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_adr_split"),
        sa.CheckConstraint("phase_ad_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_ada_split"),
        sa.CheckConstraint("phase_ac_qualified_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_ac_split"),
        sa.CheckConstraint("phase_ab_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_ab_split"),
        sa.CheckConstraint("phase_aa_requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_aar_split"),
        sa.CheckConstraint("phase_aa_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_aaa_split"),
        sa.CheckConstraint("phase_z_qualified_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_z_split"),
        sa.CheckConstraint("phase_y_executed_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_y_split"),
        sa.CheckConstraint("phase_x_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_wr_own_health_x_split"),
        sa.CheckConstraint("source_file_size_bytes >= 0", name="ck_wr_own_health_size"),
        sa.CheckConstraint("observed_local_size_bytes = source_file_size_bytes", name="ck_wr_own_health_local_size"),
        sa.CheckConstraint("observed_recovery_size_bytes = source_file_size_bytes", name="ck_wr_own_health_recovery_size"),
        sa.CheckConstraint("observed_local_hash = source_file_hash", name="ck_wr_own_health_local_hash"),
        sa.CheckConstraint("observed_recovery_hash = source_file_hash", name="ck_wr_own_health_recovery_hash"),
        sa.CheckConstraint("read_route_version_at_request >= 1", name="ck_wr_own_health_read_ver"),
        sa.CheckConstraint("write_route_version_at_request >= 1", name="ck_wr_own_health_write_ver"),
        *_safety_constraints("wr_own_health"),
    )
    op.create_index("ix_wr_own_health_org_claim", QUAL, ["organization_id", "claim_id", "status"])
    op.create_index("ix_wr_own_health_org_doc", QUAL, ["organization_id", "document_id", "status"])
    ...
    op.create_table(
        ...
        *_safety_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("organization_id", "receipt_hash", name="uq_wr_own_health_rec_org_hash"),
        sa.CheckConstraint("phase IN ('requested','qualified','rejected','expired','invalidated')", name="ck_wr_own_health_rec_phase"),
        sa.CheckConstraint("health_state = 'healthy'", name="ck_wr_own_health_rec_healthy"),
        *_safety_constraints("wr_own_health_rec"),
    )
    op.create_index("ix_wr_own_health_rec_time", RECEIPT, ["health_qualification_id", "transitioned_at"])


def downgrade() -> None:
    op.drop_index("ix_wr_own_health_rec_time", table_name=RECEIPT)
    op.drop_table(RECEIPT)
    op.drop_index("ix_wr_own_health_org_doc", table_name=QUAL)
    op.drop_index("ix_wr_own_health_org_claim", table_name=QUAL)
    op.drop_table(QUAL)
