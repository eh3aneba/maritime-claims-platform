"""separately authorized high-coverage AI cohort

Revision ID: 0048_ai_high_coverage
Revises: 0047_ai_bp_outcome
"""
import sqlalchemy as sa
from alembic import op

revision = "0048_ai_high_coverage"
down_revision = "0047_ai_bp_outcome"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        "ai_high_coverage_authorizations",
        ...
        sa.Column("authorization_mode", sa.String(60), server_default="high_coverage_bounded", nullable=False),
        ...
        *_base_columns(),
        ...
        sa.ForeignKeyConstraint(["outcome_assessment_id"], ["ai_broader_production_outcome_assessments.id"], ondelete="RESTRICT"),
        ...
        sa.UniqueConstraint("organization_id", "authorization_key", name="uq_ai_hc_org_key"),
        sa.UniqueConstraint("outcome_assessment_id", "attempt_number", name="uq_ai_hc_attempt"),
        sa.CheckConstraint("attempt_number >= 1", name="ck_ai_hc_attempt"),
        sa.CheckConstraint("previous_rollout_percentage BETWEEN 26 AND 50", name="ck_ai_hc_prev_rollout"),
        sa.CheckConstraint("rollout_percentage BETWEEN 51 AND 75", name="ck_ai_hc_rollout"),
        sa.CheckConstraint("max_reject_rate_bps = 600 AND max_edit_rate_bps = 2500", name="ck_ai_hc_review_rates"),
        sa.CheckConstraint("max_unsupported_output_rate_bps = 50 AND min_source_grounding_validity_bps = 9950", name="ck_ai_hc_grounding"),
        sa.CheckConstraint("max_p95_latency_ms = 18000 AND max_mean_cost_microusd = 450000", name="ck_ai_hc_ops"),
        sa.CheckConstraint("max_quality_regression_bps = 200 AND max_latency_regression_bps = 1000 AND max_cost_regression_bps = 1000", name="ck_ai_hc_trends"),
        sa.CheckConstraint("status IN ('pending_approvals','decision_ready','authorized','held','rejected','paused','revoked','completed','expired')", name="ck_ai_hc_status"),
    )
    op.create_index("ix_ai_hc_org", "ai_high_coverage_authorizations", ["organization_id"])
    op.create_index("ix_ai_hc_outcome", "ai_high_coverage_authorizations", ["outcome_assessment_id"])
    op.create_index("ix_ai_hc_broader", "ai_high_coverage_authorizations", ["broader_production_authorization_id"])
    op.create_index("ix_ai_hc_org_status", "ai_high_coverage_authorizations", ["organization_id", "status", "created_at"])

    op.create_table(
        "ai_high_coverage_approvals",
        sa.Column("organization_id", sa.Uuid(), nullable=False),
        ...
        *_base_columns(),
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["authorization_id"], ["ai_high_coverage_authorizations.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["approver_id"], ["users.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("authorization_id", "approval_role", name="uq_ai_hc_approval_role"),
        sa.CheckConstraint("approval_role IN ('security','privacy','product','operations','risk','claims_governance','ai_quality')", name="ck_ai_hc_approval_role"),
        sa.CheckConstraint("action IN ('approve','reject')", name="ck_ai_hc_approval_action"),
    )
    op.create_index("ix_ai_hc_approval_org", "ai_high_coverage_approvals", ["organization_id", "authorization_id"])

    op.create_table(
        "ai_high_coverage_document_eligibility",
        sa.Column("organization_id", sa.Uuid(), nullable=False),
        sa.Column("authorization_id", sa.Uuid(), nullable=False),
        ...
        *_base_columns(),
        ...
        sa.ForeignKeyConstraint(["revoked_by_id"], ["users.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("authorization_id", "document_id", "attestation_number", name="uq_ai_hc_doc_attempt"),
        sa.CheckConstraint("attestation_number >= 1", name="ck_ai_hc_doc_attempt"),
        sa.CheckConstraint("rollout_bucket BETWEEN 0 AND 99", name="ck_ai_hc_doc_bucket"),
        sa.CheckConstraint("document_type IN ('chief_engineer_report','engine_log')", name="ck_ai_hc_doc_type"),
        sa.CheckConstraint("confidentiality_level IN ('internal','confidential')", name="ck_ai_hc_doc_conf"),
        sa.CheckConstraint("status IN ('eligible','revoked')", name="ck_ai_hc_doc_status"),
    )
    op.create_index("ix_ai_hc_doc_org", "ai_high_coverage_document_eligibility", ["organization_id", "authorization_id", "status"])
    op.create_index("ix_ai_hc_doc_id", "ai_high_coverage_document_eligibility", ["document_id"])

    op.create_table(
        "ai_high_coverage_runs",
        sa.Column("organization_id", sa.Uuid(), nullable=False),
        sa.Column("authorization_id", sa.Uuid(), nullable=False),
        ...
        *_base_columns(),
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["authorization_id"], ["ai_high_coverage_authorizations.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["eligibility_id"], ["ai_high_coverage_document_eligibility.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["claim_id"], ["claims.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["document_id"], ["documents.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["requested_by_id"], ["users.id"], ondelete="SET NULL"),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("authorization_id", "run_key", name="uq_ai_hc_run_key"),
        sa.UniqueConstraint("processing_job_id", name="uq_ai_hc_processing_job"),
        sa.CheckConstraint("status IN ('queued','human_reviewed')", name="ck_ai_hc_run_status"),
        sa.CheckConstraint("human_review_action IS NULL OR human_review_action IN ('approve','edit','reject')", name="ck_ai_hc_run_action"),
    )
    op.create_index("ix_ai_hc_run_org", "ai_high_coverage_runs", ["organization_id", "authorization_id", "status"])
    op.create_index("ix_ai_hc_run_doc", "ai_high_coverage_runs", ["document_id"])

    op.create_table(
        "ai_high_coverage_monitors",
        sa.Column("organization_id", sa.Uuid(), nullable=False),
        sa.Column("authorization_id", sa.Uuid(), nullable=False),
        ...
        *_base_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("authorization_id", "monitor_key", name="uq_ai_hc_monitor_key"),
        sa.CheckConstraint("status IN ('pass','fail')", name="ck_ai_hc_monitor_status"),
    )
    op.create_index("ix_ai_hc_monitor_org", "ai_high_coverage_monitors", ["organization_id", "authorization_id", "monitored_at"])

    op.create_table(
        "ai_high_coverage_incidents",
        sa.Column("organization_id", sa.Uuid(), nullable=False),
        sa.Column("authorization_id", sa.Uuid(), nullable=False),
        ...
        *_base_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("severity IN ('low','medium','high','critical')", name="ck_ai_hc_inc_severity"),
        sa.CheckConstraint("category IN ('privacy','security','quality','cost','availability','cross_tenant','rollout','other')", name="ck_ai_hc_inc_category"),
        sa.CheckConstraint("status IN ('open','resolved')", name="ck_ai_hc_inc_status"),
    )
    op.create_index("ix_ai_hc_incident_org", "ai_high_coverage_incidents", ["organization_id", "authorization_id", "status"])


def downgrade() -> None:
    op.drop_table("ai_high_coverage_incidents")
    op.drop_table("ai_high_coverage_monitors")
    op.drop_table("ai_high_coverage_runs")
    op.drop_table("ai_high_coverage_document_eligibility")
    op.drop_table("ai_high_coverage_approvals")
    op.drop_table("ai_high_coverage_authorizations")
