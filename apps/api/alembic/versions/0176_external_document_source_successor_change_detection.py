"""Add successor-aware exact-item change detection.

Revision ID: 0176_external_doc_source_successor_change_detection
Revises: 0175_external_doc_source_checkpoint_generation
"""

from alembic import op
import sqlalchemy as sa

revision = "0176_external_doc_source_successor_change_detection"
down_revision = "0175_external_doc_source_checkpoint_generation"
branch_labels = None
depends_on = None

EXECUTION = "external_doc_source_successor_change_detect_execs"
RECEIPT = "external_doc_source_successor_change_detect_receipts"
...
def _safety_columns():
    ...
    dynamic_fields = (
        ...
        "successor_change_detection_completed",
    )
    false_fields = (
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("baseline_generation", sa.Integer(), nullable=False, server_default="2"),
        ...
        *_safety_columns(),
        ...
        sa.UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_scd_exec_request"),
        sa.CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_scd_exec_provider"),
        sa.CheckConstraint("baseline_generation = 2", name="ck_ext_doc_scd_exec_generation"),
        sa.CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_scd_exec_status"),
        sa.CheckConstraint("result_status IS NULL OR result_status IN ('unchanged','changed','missing')", name="ck_ext_doc_scd_exec_result"),
        sa.CheckConstraint("baseline_item_kind IN ('file','folder')", name="ck_ext_doc_scd_exec_base_kind"),
        sa.CheckConstraint("observed_item_kind IS NULL OR observed_item_kind IN ('file','folder')", name="ck_ext_doc_scd_exec_obs_kind"),
        sa.CheckConstraint("baseline_byte_size IS NULL OR (baseline_byte_size >= 0 AND baseline_byte_size <= 1000000000000000)", name="ck_ext_doc_scd_exec_base_size"),
        sa.CheckConstraint("observed_byte_size IS NULL OR (observed_byte_size >= 0 AND observed_byte_size <= 1000000000000000)", name="ck_ext_doc_scd_exec_obs_size"),
        sa.CheckConstraint(
            ...
            "AND provider_client_constructed = false AND exact_item_metadata_read_performed = false AND successor_change_detection_completed = false) OR "
            "(status = 'completed' AND result_status IS NOT NULL AND completed_at IS NOT NULL AND completion_hash IS NOT NULL "
            "AND provider_client_constructed = true AND exact_item_metadata_read_performed = true AND successor_change_detection_completed = true)",
            name="ck_ext_doc_scd_exec_lifecycle",
        ),
        *_safety_constraints("ext_doc_scd_exec"),
    )
    for name, columns in (
        ("ix_ext_doc_scd_exec_org_profile", ["organization_id", "profile_id"]),
        ("ix_ext_doc_scd_exec_checkpoint", ["checkpoint_generation_execution_id"]),
        ("ix_ext_doc_scd_exec_org_status", ["organization_id", "status"]),
        ("ix_ext_doc_scd_exec_org", ["organization_id"]),
        ("ix_ext_doc_scd_exec_profile", ["profile_id"]),
        ("ix_ext_doc_scd_exec_candidate", ["versioned_restaging_execution_id"]),
        ("ix_ext_doc_scd_exec_predecessor", ["predecessor_sync_checkpoint_execution_id"]),
        ("ix_ext_doc_scd_exec_change", ["change_detection_execution_id"]),
        ("ix_ext_doc_scd_exec_listing", ["listing_execution_id"]),
        ("ix_ext_doc_scd_exec_item", ["metadata_item_id"]),
        ("ix_ext_doc_scd_exec_requester", ["requested_by_id"]),
    ):
    ...
    op.create_table(
        ...
        *_safety_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_scd_rcpt_seq"),
        sa.UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_scd_rcpt_hash"),
        sa.CheckConstraint("sequence_number > 0", name="ck_ext_doc_scd_rcpt_seq"),
        sa.CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_scd_rcpt_event"),
        sa.CheckConstraint(
            "(event_type = 'requested' AND status_after = 'requested' AND provider_client_constructed = false AND exact_item_metadata_read_performed = false AND successor_change_detection_completed = false) OR "
            "(event_type = 'completed' AND status_after = 'completed' AND provider_client_constructed = true AND exact_item_metadata_read_performed = true AND successor_change_detection_completed = true)",
            name="ck_ext_doc_scd_rcpt_mapping",
        ),
        sa.CheckConstraint("(sequence_number = 1 AND prior_receipt_hash IS NULL) OR (sequence_number > 1 AND prior_receipt_hash IS NOT NULL)", name="ck_ext_doc_scd_rcpt_chain"),
        *_safety_constraints("ext_doc_scd_rcpt"),
    )
    for name, columns in (
        ("ix_ext_doc_scd_rcpt_seq", ["execution_id", "sequence_number"]),
        ("ix_ext_doc_scd_rcpt_org", ["organization_id"]),
        ("ix_ext_doc_scd_rcpt_execution", ["execution_id"]),
        ("ix_ext_doc_scd_rcpt_actor", ["actor_id"]),
    ):
