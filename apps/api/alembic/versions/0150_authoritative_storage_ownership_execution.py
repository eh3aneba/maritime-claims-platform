"""Add Phase AK bounded authoritative recovery evidence-storage ownership execution.

Revision ID: 0150_authoritative_storage_ownership_execution
Revises: 0149_authoritative_storage_ownership_authorization
"""

from alembic import op
import sqlalchemy as sa

revision = "0150_authoritative_storage_ownership_execution"
down_revision = "0149_authoritative_storage_ownership_authorization"
branch_labels = None
depends_on = None

LEASE = "evidence_recovery_auth_storage_leases"
ROUTE = "evidence_recovery_auth_storage_routes"
RECEIPT = "evidence_recovery_auth_storage_receipts"


def _preservation_columns() -> list[sa.Column]:
    return [
        ...
        sa.Column("write_route_mutation_performed", sa.Boolean(), nullable=False, server_default=sa.false()),
...
def _preservation_constraints(prefix: str) -> list[sa.CheckConstraint]:
    return [
        ...
        sa.CheckConstraint("write_route_mutation_performed = false", name=f"ck_{prefix}_no_write_route"),
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("durable_route_version_at_activation", sa.Integer(), nullable=False),
        sa.Column("authority_route_version_before_activation", sa.Integer(), nullable=False),
        sa.Column("authority_route_version_after_activation", sa.Integer(), nullable=False),
        ...
        sa.Column("ownership_transition_active", sa.Boolean(), nullable=False, server_default=sa.true()),
        ...
        *_preservation_columns(),
        ...
        sa.ForeignKeyConstraint(["authorization_approval_receipt_id"], ["evidence_recovery_auth_storage_authorization_receipts.id"], ondelete="RESTRICT"),
        ...
        sa.UniqueConstraint("authorization_id", name="uq_aso_exec_lease_auth"),
        sa.UniqueConstraint("organization_id", "lease_hash", name="uq_aso_exec_lease_org_hash"),
        sa.CheckConstraint("status IN ('active','rolled_back','expired','invalidated')", name="ck_aso_exec_lease_status"),
        sa.CheckConstraint("source_file_size_bytes >= 0", name="ck_aso_exec_lease_size"),
        sa.CheckConstraint("observed_local_size_bytes = source_file_size_bytes", name="ck_aso_exec_lease_local_size"),
        sa.CheckConstraint("observed_recovery_size_bytes = source_file_size_bytes", name="ck_aso_exec_lease_recovery_size"),
        sa.CheckConstraint("observed_local_hash = source_file_hash", name="ck_aso_exec_lease_local_hash"),
        sa.CheckConstraint("observed_recovery_hash = source_file_hash", name="ck_aso_exec_lease_recovery_hash"),
        sa.CheckConstraint("authority_route_version_before_activation >= 0", name="ck_aso_exec_lease_before_ver"),
        sa.CheckConstraint("authority_route_version_after_activation = authority_route_version_before_activation + 1", name="ck_aso_exec_lease_after_ver"),
        sa.CheckConstraint("((status = 'active' AND ownership_transition_active = true AND local_authoritative = false AND recovery_authoritative = true AND authoritative_storage_changed = true) OR (status <> 'active' AND ownership_transition_active = false AND local_authoritative = true AND recovery_authoritative = false AND authoritative_storage_changed = false))", name="ck_aso_exec_lease_state"),
        *_preservation_constraints("aso_exec_lease"),
    )
    op.create_index("ix_aso_exec_lease_org_claim", LEASE, ["organization_id", "claim_id", "status"])
    op.create_index("ix_aso_exec_lease_org_doc", LEASE, ["organization_id", "document_id", "status"])
    ...
    op.create_table(
        ...
        sa.Column("claim_id", sa.Uuid(), nullable=False),
        sa.Column("document_id", sa.Uuid(), nullable=False),
        sa.Column("authority_kind", sa.String(40), nullable=False, server_default="local_evidence"),
        sa.Column("active_authority_lease_id", sa.Uuid(), nullable=True),
        ...
        sa.Column("recovery_authoritative", sa.Boolean(), nullable=False, server_default=sa.false()),
        ...
        *_preservation_columns(),
        ...
        sa.ForeignKeyConstraint(["claim_id"], ["claims.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["document_id"], ["documents.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["active_authority_lease_id"], [LEASE + ".id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["changed_by_id"], ["users.id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("document_id", name="uq_aso_exec_route_document"),
        sa.CheckConstraint("authority_kind IN ('local_evidence','recovery_storage')", name="ck_aso_exec_route_kind"),
        sa.CheckConstraint("route_version >= 1", name="ck_aso_exec_route_ver"),
        sa.CheckConstraint("((authority_kind = 'local_evidence' AND active_authority_lease_id IS NULL AND local_authoritative = true AND recovery_authoritative = false AND authoritative_storage_changed = false) OR (authority_kind = 'recovery_storage' AND active_authority_lease_id IS NOT NULL AND local_authoritative = false AND recovery_authoritative = true AND authoritative_storage_changed = true))", name="ck_aso_exec_route_state"),
        *_preservation_constraints("aso_exec_route"),
    )
    op.create_index("ix_aso_exec_route_org_claim", ROUTE, ["organization_id", "claim_id"])
    ...
    op.create_table(
        ...
        sa.Column("from_authority_kind", sa.String(40), nullable=False),
        sa.Column("to_authority_kind", sa.String(40), nullable=False),
        ...
        sa.Column("local_authoritative", sa.Boolean(), nullable=False),
        sa.Column("recovery_authoritative", sa.Boolean(), nullable=False),
        sa.Column("authoritative_storage_changed", sa.Boolean(), nullable=False),
        ...
        *_preservation_columns(),
        ...
        sa.ForeignKeyConstraint(["claim_id"], ["claims.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["document_id"], ["documents.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["lease_id"], [LEASE + ".id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["authorization_id"], ["evidence_recovery_auth_storage_authorizations.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["actor_id"], ["users.id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("organization_id", "receipt_hash", name="uq_aso_exec_rec_org_hash"),
        sa.CheckConstraint("phase IN ('activated','rolled_back','expired','invalidated')", name="ck_aso_exec_rec_phase"),
        sa.CheckConstraint("from_authority_kind IN ('local_evidence','recovery_storage')", name="ck_aso_exec_rec_from"),
        sa.CheckConstraint("to_authority_kind IN ('local_evidence','recovery_storage')", name="ck_aso_exec_rec_to"),
        sa.CheckConstraint("route_version >= 1", name="ck_aso_exec_rec_ver"),
        sa.CheckConstraint("((phase = 'activated' AND local_authoritative = false AND recovery_authoritative = true AND authoritative_storage_changed = true) OR (phase <> 'activated' AND local_authoritative = true AND recovery_authoritative = false AND authoritative_storage_changed = false))", name="ck_aso_exec_rec_state"),
        *_preservation_constraints("aso_exec_rec"),
    )
    op.create_index("ix_aso_exec_rec_time", RECEIPT, ["lease_id", "transitioned_at"])


def downgrade() -> None:
    op.drop_index("ix_aso_exec_rec_time", table_name=RECEIPT)
    op.drop_table(RECEIPT)
    op.drop_index("ix_aso_exec_route_org_claim", table_name=ROUTE)
    op.drop_table(ROUTE)
    op.drop_index("ix_aso_exec_lease_org_doc", table_name=LEASE)
    op.drop_index("ix_aso_exec_lease_org_claim", table_name=LEASE)
    op.drop_table(LEASE)
