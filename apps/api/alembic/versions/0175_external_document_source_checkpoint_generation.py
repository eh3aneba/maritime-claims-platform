"""Add bounded checkpoint-generation advancement.

Revision ID: 0175_external_doc_source_checkpoint_generation
Revises: 0174_external_doc_source_versioned_restaging
"""

from alembic import op
import sqlalchemy as sa

revision = "0175_external_doc_source_checkpoint_generation"
down_revision = "0174_external_doc_source_versioned_restaging"
branch_labels = None
depends_on = None

EXECUTION = "external_doc_source_checkpoint_generation_execs"
RECEIPT = "external_doc_source_checkpoint_generation_receipts"
...
def _safety_columns():
    ...
    dynamic_fields = (
        "checkpoint_created",
        "checkpoint_advanced",
        "checkpoint_generation_advance_completed",
    )
    false_fields = (
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("predecessor_checkpoint_generation", sa.Integer(), nullable=False, server_default="1"),
        ...
        *_safety_columns(),
        ...
        sa.UniqueConstraint("versioned_restaging_execution_id", name="uq_ext_doc_cg_exec_restage"),
        sa.UniqueConstraint("predecessor_sync_checkpoint_execution_id", name="uq_ext_doc_cg_exec_predecessor"),
        sa.UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_cg_exec_request"),
        sa.CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_cg_exec_provider"),
        sa.CheckConstraint("predecessor_checkpoint_generation = 1", name="ck_ext_doc_cg_exec_predecessor_generation"),
        sa.CheckConstraint("candidate_generation = 2", name="ck_ext_doc_cg_exec_candidate_generation"),
        sa.CheckConstraint(f"successor_checkpoint_kind = '{KIND}'", name="ck_ext_doc_cg_exec_kind"),
        sa.CheckConstraint("successor_checkpoint_generation = 2", name="ck_ext_doc_cg_exec_generation"),
        sa.CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_cg_exec_status"),
        sa.CheckConstraint("result_status IS NULL OR result_status = 'checkpoint_generation_advanced'", name="ck_ext_doc_cg_exec_result"),
        sa.CheckConstraint("content_byte_count >= 0 AND content_byte_count <= 8388608", name="ck_ext_doc_cg_exec_size"),
        sa.CheckConstraint(
            "(status = 'requested' AND result_status IS NULL AND completed_at IS NULL AND completion_hash IS NULL AND checkpoint_created = false AND checkpoint_advanced = false AND checkpoint_generation_advance_completed = false) OR "
            "(status = 'completed' AND result_status = 'checkpoint_generation_advanced' AND completed_at IS NOT NULL AND completion_hash IS NOT NULL AND checkpoint_created = true AND checkpoint_advanced = true AND checkpoint_generation_advance_completed = true)",
            name="ck_ext_doc_cg_exec_lifecycle",
        ),
        *_safety_constraints("ext_doc_cg_exec"),
    )
    for name, columns in (
        ("ix_ext_doc_cg_exec_org_profile", ["organization_id", "profile_id"]),
        ("ix_ext_doc_cg_exec_restage", ["versioned_restaging_execution_id"]),
        ("ix_ext_doc_cg_exec_predecessor", ["predecessor_sync_checkpoint_execution_id"]),
        ("ix_ext_doc_cg_exec_org_status", ["organization_id", "status"]),
        ("ix_ext_doc_cg_exec_org", ["organization_id"]),
        ("ix_ext_doc_cg_exec_profile", ["profile_id"]),
        ("ix_ext_doc_cg_exec_change", ["change_detection_execution_id"]),
        ("ix_ext_doc_cg_exec_stage", ["original_staging_execution_id"]),
        ("ix_ext_doc_cg_exec_listing", ["listing_execution_id"]),
        ("ix_ext_doc_cg_exec_item", ["metadata_item_id"]),
        ("ix_ext_doc_cg_exec_requester", ["requested_by_id"]),
    ):
    ...
    op.create_table(
        ...
        *_safety_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_cg_rcpt_seq"),
        sa.UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_cg_rcpt_hash"),
        sa.CheckConstraint("sequence_number > 0", name="ck_ext_doc_cg_rcpt_seq"),
        sa.CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_cg_rcpt_event"),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint("(sequence_number = 1 AND prior_receipt_hash IS NULL) OR (sequence_number > 1 AND prior_receipt_hash IS NOT NULL)", name="ck_ext_doc_cg_rcpt_chain"),
        *_safety_constraints("ext_doc_cg_rcpt"),
    )
    for name, columns in (
        ("ix_ext_doc_cg_rcpt_seq", ["execution_id", "sequence_number"]),
        ("ix_ext_doc_cg_rcpt_org", ["organization_id"]),
        ("ix_ext_doc_cg_rcpt_execution", ["execution_id"]),
        ("ix_ext_doc_cg_rcpt_actor", ["actor_id"]),
    ):
