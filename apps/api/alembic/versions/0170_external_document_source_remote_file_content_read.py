"""Add bounded remote file-content read proof.

Revision ID: 0170_external_doc_source_remote_file_content_read
Revises: 0169_external_doc_source_remote_metadata_listing
"""

from alembic import op
import sqlalchemy as sa

revision = "0170_external_doc_source_remote_file_content_read"
down_revision = "0169_external_doc_source_remote_metadata_listing"
branch_labels = None
depends_on = None

EXECUTION = "external_doc_source_remote_content_read_execs"
RECEIPT = "external_doc_source_remote_content_read_receipts"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("listing_completion_hash", sa.String(64), nullable=False),
        sa.Column("listing_items_hash", sa.String(64), nullable=False),
        sa.Column("metadata_item_hash", sa.String(64), nullable=False),
        sa.Column("metadata_item_version_token_hash", sa.String(64), nullable=True),
        sa.Column("declared_byte_size", sa.BigInteger(), nullable=True),
        sa.Column("metadata_mime_type_class", sa.String(128), nullable=True),
        ...
        *_safety_columns(),
        ...
        sa.UniqueConstraint("metadata_item_id", name="uq_ext_doc_rcr_exec_item"),
        sa.UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_rcr_exec_request"),
        sa.CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_rcr_exec_provider"),
        sa.CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_rcr_exec_status"),
        sa.CheckConstraint("result_status IS NULL OR result_status = 'read_verified'", name="ck_ext_doc_rcr_exec_result"),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint("latency_class IS NULL OR latency_class IN ('fast','normal','slow','unknown')", name="ck_ext_doc_rcr_exec_latency"),
        sa.CheckConstraint(
    ...
    op.create_index("ix_ext_doc_rcr_exec_org_profile", EXECUTION, ["organization_id", "profile_id"])
    op.create_index("ix_ext_doc_rcr_exec_org_status", EXECUTION, ["organization_id", "status"])
    for name, column in (
        ("ix_ext_doc_rcr_exec_org", "organization_id"),
        ("ix_ext_doc_rcr_exec_profile", "profile_id"),
        ("ix_ext_doc_rcr_exec_listing", "listing_execution_id"),
        ("ix_ext_doc_rcr_exec_item", "metadata_item_id"),
        ("ix_ext_doc_rcr_exec_health", "provider_client_health_execution_id"),
        ("ix_ext_doc_rcr_exec_token", "token_acquisition_execution_id"),
        ("ix_ext_doc_rcr_exec_resolution", "credential_resolution_execution_id"),
        ("ix_ext_doc_rcr_exec_binding", "credential_reference_binding_id"),
        ("ix_ext_doc_rcr_exec_requester", "requested_by_id"),
    ):
    ...
    op.create_table(
        ...
        *_safety_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_rcr_rcpt_seq"),
        sa.UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_rcr_rcpt_hash"),
        sa.CheckConstraint("sequence_number > 0", name="ck_ext_doc_rcr_rcpt_seq"),
        sa.CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_rcr_rcpt_event"),
        sa.CheckConstraint(
    ...
    op.create_index("ix_ext_doc_rcr_rcpt_seq", RECEIPT, ["execution_id", "sequence_number"])
    op.create_index("ix_ext_doc_rcr_rcpt_org", RECEIPT, ["organization_id"])
    op.create_index("ix_ext_doc_rcr_rcpt_exec", RECEIPT, ["execution_id"])
    op.create_index("ix_ext_doc_rcr_rcpt_actor", RECEIPT, ["actor_id"])
