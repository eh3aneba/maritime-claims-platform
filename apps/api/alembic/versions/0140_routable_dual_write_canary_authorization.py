"""Add Phase AA routable dual-write canary authorization.

Revision ID: 0140_routable_dual_write_canary_authorization
Revises: 0139_dual_write_rehearsal_health
"""

from alembic import op
import sqlalchemy as sa

revision = "0140_routable_dual_write_canary_authorization"
down_revision = "0139_dual_write_rehearsal_health"
branch_labels = None
depends_on = None

AUTH = "evidence_recovery_routable_dual_write_canary_authorizations"
RECEIPT = "evidence_recovery_routable_dual_write_canary_auth_receipts"


def _safety_columns() -> list[sa.Column]:
    return [
        sa.Column("storage_write_performed", sa.Boolean(), nullable=False, server_default=sa.false()),
        sa.Column("canary_executed", sa.Boolean(), nullable=False, server_default=sa.false()),
...
def _safety_constraints(prefix: str) -> list[sa.CheckConstraint]:
    return [
        ...
        sa.CheckConstraint("canary_executed = false", name=f"ck_{prefix}_no_exec"),
        sa.CheckConstraint("routable_dual_write_active = false", name=f"ck_{prefix}_no_routable_dual"),
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("phase_z_health_receipt_id", sa.Uuid(), nullable=False),
        ...
        sa.Column("phase_z_request_snapshot_hash", sa.String(64), nullable=False),
        sa.Column("phase_z_integrity_proof_hash", sa.String(64), nullable=False),
        sa.Column("phase_z_health_receipt_hash", sa.String(64), nullable=False),
        ...
        sa.Column("max_canary_windows", sa.Integer(), nullable=False, server_default="1"),
        ...
        *_safety_columns(),
        ...
        sa.ForeignKeyConstraint(["phase_z_health_receipt_id"], ["evidence_recovery_dual_write_rehearsal_health_receipts.id"], ondelete="RESTRICT"),
        ...
        sa.UniqueConstraint("phase_z_health_qualification_id", name="uq_dw_can_auth_z_health"),
        sa.UniqueConstraint("organization_id", "authorization_hash", name="uq_dw_can_auth_org_hash"),
        sa.CheckConstraint("status IN ('pending_second_approval','approved','rejected','expired','invalidated')", name="ck_dw_can_auth_status"),
        sa.CheckConstraint("health_state = 'healthy'", name="ck_dw_can_auth_healthy"),
        sa.CheckConstraint("requested_by_id <> approved_by_id OR approved_by_id IS NULL", name="ck_dw_can_auth_four_eyes"),
        sa.CheckConstraint("phase_z_qualified_by_id <> approved_by_id OR approved_by_id IS NULL", name="ck_dw_can_auth_z_split"),
        sa.CheckConstraint("phase_y_executed_by_id <> approved_by_id OR approved_by_id IS NULL", name="ck_dw_can_auth_y_split"),
        sa.CheckConstraint("phase_x_approved_by_id <> approved_by_id OR approved_by_id IS NULL", name="ck_dw_can_auth_x_split"),
        sa.CheckConstraint("source_file_size_bytes >= 0", name="ck_dw_can_auth_size"),
        sa.CheckConstraint("observed_file_size_bytes = source_file_size_bytes", name="ck_dw_can_auth_obs_size"),
        sa.CheckConstraint("route_version_at_request >= 1", name="ck_dw_can_auth_routever"),
        sa.CheckConstraint("max_canary_windows = 1", name="ck_dw_can_auth_one_window"),
        sa.CheckConstraint("status <> 'approved' OR authorization_expires_at IS NOT NULL", name="ck_dw_can_auth_approved_exp"),
        sa.CheckConstraint("status = 'approved' OR authorization_expires_at IS NULL", name="ck_dw_can_auth_nonapproved_exp"),
        *_safety_constraints("dw_can_auth"),
    )
    op.create_index("ix_dw_can_auth_org_claim", AUTH, ["organization_id", "claim_id", "status"])
    op.create_index("ix_dw_can_auth_org_doc", AUTH, ["organization_id", "document_id", "status"])
    ...
    op.create_table(
        ...
        *_safety_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("organization_id", "receipt_hash", name="uq_dw_can_rec_org_hash"),
        sa.CheckConstraint("phase IN ('requested','approved','rejected','expired','invalidated')", name="ck_dw_can_rec_phase"),
        sa.CheckConstraint("health_state = 'healthy'", name="ck_dw_can_rec_healthy"),
        *_safety_constraints("dw_can_rec"),
    )
    op.create_index("ix_dw_can_rec_time", RECEIPT, ["authorization_id", "transitioned_at"])


def downgrade() -> None:
    op.drop_index("ix_dw_can_rec_time", table_name=RECEIPT)
    op.drop_table(RECEIPT)
    op.drop_index("ix_dw_can_auth_org_doc", table_name=AUTH)
    op.drop_index("ix_dw_can_auth_org_claim", table_name=AUTH)
    op.drop_table(AUTH)
