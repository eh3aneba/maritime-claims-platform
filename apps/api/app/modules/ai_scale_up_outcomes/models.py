...
class AIScaleUpOutcomeAssessment(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_scale_up_outcome_assessments"
    __table_args__ = (
        UniqueConstraint("organization_id", "assessment_key", name="uq_ai_su_outcome_org_key"),
        UniqueConstraint("scale_up_authorization_id", "attempt_number", name="uq_ai_su_outcome_attempt"),
        Index("ix_ai_su_outcome_org_status", "organization_id", "status", "created_at"),
    )
    ...
    assessment_profile: Mapped[str] = mapped_column(String(80), default="controlled_scale_up_readiness_v1", server_default="controlled_scale_up_readiness_v1")
    ...
    min_reviewed_runs: Mapped[int] = mapped_column(Integer, default=20, server_default="20")
    min_runs_per_workflow: Mapped[int] = mapped_column(Integer, default=5, server_default="5")
    ...
    min_mean_usefulness_bps: Mapped[int] = mapped_column(Integer, default=8600, server_default="8600")
    ...
    max_mean_review_seconds: Mapped[int] = mapped_column(Integer, default=420, server_default="420")
...
class AIScaleUpOutcomeObservation(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_scale_up_outcome_observations"
    __table_args__ = (
        UniqueConstraint("assessment_id", "scale_up_run_id", name="uq_ai_su_outcome_obs_run"),
        Index("ix_ai_su_outcome_obs_org", "organization_id", "assessment_id", "workflow_type"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    assessment_id: Mapped[UUID] = mapped_column(ForeignKey("ai_scale_up_outcome_assessments.id", ondelete="CASCADE"), index=True)
    scale_up_run_id: Mapped[UUID] = mapped_column(ForeignKey("ai_scale_up_runs.id", ondelete="RESTRICT"), index=True)
...
class AIScaleUpOutcomeReview(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_scale_up_outcome_reviews"
    __table_args__ = (
        UniqueConstraint("assessment_id", "review_role", name="uq_ai_su_outcome_review_role"),
        Index("ix_ai_su_outcome_review_org", "organization_id", "assessment_id", "review_role"),
    )
