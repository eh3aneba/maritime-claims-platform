...
ApprovalRole = Literal["security", "privacy", "product", "operations", "risk"]


class AIScaleUpCreate(BaseModel):
    ...
    rollout_percentage: int = Field(ge=11, le=25)
    max_claims: int = Field(ge=1, le=25)
    max_documents: int = Field(ge=1, le=75)
    max_users: int = Field(ge=1, le=25)
    max_provider_runs: int = Field(ge=1, le=250)
    starts_at: datetime
    expires_at: datetime
    ...
    confirm_separate_controlled_scale_up: bool


class AIScaleUpApprovalWrite(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIScaleUpDecision(BaseModel):
    model_config = ConfigDict(extra="forbid")
    outcome: Literal["authorize_scale_up", "hold"]
    confirm_decision: bool
    note: str = Field(min_length=10, max_length=4000)


class AIScaleUpDocumentCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
    claim_id: UUID
    document_id: UUID
    ...
    change_ticket_reference: str = Field(min_length=8, max_length=500)
    note: str = Field(min_length=10, max_length=4000)
    confirm_new_scale_up_eligibility: bool


class AIScaleUpRunOutcome(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIScaleUpMonitorCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIScaleUpIncidentCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIScaleUpIncidentResolve(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIScaleUpResume(BaseModel):
    model_config = ConfigDict(extra="forbid")
    confirm_resume: bool
    note: str = Field(min_length=10, max_length=4000)


class AIScaleUpRevoke(BaseModel):
    model_config = ConfigDict(extra="forbid")
    confirm_revoke: bool
    note: str = Field(min_length=10, max_length=4000)


class AIScaleUpComplete(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIScaleUpApprovalResponse(_ORMResponse):
...
class AIScaleUpDocumentResponse(_ORMResponse):
...
class AIScaleUpRunResponse(_ORMResponse):
    ...
    human_edit_count: int | None; unsupported_output_count: int | None
    source_grounded_output_count: int | None; source_grounding_total_count: int | None
    latency_ms: int | None; observed_provider_cost_microusd: int | None
    evidence_reference: str | None; note: str | None; outcome_hash: str | None
    queued_at: datetime; reviewed_at: datetime | None; created_at: datetime


class AIScaleUpMonitorResponse(_ORMResponse):
...
class AIScaleUpIncidentResponse(_ORMResponse):
...
class AIScaleUpResponse(BaseModel):
    id: UUID; outcome_assessment_id: UUID; limited_production_authorization_id: UUID
    requested_by_id: UUID | None; finalized_by_id: UUID | None; revoked_by_id: UUID | None
    attempt_number: int; authorization_key: str; environment: str; authorization_mode: str
    outcome_assessment_hash: str; outcome_decision_hash: str
    ...
    previous_rollout_percentage: int; rollout_percentage: int
    max_claims: int; max_documents: int; max_users: int; max_provider_runs: int
    starts_at: datetime; expires_at: datetime; controls: dict; references: dict
    status: str; outcome: str | None; decision_note: str | None; decision_hash: str | None
    decided_at: datetime | None; completed_at: datetime | None; completion_note: str | None
    revoked_at: datetime | None; revocation_note: str | None
    approvals: list[AIScaleUpApprovalResponse]; document_eligibility: list[AIScaleUpDocumentResponse]
    runs: list[AIScaleUpRunResponse]; monitors: list[AIScaleUpMonitorResponse]
    incidents: list[AIScaleUpIncidentResponse]; summary: dict; created_at: datetime


class AIScaleUpDashboard(BaseModel):
    authorizations: list[AIScaleUpResponse]
