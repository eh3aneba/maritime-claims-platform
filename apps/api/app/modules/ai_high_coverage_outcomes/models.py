...
class AIHighCoverageOutcomeAssessment(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_high_coverage_outcome_assessments"
    __table_args__ = (
        UniqueConstraint("organization_id", "assessment_key", name="uq_ai_hco_org_key"),
        UniqueConstraint("high_coverage_authorization_id", "attempt_number", name="uq_ai_hco_attempt"),
        Index("ix_ai_hco_org_status", "organization_id", "status", "created_at"),
    )
    ...
    assessment_profile: Mapped[str] = mapped_column(String(80), default="high_coverage_final_readiness_v1", server_default="high_coverage_final_readiness_v1")
    ...
    min_reviewed_runs: Mapped[int] = mapped_column(Integer, default=80, server_default="80")
    min_runs_per_workflow: Mapped[int] = mapped_column(Integer, default=20, server_default="20")
    ...
    min_mean_usefulness_bps: Mapped[int] = mapped_column(Integer, default=9000, server_default="9000")
    ...
    max_mean_review_seconds: Mapped[int] = mapped_column(Integer, default=300, server_default="300")
...
class AIHighCoverageOutcomeObservation(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_high_coverage_outcome_observations"
    __table_args__ = (
        UniqueConstraint("assessment_id", "high_coverage_run_id", name="uq_ai_hco_obs_run"),
        Index("ix_ai_hco_obs_org", "organization_id", "assessment_id", "workflow_type"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    assessment_id: Mapped[UUID] = mapped_column(ForeignKey("ai_high_coverage_outcome_assessments.id", ondelete="CASCADE"), index=True)
    high_coverage_run_id: Mapped[UUID] = mapped_column(ForeignKey("ai_high_coverage_runs.id", ondelete="RESTRICT"), index=True)
...
class AIHighCoverageOutcomeReview(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_high_coverage_outcome_reviews"
    __table_args__ = (
        UniqueConstraint("assessment_id", "review_role", name="uq_ai_hco_review_role"),
        Index("ix_ai_hco_review_org", "organization_id", "assessment_id", "review_role"),
    )
