...
class AIBoundedFullProductionAuthorization(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_bounded_full_production_authorizations"
    __table_args__ = (
        UniqueConstraint("organization_id", "authorization_key", name="uq_ai_bfp_org_key"),
        UniqueConstraint("near_universal_outcome_assessment_id", "attempt_number", name="uq_ai_bfp_attempt"),
        Index("ix_ai_bfp_org_status", "organization_id", "status", "created_at"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    near_universal_outcome_assessment_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_outcome_assessments.id", ondelete="RESTRICT"), index=True)
    near_universal_authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_authorizations.id", ondelete="RESTRICT"), index=True)
    ...
    authorization_mode: Mapped[str] = mapped_column(String(80), default="bounded_full_100_percent", server_default="bounded_full_100_percent")
    ...
    rollout_percentage: Mapped[int] = mapped_column(Integer, default=100, server_default="100")

    previous_max_claims: Mapped[int] = mapped_column(Integer)
    previous_max_documents: Mapped[int] = mapped_column(Integer)
    previous_max_users: Mapped[int] = mapped_column(Integer)
    previous_max_provider_runs: Mapped[int] = mapped_column(Integer)
    ...
    platform_reliability_reference: Mapped[str] = mapped_column(String(500))
    data_protection_reference: Mapped[str] = mapped_column(String(500))
    executive_sponsor_reference: Mapped[str] = mapped_column(String(500))
...
class AIBoundedFullProductionApproval(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_bounded_full_production_approvals"
    __table_args__ = (
        UniqueConstraint("authorization_id", "approval_role", name="uq_ai_bfp_approval_role"),
        Index("ix_ai_bfp_approval_org", "organization_id", "authorization_id"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_bounded_full_production_authorizations.id", ondelete="CASCADE"), index=True)
    approver_id: Mapped[UUID | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    approval_role: Mapped[str] = mapped_column(String(60))
...
class AIBoundedFullProductionDocumentEligibility(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_bounded_full_production_document_eligibility"
    __table_args__ = (
        UniqueConstraint("authorization_id", "document_id", "attestation_number", name="uq_ai_bfp_doc_attempt"),
        Index("ix_ai_bfp_doc_org", "organization_id", "authorization_id", "status"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_bounded_full_production_authorizations.id", ondelete="CASCADE"), index=True)
...
class AIBoundedFullProductionRun(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_bounded_full_production_runs"
    __table_args__ = (
        UniqueConstraint("authorization_id", "run_key", name="uq_ai_bfp_run_key"),
        UniqueConstraint("processing_job_id", name="uq_ai_bfp_processing_job"),
        Index("ix_ai_bfp_run_org", "organization_id", "authorization_id", "status"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_bounded_full_production_authorizations.id", ondelete="CASCADE"), index=True)
    eligibility_id: Mapped[UUID] = mapped_column(ForeignKey("ai_bounded_full_production_document_eligibility.id", ondelete="RESTRICT"), index=True)
    claim_id: Mapped[UUID] = mapped_column(ForeignKey("claims.id", ondelete="RESTRICT"), index=True)
    document_id: Mapped[UUID] = mapped_column(ForeignKey("documents.id", ondelete="RESTRICT"), index=True)
    requested_by_id: Mapped[UUID | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class AIBoundedFullProductionMonitor(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_bounded_full_production_monitors"
    __table_args__ = (
        UniqueConstraint("authorization_id", "monitor_key", name="uq_ai_bfp_monitor_key"),
        Index("ix_ai_bfp_monitor_org", "organization_id", "authorization_id", "monitored_at"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_bounded_full_production_authorizations.id", ondelete="CASCADE"), index=True)
...
class AIBoundedFullProductionIncident(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_bounded_full_production_incidents"
    __table_args__ = (Index("ix_ai_bfp_incident_org", "organization_id", "authorization_id", "status"),)
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_bounded_full_production_authorizations.id", ondelete="CASCADE"), index=True)
