...
class AINearUniversalAuthorization(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_near_universal_authorizations"
    __table_args__ = (
        UniqueConstraint("organization_id", "authorization_key", name="uq_ai_nu_org_key"),
        UniqueConstraint("outcome_assessment_id", "attempt_number", name="uq_ai_nu_attempt"),
        Index("ix_ai_nu_org_status", "organization_id", "status", "created_at"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    outcome_assessment_id: Mapped[UUID] = mapped_column(ForeignKey("ai_final_production_outcome_assessments.id", ondelete="RESTRICT"), index=True)
    ...
    authorization_mode: Mapped[str] = mapped_column(String(80), default="near_universal_bounded_91_99", server_default="near_universal_bounded_91_99")
...
class AINearUniversalApproval(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_near_universal_approvals"
    __table_args__ = (
        UniqueConstraint("authorization_id", "approval_role", name="uq_ai_nu_approval_role"),
        Index("ix_ai_nu_approval_org", "organization_id", "authorization_id"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_authorizations.id", ondelete="CASCADE"), index=True)
...
class AINearUniversalDocumentEligibility(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_near_universal_document_eligibility"
    __table_args__ = (
        UniqueConstraint("authorization_id", "document_id", "attestation_number", name="uq_ai_nu_doc_attempt"),
        Index("ix_ai_nu_doc_org", "organization_id", "authorization_id", "status"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_authorizations.id", ondelete="CASCADE"), index=True)
...
class AINearUniversalRun(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_near_universal_runs"
    __table_args__ = (
        UniqueConstraint("authorization_id", "run_key", name="uq_ai_nu_run_key"),
        UniqueConstraint("processing_job_id", name="uq_ai_nu_processing_job"),
        Index("ix_ai_nu_run_org", "organization_id", "authorization_id", "status"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_authorizations.id", ondelete="CASCADE"), index=True)
    eligibility_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_document_eligibility.id", ondelete="RESTRICT"), index=True)
    claim_id: Mapped[UUID] = mapped_column(ForeignKey("claims.id", ondelete="RESTRICT"), index=True)
    document_id: Mapped[UUID] = mapped_column(ForeignKey("documents.id", ondelete="RESTRICT"), index=True)
    requested_by_id: Mapped[UUID | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class AINearUniversalMonitor(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_near_universal_monitors"
    __table_args__ = (
        UniqueConstraint("authorization_id", "monitor_key", name="uq_ai_nu_monitor_key"),
        Index("ix_ai_nu_monitor_org", "organization_id", "authorization_id", "monitored_at"),
    )
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_authorizations.id", ondelete="CASCADE"), index=True)
...
class AINearUniversalIncident(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "ai_near_universal_incidents"
    __table_args__ = (Index("ix_ai_nu_incident_org", "organization_id", "authorization_id", "status"),)
    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), index=True)
    authorization_id: Mapped[UUID] = mapped_column(ForeignKey("ai_near_universal_authorizations.id", ondelete="CASCADE"), index=True)
