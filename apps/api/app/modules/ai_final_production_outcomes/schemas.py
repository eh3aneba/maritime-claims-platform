...
class AIFinalProductionOutcomeCreate(BaseModel):
...
class AIFinalProductionOutcomeObservationCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
    final_production_run_id: UUID
...
class AIFinalProductionOutcomeBusinessCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
    claim_id: UUID
...
class AIFinalProductionOutcomeFinalize(BaseModel):
    model_config = ConfigDict(extra="forbid")
    confirm_finalize: bool
    note: str = Field(min_length=10, max_length=4000)


class AIFinalProductionOutcomeReviewWrite(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIFinalProductionOutcomeDecision(BaseModel):
    model_config = ConfigDict(extra="forbid")
    outcome: Literal[
...
class AIFinalProductionOutcomeObservationResponse(BaseModel):
...
class AIFinalProductionOutcomeBusinessResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: UUID
    assessment_id: UUID
    claim_id: UUID
...
class AIFinalProductionOutcomeReviewResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: UUID
    assessment_id: UUID
...
class AIFinalProductionOutcomeResponse(BaseModel):
    id: UUID
    final_production_authorization_id: UUID
    ...
    final_production_decision_hash: str
    final_production_completion_hash: str
    ...
    observations: list[AIFinalProductionOutcomeObservationResponse]
    business_evidence: list[AIFinalProductionOutcomeBusinessResponse]
    reviews: list[AIFinalProductionOutcomeReviewResponse]
    summary: dict
    created_at: datetime


class AIFinalProductionOutcomeDashboard(BaseModel):
    assessments: list[AIFinalProductionOutcomeResponse]
