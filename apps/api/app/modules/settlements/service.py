...
def create_settlement(db: Session, claim: Claim, user: User, payload: SettlementCreate) -> SettlementProposal:
    ...
    source = adjustment_source_state(db, statement=adjustment, user_id=user.id)
    if source["status"] != "current":
        raise HTTPException(
            409,
            "The approved adjustment is historical against the current financial evidence. Create, review and approve a current rebased adjustment before opening a new settlement proposal.",
        )
    ...
    version = (
        db.scalar(select(func.max(SettlementProposal.version)).where(SettlementProposal.claim_id == claim.id)) or 0
    ) + 1
    item = SettlementProposal(
        ...
        without_prejudice=payload.without_prejudice,
        expires_on=payload.expires_on,
        source_adjustment_hash=adjustment.content_hash,
        source_snapshot={
            "adjustment_statement_id": str(adjustment.id),
            "version": adjustment.version,
            "currency": adjustment.currency,
            "net_adjusted": str(adjustment.net_adjusted),
            "content_hash": adjustment.content_hash,
            "adjustment_source_state_hash": adjustment.source_state_hash,
        },
    )
    db.add(item)
    db.flush()
    _audit(
        ...
        obj=item,
        user=user,
        action="CREATE_SETTLEMENT_PROPOSAL",
...
def review_settlement(db: Session, item: SettlementProposal, user: User, approve: bool, note: str) -> SettlementProposal:
        ...
        item.content_hash = _hash(
            {
                ...
                "adjustment_hash": item.source_adjustment_hash,
                "title": item.title,
                "type": item.settlement_type.value,
                "currency": item.currency,
                "amount": str(item.amount),
                "terms": item.terms,
                "release_required": item.release_required,
                "without_prejudice": item.without_prejudice,
                "expires_on": str(item.expires_on) if item.expires_on else None,
            }
        )
    else:
        item.content_hash = None
    _audit(
        ...
        obj=item,
        user=user,
        action="APPROVE_SETTLEMENT_PROPOSAL" if approve else "REJECT_SETTLEMENT_PROPOSAL",
...
def record_disposition(db: Session, item: SettlementProposal, user: User, disposition: str, note: str) -> SettlementProposal:
    ...
    allowed = {
        ...
        "withdrawn": SettlementStatus.WITHDRAWN,
    }
    if disposition not in allowed:
        raise HTTPException(422, "Disposition must be accepted, declined or withdrawn")
    item.status = allowed[disposition]
    item.disposition_by_id = user.id
    item.disposition_at = datetime.now(UTC)
    item.disposition_note = note.strip()
    _audit(
        db,
        obj=item,
        user=user,
        action=f"RECORD_SETTLEMENT_{disposition.upper()}",
        values={"status": item.status.value},
        details="External outcome recorded manually; the platform did not communicate or accept it.",
    )
    db.commit()
    db.refresh(item)
    return item
...
def create_payment(db: Session, claim: Claim, user: User, payload: PaymentCreate) -> PaymentAuthorization:
    ...
    sequence = (
        db.scalar(select(func.max(PaymentAuthorization.sequence)).where(PaymentAuthorization.settlement_id == settlement.id)) or 0
    ) + 1
    ...
    _audit(
        ...
        obj=item,
        user=user,
        action="CREATE_PAYMENT_AUTHORIZATION",
...
def approve_payment(db: Session, item: PaymentAuthorization, user: User, note: str) -> PaymentAuthorization:
    ...
    if item.status == PaymentStatus.UNDER_REVIEW:
        ...
        item.first_approved_by_id = user.id
        ...
        item.second_approval_note = note.strip()
        item.content_hash = _hash(
            {
                "id": str(item.id),
...
def reject_payment(db: Session, item: PaymentAuthorization, user: User, note: str) -> PaymentAuthorization:
    ...
    item.rejection_note = note.strip()
    item.first_approved_by_id = None
    item.first_approved_at = None
    item.first_approval_note = None
...
def record_paid(
    ...
    *,
    ...
    item.paid_channel = channel.strip()
    item.external_reference = external_reference.strip()
    item.value_date = value_date
    item.paid_note = (note or "").strip() or None
    item.paid_recorded_by_id = user.id
    item.paid_recorded_at = datetime.now(UTC)
    _audit(
        db,
        obj=item,
        user=user,
        action="RECORD_PAYMENT_PAID_EXTERNALLY",
        values={
            "status": item.status.value,
            "channel": item.paid_channel,
