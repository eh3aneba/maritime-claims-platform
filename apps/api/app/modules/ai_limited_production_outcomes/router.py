...
from app.modules.ai_limited_production_outcomes.schemas import (
    ...
    AILimitedProductionOutcomeDashboard,
    AILimitedProductionOutcomeDecision,
    AILimitedProductionOutcomeFinalize,
    AILimitedProductionOutcomeObservationCreate,
    AILimitedProductionOutcomeResponse,
    AILimitedProductionOutcomeReviewWrite,
)
from app.modules.ai_limited_production_outcomes.service import (
    create_assessment,
    decide_outcome,
    finalize_assessment,
    get_assessment,
...
router = APIRouter(
    prefix="/ai-limited-production-outcomes",
    tags=["ai-limited-production-outcomes"],
)
Manager = Annotated[User, Depends(require_roles(UserRole.ADMIN, UserRole.CLAIMS_MANAGER))]
Admin = Annotated[User, Depends(require_roles(UserRole.ADMIN))]


@router.get("", response_model=AILimitedProductionOutcomeDashboard)
def dashboard(current_user: CurrentUser, db: Annotated[Session, Depends(get_db)]):
    return AILimitedProductionOutcomeDashboard(
        assessments=list_assessments(db, current_user.organization_id))


@router.post("/assessments", response_model=AILimitedProductionOutcomeResponse, status_code=201)
def assessment_create(payload: AILimitedProductionOutcomeCreate, manager: Manager,
                      db: Annotated[Session, Depends(get_db)]):
    return create_assessment(db, manager, payload)


@router.post("/assessments/{assessment_id}/observations",
             response_model=AILimitedProductionOutcomeResponse, status_code=201)
...
@router.post("/assessments/{assessment_id}/finalize",
             response_model=AILimitedProductionOutcomeResponse)
def assessment_finalize(
    assessment_id: UUID,
    payload: AILimitedProductionOutcomeFinalize,
    manager: Manager,
    db: Annotated[Session, Depends(get_db)],
):
...
def assessment_review(
    assessment_id: UUID,
    payload: AILimitedProductionOutcomeReviewWrite,
    manager: Manager,
    db: Annotated[Session, Depends(get_db)],
):
...
def assessment_decision(
    assessment_id: UUID,
    payload: AILimitedProductionOutcomeDecision,
    admin: Admin,
    db: Annotated[Session, Depends(get_db)],
):
