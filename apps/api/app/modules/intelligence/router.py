...
from app.modules.ai_runtime import (
    require_external_ai_runtime_authorization,
    reserve_production_ai_run,
)
...
from app.modules.intelligence.schemas import (
    ...
    EngineLogEventCandidateResponse,
...
def _claim_document_and_text(
    *, claim_id: UUID, document_id: UUID, current_user: CurrentUser, db: Session,
) -> tuple[Document, DocumentTextExtraction]:
    ...
    document = get_document_for_tenant(
        db, document_id=document_id, claim_id=claim.id,
...
def _provider_for_document(document: Document):
    try:
...
def _enqueue_primary_intelligence(
    *, claim_id: UUID, document_id: UUID, current_user: CurrentUser, db: Session,
    expected_document_type: str, job_type: ProcessingJobType,
) -> dict[str, str]:
    document, text_extraction = _claim_document_and_text(
        claim_id=claim_id, document_id=document_id, current_user=current_user, db=db)
...
def intelligence_summary(
    claim_id: UUID, document_id: UUID, current_user: CurrentUser,
...
def enqueue_ce_report_intelligence(
    ...
    return _enqueue_primary_intelligence(
        claim_id=claim_id, document_id=document_id, current_user=current_user, db=db,
...
def enqueue_running_hours_intelligence(
    claim_id: UUID, document_id: UUID, current_user: CurrentUser,
    db: Annotated[Session, Depends(get_db)],
) -> dict[str, str]:
    return _enqueue_specialized_intelligence(
        claim_id=claim_id, document_id=document_id, current_user=current_user,
        db=db, job_type=ProcessingJobType.AI_EXTRACT_RUNNING_HOURS)


@router.post("/pms-history", status_code=status.HTTP_202_ACCEPTED)
def enqueue_pms_history_intelligence(
    claim_id: UUID, document_id: UUID, current_user: CurrentUser,
    db: Annotated[Session, Depends(get_db)],
) -> dict[str, str]:
    return _enqueue_specialized_intelligence(
        claim_id=claim_id, document_id=document_id, current_user=current_user,
        db=db, job_type=ProcessingJobType.AI_EXTRACT_PMS_HISTORY)


@router.post("/workshop-report", status_code=status.HTTP_202_ACCEPTED)
def enqueue_workshop_report_intelligence(
    claim_id: UUID, document_id: UUID, current_user: CurrentUser,
    db: Annotated[Session, Depends(get_db)],
) -> dict[str, str]:
    return _enqueue_specialized_intelligence(
        claim_id=claim_id, document_id=document_id, current_user=current_user,
        db=db, job_type=ProcessingJobType.AI_EXTRACT_WORKSHOP_REPORT)


@router.post("/quotation", status_code=status.HTTP_202_ACCEPTED)
def enqueue_quotation_intelligence(
    claim_id: UUID, document_id: UUID, current_user: CurrentUser,
    db: Annotated[Session, Depends(get_db)],
) -> dict[str, str]:
    return _enqueue_specialized_intelligence(
        claim_id=claim_id, document_id=document_id, current_user=current_user,
        db=db, job_type=ProcessingJobType.AI_EXTRACT_QUOTATION)


@router.post("/invoice", status_code=status.HTTP_202_ACCEPTED)
def enqueue_invoice_intelligence(
    claim_id: UUID, document_id: UUID, current_user: CurrentUser,
    db: Annotated[Session, Depends(get_db)],
) -> dict[str, str]:
    return _enqueue_specialized_intelligence(
        claim_id=claim_id, document_id=document_id, current_user=current_user,
        db=db, job_type=ProcessingJobType.AI_EXTRACT_INVOICE)
