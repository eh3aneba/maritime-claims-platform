...
from app.modules.ai_high_coverage.schemas import AIHighCoverageCreate, AIHighCoverageDocumentCreate
...
def _reference(value: str) -> str:
    ...
    if not REFERENCE.fullmatch(reference):
        raise HTTPException(422, "High-coverage evidence must use a bounded allowlisted reference")
    return reference
...
def _approvals(db: Session, authorization_id: UUID) -> list[AIHighCoverageApproval]:
    return list(db.scalars(select(AIHighCoverageApproval).where(
        AIHighCoverageApproval.authorization_id == authorization_id,
    ).order_by(AIHighCoverageApproval.approval_role.asc())))


def _documents(db: Session, authorization_id: UUID) -> list[AIHighCoverageDocumentEligibility]:
    return list(db.scalars(select(AIHighCoverageDocumentEligibility).where(
        AIHighCoverageDocumentEligibility.authorization_id == authorization_id,
    ).order_by(AIHighCoverageDocumentEligibility.created_at.asc())))
...
def latest_high_coverage_attempt(db: Session, organization_id: UUID) -> AIHighCoverageAuthorization | None:
    return db.scalar(select(AIHighCoverageAuthorization).where(
        AIHighCoverageAuthorization.organization_id == organization_id,
    ).order_by(AIHighCoverageAuthorization.created_at.desc(), AIHighCoverageAuthorization.id.desc()))


def _active(item: AIHighCoverageAuthorization) -> bool:
...
def _controls(item: AIHighCoverageAuthorization) -> dict:
    return {
...
def _latest_monitor_pass(db: Session, item: AIHighCoverageAuthorization, *, require_fresh: bool) -> bool:
...
def _recovery_complete(db: Session, item: AIHighCoverageAuthorization) -> bool:
    monitors = _monitors(db, item.id)
    for incident in _incidents(db, item.id):
        ...
        if not any(
            monitor.status == "pass"
            and _as_utc(monitor.monitored_at) >= _as_utc(incident.resolved_at)
...
def authorization_response(db: Session, item: AIHighCoverageAuthorization) -> dict:
    ...
    return {
        ...
        "summary": {
            ...
            "high_coverage_cohort_authorized": _active(item),
            "rollout_percentage": item.rollout_percentage,
...
def list_authorizations(db: Session, organization_id: UUID) -> list[dict]:
    items = list(db.scalars(select(AIHighCoverageAuthorization).where(
        AIHighCoverageAuthorization.organization_id == organization_id,
    ).order_by(AIHighCoverageAuthorization.created_at.desc()).limit(20)))
    return [authorization_response(db, item) for item in items]


def get_authorization(db: Session, organization_id: UUID, authorization_id: UUID) -> AIHighCoverageAuthorization:
    item = db.scalar(select(AIHighCoverageAuthorization).where(
        AIHighCoverageAuthorization.id == authorization_id,
        AIHighCoverageAuthorization.organization_id == organization_id,
    ))
    if item is None:
        raise HTTPException(404, "High-coverage authorization not found")
    return item


def _outcome(db: Session, organization_id: UUID, assessment_id: UUID) -> AIBroaderProductionOutcomeAssessment:
    ...
    if item is None:
        raise HTTPException(404, "Sprint 11J outcome assessment not found")
    return item


def _broader(db: Session, organization_id: UUID, authorization_id: UUID) -> AIBroaderProductionAuthorization:
    ...
    if item is None:
        raise HTTPException(409, "The Sprint 11I anchor is missing")
    return item


def _anchor_still_valid(db: Session, item: AIHighCoverageAuthorization) -> None:
    outcome = _outcome(db, item.organization_id, item.outcome_assessment_id)
    broader = _broader(db, item.organization_id, item.broader_production_authorization_id)
    if (
        outcome.status != "recommended"
        or outcome.outcome != "recommend_next_broader_stage"
        ...
        or broader.status != "completed"
        or broader.decision_hash != item.broader_production_decision_hash
        or broader.readiness_assessment_hash != item.readiness_assessment_hash
        or broader.readiness_decision_hash != item.readiness_decision_hash
        or broader.scale_up_decision_hash != item.scale_up_decision_hash
        or broader.inherited_outcome_assessment_hash != item.inherited_outcome_assessment_hash
        or broader.inherited_outcome_decision_hash != item.inherited_outcome_decision_hash
        or broader.model != item.model
        or broader.prompt_bundle_version != item.prompt_bundle_version
        or broader.schema_bundle_version != item.schema_bundle_version
        or broader.rollout_percentage != item.previous_rollout_percentage
    ):
        raise HTTPException(409, "The persisted Sprint 11J/11I evidence anchor no longer matches")


def create_authorization(db: Session, user: User, payload: AIHighCoverageCreate) -> dict:
    if not payload.confirm_separate_high_coverage:
        raise HTTPException(422, "Explicit separate high-coverage confirmation is required")
    ...
    if starts < now - timedelta(minutes=5) or starts > now + timedelta(days=2):
        raise HTTPException(422, "High-coverage start must be current or within two days")
    if expires <= starts or expires - starts > timedelta(days=30):
        raise HTTPException(422, "High-coverage authorization must expire within 30 days")
    ...
    if (
        ...
        raise HTTPException(409, "A passing, positively recommended Sprint 11J assessment is required")
    broader = _broader(db, user.organization_id, outcome.broader_production_authorization_id)
    if (
        broader.status != "completed"
        or not broader.decision_hash
        or outcome.broader_production_decision_hash != broader.decision_hash
        or outcome.readiness_assessment_hash != broader.readiness_assessment_hash
        or outcome.readiness_decision_hash != broader.readiness_decision_hash
        or outcome.scale_up_decision_hash != broader.scale_up_decision_hash
        or outcome.inherited_outcome_assessment_hash != broader.inherited_outcome_assessment_hash
        or outcome.inherited_outcome_decision_hash != broader.inherited_outcome_decision_hash
        or outcome.model != broader.model
        or outcome.prompt_bundle_version != broader.prompt_bundle_version
        or outcome.schema_bundle_version != broader.schema_bundle_version
        or outcome.rollout_percentage != broader.rollout_percentage
    ):
        raise HTTPException(409, "The completed Sprint 11I anchor is invalid")
    if not 26 <= broader.rollout_percentage <= 50:
        raise HTTPException(409, "Sprint 11I rollout is outside its authorized envelope")

    prior = list(db.scalars(select(AIHighCoverageAuthorization).where(
        AIHighCoverageAuthorization.outcome_assessment_id == outcome.id,
    )))
    item = AIHighCoverageAuthorization(
        organization_id=user.organization_id,
        outcome_assessment_id=outcome.id,
        broader_production_authorization_id=broader.id,
        ...
        model=broader.model,
        prompt_bundle_version=broader.prompt_bundle_version,
        schema_bundle_version=broader.schema_bundle_version,
        max_input_chars=broader.max_input_chars,
        max_output_tokens=broader.max_output_tokens,
        allowed_document_types=allowed,
        previous_rollout_percentage=broader.rollout_percentage,
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(409, "High-coverage authorization key or attempt already exists")
    _audit(db, user, "CREATE_AI_HIGH_COVERAGE_AUTHORIZATION", "ai_high_coverage_authorization", item.id,
           {"outcome_assessment_id": str(outcome.id), "previous_rollout_percentage": broader.rollout_percentage,
            "rollout_percentage": payload.rollout_percentage, "production_wide_authorized": False},
           "Created an expiring Sprint 11K 51–75% authorization attempt; no rollout is authorized yet.")
    db.commit()
    db.refresh(item)
    return authorization_response(db, item)


def record_approval(db: Session, user: User, item: AIHighCoverageAuthorization,
    ...
    if role not in APPROVAL_ROLES:
        raise HTTPException(422, "Unsupported high-coverage approval role")
    if user.id == item.requested_by_id:
        raise HTTPException(409, "The requester cannot approve the high-coverage attempt")
    ...
    if any(entry.approver_id == user.id for entry in existing):
        raise HTTPException(409, "Each high-coverage approval role requires a distinct human")
    reference = _reference(evidence_reference) if evidence_reference else None
    approval = AIHighCoverageApproval(
    ...
    db.add(approval)
    db.flush()
    _audit(db, user, f"{action.upper()}_AI_HIGH_COVERAGE_APPROVAL", "ai_high_coverage_authorization",
           item.id, {"approval_role": role, "action": action},
           f"Independent Sprint 11K {role} review recorded.")
    approvals = _approvals(db, item.id)
    if action == "reject":
        item.status = "held"
        item.outcome = "approval_rejected"
    elif (len(approvals) == len(APPROVAL_ROLES)
...
def decide_authorization(db: Session, user: User, item: AIHighCoverageAuthorization,
                         outcome: str, confirm: bool, note: str) -> dict:
    if not confirm:
        raise HTTPException(422, "Explicit Admin high-coverage decision confirmation is required")
    if item.status != "decision_ready":
        raise HTTPException(409, "Seven independent approvals are required before Admin decision")
    approvals = _approvals(db, item.id)
    ...
    if user.id in {entry.approver_id for entry in approvals}:
        raise HTTPException(409, "Final Admin authorization must be separate from the seven independent reviewers")
    _anchor_still_valid(db, item)
    snapshot = {
        "schema": "mcri-ai-high-coverage-authorization-v1",
        "authorization_id": str(item.id), "outcome_assessment_id": str(item.outcome_assessment_id),
        "outcome_assessment_hash": item.outcome_assessment_hash, "outcome_decision_hash": item.outcome_decision_hash,
        "broader_production_decision_hash": item.broader_production_decision_hash,
        ...
        "outcome": outcome, "production_wide_authorized": False, "rollout_above_75_authorized": False,
    }
    item.finalized_by_id = user.id
    item.decision_note = note.strip()
    item.decision_hash = _hash(snapshot)
    item.decided_at = datetime.now(UTC)
    if outcome == "authorize_high_coverage_cohort":
    ...
    _audit(db, user, "DECIDE_AI_HIGH_COVERAGE_AUTHORIZATION", "ai_high_coverage_authorization", item.id,
           ...
           "Admin recorded the immutable Sprint 11K decision without changing the pinned bundle.")
    db.commit()
    db.refresh(item)
    return authorization_response(db, item)


def attest_document(db: Session, user: User, item: AIHighCoverageAuthorization,
                    payload: AIHighCoverageDocumentCreate) -> dict:
    if not payload.confirm_new_high_coverage_eligibility:
        raise HTTPException(422, "Explicit fresh Sprint 11K eligibility confirmation is required")
    if not _active(item):
        raise HTTPException(409, "Only an active Sprint 11K authorization may attest documents")
    _anchor_still_valid(db, item)
    document = db.scalar(select(Document).where(Document.id == payload.document_id,
        Document.organization_id == user.organization_id, Document.claim_id == payload.claim_id))
    if document is None:
    ...
    if document.document_type not in item.allowed_document_types:
        raise HTTPException(409, "Document type is outside the Sprint 11K allowlist")
    bucket = _rollout_bucket(document.id)
    if bucket >= item.rollout_percentage:
        raise HTTPException(409, "Document is outside the deterministic high-coverage rollout")
    ...
    if any(entry.document_id == document.id for entry in existing):
        raise HTTPException(409, "Document already has active Sprint 11K eligibility")
    claims = {entry.claim_id for entry in existing}
    if document.claim_id not in claims and len(claims) >= item.max_claims:
        raise HTTPException(409, "High-coverage claim cap reached")
    if len(existing) >= item.max_documents:
        raise HTTPException(409, "High-coverage document cap reached")
    attempts = [entry for entry in _documents(db, item.id) if entry.document_id == document.id]
    now = datetime.now(UTC)
    snapshot = {"schema": "mcri-ai-high-coverage-document-eligibility-v1", "authorization_id": str(item.id),
                "authorization_decision_hash": item.decision_hash, "document_id": str(document.id),
                "claim_id": str(document.claim_id), "document_type": document.document_type,
                "confidentiality_level": confidentiality, "file_hash": document.file_hash,
                "rollout_bucket": bucket, "legal_basis_reference": _reference(payload.legal_basis_reference),
    ...
    entry = AIHighCoverageDocumentEligibility(
        ...
        rollout_bucket=bucket, document_type=document.document_type, confidentiality_level=confidentiality,
    ...
    db.add(entry)
    db.flush()
    _audit(db, user, "ATTEST_AI_HIGH_COVERAGE_DOCUMENT", "ai_high_coverage_document_eligibility", entry.id,
           {"authorization_id": str(item.id), "document_id": str(document.id), "snapshot_hash": entry.snapshot_hash,
            "fresh_eligibility": True},
           "Fresh Sprint 11K legal-basis and minimization attestation; prior eligibility was not carried forward.")
    db.commit()
    db.refresh(item)
    return authorization_response(db, item)


def revoke_document(db: Session, user: User, item: AIHighCoverageAuthorization,
    ...
    entry = db.scalar(select(AIHighCoverageDocumentEligibility).where(
        AIHighCoverageDocumentEligibility.id == eligibility_id,
        AIHighCoverageDocumentEligibility.authorization_id == item.id,
        AIHighCoverageDocumentEligibility.organization_id == user.organization_id))
    if entry is None:
        raise HTTPException(404, "Sprint 11K document eligibility not found")
    ...
    _audit(db, user, "REVOKE_AI_HIGH_COVERAGE_DOCUMENT", "ai_high_coverage_document_eligibility", entry.id,
           {"status": "revoked"}, "Sprint 11K document eligibility revoked.")
    db.commit()
    db.refresh(item)
    return authorization_response(db, item)


def require_high_coverage_runtime_authorization(db: Session, *, organization_id: UUID, document: Document,
    expected_document_type: str, input_char_count: int, requested_by_id: UUID | None = None
) -> tuple[AIHighCoverageAuthorization, AIHighCoverageDocumentEligibility]:
    item = latest_high_coverage_attempt(db, organization_id)
    if item is None:
        raise HTTPException(409, "No Sprint 11K control plane exists")
    if not _active(item):
        raise HTTPException(409, "No active high-coverage authorization exists")
    _anchor_still_valid(db, item)
    ...
    if (settings.ai_model != item.model or settings.ai_prompt_bundle_version != item.prompt_bundle_version
        ...
        raise HTTPException(409, "Configured AI bundle differs from the authorized Sprint 11K bundle")
    confidentiality = document.confidentiality_level.value if hasattr(document.confidentiality_level, "value") else str(document.confidentiality_level)
    if confidentiality not in {ConfidentialityLevel.INTERNAL.value, ConfidentialityLevel.CONFIDENTIAL.value}:
    ...
    if expected_document_type not in item.allowed_document_types or document.document_type != expected_document_type:
        raise HTTPException(409, "Document type is outside the high-coverage allowlist")
    ...
    if _rollout_bucket(document.id) >= item.rollout_percentage:
        raise HTTPException(409, "Document is outside the deterministic high-coverage rollout")
    eligibility = db.scalar(select(AIHighCoverageDocumentEligibility).where(
        AIHighCoverageDocumentEligibility.organization_id == organization_id,
        AIHighCoverageDocumentEligibility.authorization_id == item.id,
        AIHighCoverageDocumentEligibility.document_id == document.id,
        AIHighCoverageDocumentEligibility.status == "eligible").order_by(
            AIHighCoverageDocumentEligibility.attestation_number.desc()))
    if eligibility is None:
        raise HTTPException(409, "Document requires fresh Sprint 11K eligibility")
    incidents = _incidents(db, item.id)
    if any(entry.status == "open" for entry in incidents):
        raise HTTPException(409, "An open incident blocks high-coverage AI")
    if any(entry.category in SAFETY_CATEGORIES for entry in incidents):
        raise HTTPException(409, "Privacy, Security or Cross-tenant incident history blocks this Sprint 11K attempt")
    runs = _runs(db, item.id)
    if len(runs) >= item.max_provider_runs:
        raise HTTPException(409, "High-coverage provider-run cap reached")
    ...
    if requested_by_id is not None and requested_by_id not in participating and len(participating) >= item.max_users:
        raise HTTPException(409, "High-coverage user cap reached")
    if (runs and item.decided_at is not None
        ...
        raise HTTPException(409, "A fresh passing Sprint 11K monitor is required")
    return item, eligibility


def reserve_run_if_high_coverage(db: Session, *, user: User, document: Document, expected_document_type: str,
    input_char_count: int, processing_job_id: UUID) -> AIHighCoverageRun | None:
    if get_settings().app_env.lower().strip() != "production":
        return None
    latest = latest_high_coverage_attempt(db, user.organization_id)
    if latest is None:
        return None
    existing = db.scalar(select(AIHighCoverageRun).where(
        AIHighCoverageRun.organization_id == user.organization_id,
        AIHighCoverageRun.processing_job_id == processing_job_id))
    if existing is not None:
        return existing
    item, eligibility = require_high_coverage_runtime_authorization(
    ...
    db.add(run)
    db.flush()
    _audit(db, user, "RESERVE_AI_HIGH_COVERAGE_RUN", "ai_high_coverage_run", run.id,
           ...
           "Content-free Sprint 11K provider-run reservation.")
    return run


def get_run(db: Session, organization_id: UUID, run_id: UUID) -> AIHighCoverageRun:
    run = db.scalar(select(AIHighCoverageRun).where(
        AIHighCoverageRun.id == run_id, AIHighCoverageRun.organization_id == organization_id))
    if run is None:
        raise HTTPException(404, "High-coverage run not found")
    return run


def record_run_outcome(db: Session, user: User, run: AIHighCoverageRun, *, human_review_action: str,
    ...
    if run.requested_by_id == user.id:
        raise HTTPException(409, "A different human must review every high-coverage AI output")
    ...
    snapshot = {"schema": "mcri-ai-high-coverage-run-outcome-v1", "run_id": str(run.id),
    ...
    _audit(db, user, "REVIEW_AI_HIGH_COVERAGE_RUN", "ai_high_coverage_run", run.id,
           ...
           "Mandatory different-human Sprint 11K review.")
    db.commit()
...
def _segment_metrics(rows: list[AIHighCoverageRun]) -> dict:
    ...
    return {"reject_rate_bps": _rate_bps(sum(row.human_review_action == "reject" for row in actions), len(actions)),
...
def record_monitor(db: Session, user: User, item: AIHighCoverageAuthorization,
    ...
    if item.status not in {"authorized", "paused"}:
        raise HTTPException(409, "Only an authorized or paused Sprint 11K cohort can be monitored")
    _anchor_still_valid(db, item)
    runs = _runs(db, item.id)
    incidents = _incidents(db, item.id)
    ...
    metrics = {"provider_run_count": len(runs), "human_reviewed_run_count": len(reviewed),
               "human_review_rate_bps": review_rate, "human_reject_rate_bps": reject_rate,
               "human_edit_rate_bps": edit_rate, "unsupported_output_rate_bps": unsupported_rate,
    ...
    now = datetime.now(UTC)
    snapshot = {"schema": "mcri-ai-high-coverage-monitor-v1", "authorization_id": str(item.id),
    ...
    monitor = AIHighCoverageMonitor(organization_id=user.organization_id, authorization_id=item.id,
        initiated_by_id=user.id, monitor_key=monitor_key, metrics=metrics, failure_reasons=failures,
    ...
    if failures and item.status == "authorized":
        ...
        db.add(AIHighCoverageIncident(organization_id=user.organization_id, authorization_id=item.id,
            reported_by_id=user.id, severity="high", category="quality",
            evidence_reference=f"monitor://ai-high-coverage/{monitor.id}",
            note="Automatic rollback incident created by a failing Sprint 11K monitor.",
            status="open", reported_at=now))
        db.flush()
    _audit(db, user, "RECORD_AI_HIGH_COVERAGE_MONITOR", "ai_high_coverage_authorization", item.id,
           {"monitor_id": str(monitor.id), "monitor_status": monitor.status,
            "failure_reasons": failures, "authorization_status": item.status},
           "Sprint 11K live monitor recorded; failures pause and roll back execution.")
    db.commit()
    db.refresh(item)
    return authorization_response(db, item)


def report_incident(db: Session, user: User, item: AIHighCoverageAuthorization, *, severity: str,
                    category: str, evidence_reference: str, note: str, confirm: bool) -> dict:
    if not confirm: raise HTTPException(422, "Explicit pause-and-rollback confirmation is required")
    if item.status != "authorized": raise HTTPException(409, "Only an active Sprint 11K authorization can be paused")
    incident = AIHighCoverageIncident(organization_id=user.organization_id, authorization_id=item.id,
        reported_by_id=user.id, severity=severity, category=category,
        evidence_reference=_reference(evidence_reference), note=note.strip(), status="open",
        reported_at=datetime.now(UTC))
    db.add(incident); db.flush(); item.status = "paused"; item.outcome = "incident_rollback"
    _audit(db, user, "PAUSE_AI_HIGH_COVERAGE_INCIDENT", "ai_high_coverage_authorization", item.id,
           ...
           "Immediate Sprint 11K pause and rollback trigger.")
    db.commit(); db.refresh(item); return authorization_response(db, item)


def resolve_incident(db: Session, user: User, item: AIHighCoverageAuthorization, incident_id: UUID, *,
    ...
    incident = db.scalar(select(AIHighCoverageIncident).where(AIHighCoverageIncident.id == incident_id,
        AIHighCoverageIncident.authorization_id == item.id,
        AIHighCoverageIncident.organization_id == user.organization_id))
    if incident is None: raise HTTPException(404, "Sprint 11K incident not found")
    ...
    _audit(db, user, "RESOLVE_AI_HIGH_COVERAGE_INCIDENT", "ai_high_coverage_authorization", item.id,
...
def resume_authorization(db: Session, user: User, item: AIHighCoverageAuthorization,
                         confirm: bool, note: str) -> dict:
    if not confirm: raise HTTPException(422, "Explicit Admin recovery confirmation is required")
    if item.status != "paused": raise HTTPException(409, "Only a paused Sprint 11K authorization can resume")
    incidents = _incidents(db, item.id)
    if any(entry.status == "open" for entry in incidents): raise HTTPException(409, "Open incidents block recovery")
    if any(entry.category in SAFETY_CATEGORIES for entry in incidents):
        raise HTTPException(409, "Privacy, Security or Cross-tenant incidents require a new Sprint 11K attempt")
    if not _latest_monitor_pass(db, item, require_fresh=True): raise HTTPException(409, "A fresh passing monitor is required before recovery")
    if _as_utc(item.expires_at) <= datetime.now(UTC): raise HTTPException(409, "Expired authorization cannot resume")
    _anchor_still_valid(db, item); item.status = "authorized"; item.outcome = "resumed_after_monitor"
    _audit(db, user, "RESUME_AI_HIGH_COVERAGE", "ai_high_coverage_authorization", item.id,
           {"status": "authorized", "production_wide_authorized": False},
           "Admin resumed only the existing bounded Sprint 11K cohort.")
    db.commit(); db.refresh(item); return authorization_response(db, item)


def revoke_authorization(db: Session, user: User, item: AIHighCoverageAuthorization,
                         confirm: bool, note: str) -> dict:
    if not confirm: raise HTTPException(422, "Explicit Sprint 11K kill-switch confirmation is required")
    ...
    _audit(db, user, "REVOKE_AI_HIGH_COVERAGE", "ai_high_coverage_authorization", item.id,
           {"status": "revoked", "production_wide_authorized": False}, "Immediate Sprint 11K kill switch.")
    db.commit(); db.refresh(item); return authorization_response(db, item)


def complete_authorization(db: Session, user: User, item: AIHighCoverageAuthorization,
                           confirm: bool, note: str) -> dict:
    if not confirm: raise HTTPException(422, "Explicit Sprint 11K completion confirmation is required")
    if item.status != "authorized": raise HTTPException(409, "Only an active Sprint 11K cohort can complete")
    _anchor_still_valid(db, item)
    runs = _runs(db, item.id); incidents = _incidents(db, item.id); monitors = _monitors(db, item.id)
    if not runs or any(run.status != "human_reviewed" for run in runs):
        raise HTTPException(409, "Every Sprint 11K provider run requires completed different-human review")
    ...
    now = datetime.now(UTC)
    snapshot = {"schema": "mcri-ai-high-coverage-completion-v1", "authorization_id": str(item.id),
                "authorization_decision_hash": item.decision_hash, "outcome_assessment_hash": item.outcome_assessment_hash,
                "outcome_decision_hash": item.outcome_decision_hash, "rollout_percentage": item.rollout_percentage,
                "run_hashes": [run.outcome_hash for run in runs], "monitor_hashes": [monitor.monitor_hash for monitor in monitors],
                "incidents": [{"id": str(incident.id), "category": incident.category, "severity": incident.severity,
                               "status": incident.status, "resolution_reference": incident.resolution_reference}
                              for incident in incidents],
                "completed_at": now.isoformat(), "production_wide_authorized": False,
                "rollout_above_75_authorized": False}
    item.status = "completed"; item.outcome = "completed"; item.completed_at = now
    item.completion_note = note.strip(); item.completion_hash = _hash(snapshot)
    _audit(db, user, "COMPLETE_AI_HIGH_COVERAGE", "ai_high_coverage_authorization", item.id,
           {"status": "completed", "provider_run_count": len(runs), "completion_hash": item.completion_hash,
            "production_wide_authorized": False, "rollout_above_75_authorized": False,
            "restricted_documents_authorized": False, "new_document_classes_authorized": False},
           "Completed bounded Sprint 11K cohort; no further rollout is granted.")
    db.commit(); db.refresh(item); return authorization_response(db, item)
