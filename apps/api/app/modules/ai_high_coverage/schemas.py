...
class AIHighCoverageCreate(BaseModel):
    ...
    rollout_percentage: int = Field(ge=51, le=75)
    max_claims: int = Field(ge=1, le=75)
    max_documents: int = Field(ge=1, le=225)
    max_users: int = Field(ge=1, le=75)
    max_provider_runs: int = Field(ge=1, le=1000)
    starts_at: datetime
    expires_at: datetime
    ...
    confirm_separate_high_coverage: bool


class AIHighCoverageApprovalWrite(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIHighCoverageDecision(BaseModel):
    model_config = ConfigDict(extra="forbid")
    outcome: Literal["authorize_high_coverage_cohort", "hold_for_remediation", "reject_progression"]
    confirm_decision: bool
    note: str = Field(min_length=10, max_length=4000)


class AIHighCoverageDocumentCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
    claim_id: UUID
    document_id: UUID
    ...
    change_ticket_reference: str = Field(min_length=8, max_length=500)
    note: str = Field(min_length=10, max_length=4000)
    confirm_new_high_coverage_eligibility: bool


class AIHighCoverageRunOutcome(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIHighCoverageMonitorCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIHighCoverageIncidentCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIHighCoverageIncidentResolve(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AIHighCoverageLifecycle(BaseModel):
    model_config = ConfigDict(extra="forbid")
    confirm: bool
    note: str = Field(min_length=10, max_length=4000)
...
class AIHighCoverageApprovalResponse(_ORMResponse):
    id: UUID
...
class AIHighCoverageDocumentResponse(_ORMResponse):
    id: UUID
    authorization_id: UUID
    claim_id: UUID
    document_id: UUID
...
class AIHighCoverageRunResponse(_ORMResponse):
    id: UUID
    authorization_id: UUID
    eligibility_id: UUID
    claim_id: UUID
...
class AIHighCoverageMonitorResponse(_ORMResponse):
    id: UUID
    authorization_id: UUID
...
class AIHighCoverageIncidentResponse(_ORMResponse):
    id: UUID
    authorization_id: UUID
...
class AIHighCoverageResponse(BaseModel):
    id: UUID
    outcome_assessment_id: UUID
    ...
    outcome_assessment_hash: str
    outcome_decision_hash: str
    ...
    revoked_at: datetime | None
    revocation_note: str | None
    approvals: list[AIHighCoverageApprovalResponse]
    document_eligibility: list[AIHighCoverageDocumentResponse]
    runs: list[AIHighCoverageRunResponse]
    monitors: list[AIHighCoverageMonitorResponse]
    incidents: list[AIHighCoverageIncidentResponse]
    summary: dict
    created_at: datetime


class AIHighCoverageDashboard(BaseModel):
    authorizations: list[AIHighCoverageResponse]
