...
PREDECESSOR_CHECKPOINT_GENERATION = 2
SUCCESSOR_CHECKPOINT_GENERATION = 3
CANDIDATE_GENERATION = 3
MAX_CHECKPOINT_GENERATION_3_CONTENT_BYTES = 8 * 1024 * 1024


class _CheckpointGeneration3SafetyMixin:
    ...
    upstream_successor_versioned_restaging_completed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default=true())
    ...
    checkpoint_generation_3_advance_completed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=false())
...
class ExternalDocumentSourceCheckpointGeneration3Execution(UUIDPrimaryKeyMixin, TimestampMixin, _CheckpointGeneration3SafetyMixin, Base):
    __tablename__ = "external_doc_source_checkpoint_gen3_execs"
    __table_args__ = (
        UniqueConstraint("successor_versioned_restaging_execution_id", name="uq_ext_doc_cg3_exec_restage"),
        UniqueConstraint("predecessor_checkpoint_generation_execution_id", name="uq_ext_doc_cg3_exec_predecessor"),
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_cg3_exec_request"),
        CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_cg3_exec_provider"),
        CheckConstraint(f"predecessor_checkpoint_generation = {PREDECESSOR_CHECKPOINT_GENERATION}", name="ck_ext_doc_cg3_exec_pred_gen"),
        CheckConstraint(f"candidate_generation = {CANDIDATE_GENERATION}", name="ck_ext_doc_cg3_exec_candidate_gen"),
        CheckConstraint(f"successor_checkpoint_kind = '{SUCCESSOR_CHECKPOINT_KIND}'", name="ck_ext_doc_cg3_exec_kind"),
        CheckConstraint(f"successor_checkpoint_generation = {SUCCESSOR_CHECKPOINT_GENERATION}", name="ck_ext_doc_cg3_exec_generation"),
        CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_cg3_exec_status"),
        CheckConstraint("result_status IS NULL OR result_status = 'checkpoint_generation_3_advanced'", name="ck_ext_doc_cg3_exec_result"),
        CheckConstraint(f"content_byte_count >= 0 AND content_byte_count <= {MAX_CHECKPOINT_GENERATION_3_CONTENT_BYTES}", name="ck_ext_doc_cg3_exec_size"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_cg3_exec_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_cg3_exec_restage", "successor_versioned_restaging_execution_id"),
        Index("ix_ext_doc_cg3_exec_pred", "predecessor_checkpoint_generation_execution_id"),
        Index("ix_ext_doc_cg3_exec_org_status", "organization_id", "status"),
    ...
    predecessor_checkpoint_generation_execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_checkpoint_generation_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
    successor_change_detection_execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_successor_change_detect_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    successor_change_scope_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    successor_change_request_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ExternalDocumentSourceCheckpointGeneration3Receipt(UUIDPrimaryKeyMixin, TimestampMixin, _CheckpointGeneration3SafetyMixin, Base):
    __tablename__ = "external_doc_source_checkpoint_gen3_receipts"
    __table_args__ = (
        UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_cg3_rcpt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_cg3_rcpt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_cg3_rcpt_seq"),
        CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_cg3_rcpt_event"),
        CheckConstraint(
        ...
        CheckConstraint("(sequence_number = 1 AND prior_receipt_hash IS NULL) OR (sequence_number > 1 AND prior_receipt_hash IS NOT NULL)", name="ck_ext_doc_cg3_rcpt_chain"),
        Index("ix_ext_doc_cg3_rcpt_seq", "execution_id", "sequence_number"),
        *_safety_constraints("ext_doc_cg3_rcpt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_checkpoint_gen3_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
