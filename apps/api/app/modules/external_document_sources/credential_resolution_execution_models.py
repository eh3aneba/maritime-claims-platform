...
class _CredentialResolutionExecutionSafetyMixin:
...
class ExternalDocumentSourceCredentialResolutionExecution(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _CredentialResolutionExecutionSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_credential_resolution_execs"
    __table_args__ = (
        UniqueConstraint("activation_execution_id", name="uq_ext_doc_cr_exec_activation"),
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_cr_exec_request"),
        CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_cr_exec_provider"),
        CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_cr_exec_status"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_cr_exec_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_cr_exec_org_status", "organization_id", "status"),
    ...
    health_resolver_kind: Mapped[str] = mapped_column(String(128), nullable=False)
    ...
    activation_execution_scope_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    activation_execution_request_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    activation_execution_completion_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    activation_authorization_terminal_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ExternalDocumentSourceCredentialResolutionExecutionReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _CredentialResolutionExecutionSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_credential_resolution_receipts"
    __table_args__ = (
        UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_cr_rcpt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_cr_rcpt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_cr_rcpt_seq"),
        CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_cr_rcpt_event"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_cr_rcpt_seq", "execution_id", "sequence_number"),
        *_safety_constraints("ext_doc_cr_rcpt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_credential_resolution_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
