...
class _CredentialReferenceSafetyMixin:
...
def _reference_safety_constraints(prefix: str):
    fields = (
...
class ExternalDocumentSourceCredentialReferenceBinding(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _CredentialReferenceSafetyMixin,
    Base,
):
    __tablename__ = "external_document_source_credential_reference_bindings"
    __table_args__ = (
        UniqueConstraint("bootstrap_execution_id", name="uq_ext_doc_cred_ref_execution"),
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_cred_ref_request"),
        CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_cred_ref_provider"),
        CheckConstraint(
        ...
        CheckConstraint("status IN ('pending_second_approval','active','rejected','disabled')", name="ck_ext_doc_cred_ref_status"),
        CheckConstraint("requested_by_id <> approved_by_id OR approved_by_id IS NULL", name="ck_ext_doc_cred_ref_four_eyes"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_cred_ref_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_cred_ref_org_status", "organization_id", "status"),
        *_reference_safety_constraints("ext_doc_cred_ref"),
    )
    ...
    bootstrap_execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_document_source_connection_bootstrap_executions.id", ondelete="RESTRICT"), nullable=False, index=True)
    provider_kind: Mapped[str] = mapped_column(String(32), nullable=False)
    profile_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    bootstrap_scope_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    bootstrap_request_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    bootstrap_completion_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ExternalDocumentSourceCredentialReferenceReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _CredentialReferenceSafetyMixin,
    Base,
):
    __tablename__ = "external_document_source_credential_reference_receipts"
    __table_args__ = (
        UniqueConstraint("binding_id", "sequence_number", name="uq_ext_doc_cred_ref_receipt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_cred_ref_receipt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_cred_ref_receipt_seq"),
        CheckConstraint("event_type IN ('requested','approved','rejected','disabled')", name="ck_ext_doc_cred_ref_receipt_event"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_cred_ref_receipt_binding_seq", "binding_id", "sequence_number"),
        *_reference_safety_constraints("ext_doc_cred_ref_receipt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    binding_id: Mapped[UUID] = mapped_column(ForeignKey("external_document_source_credential_reference_bindings.id", ondelete="RESTRICT"), nullable=False, index=True)
