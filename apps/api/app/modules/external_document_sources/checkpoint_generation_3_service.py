...
from app.modules.external_document_sources.checkpoint_generation_3_models import (
    ...
    MAX_CHECKPOINT_GENERATION_3_CONTENT_BYTES,
...
from app.modules.external_document_sources.checkpoint_generation_models import (
    SUCCESSOR_CHECKPOINT_GENERATION as PHASE_R_SUCCESSOR_GENERATION,
...
def _base_safety(completed: bool) -> dict[str, bool]:
    return {
        ...
        "checkpoint_generation_3_advance_completed": completed,
        "sync_executed": False,
...
def _successor_state_hash(
    predecessor: ExternalDocumentSourceCheckpointGenerationExecution,
    ...
    if (
        ...
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 upstream completion facts are incomplete")
    return _canonical_hash(
        {
            ...
            "predecessor_checkpoint_generation_execution_id": str(predecessor.id),
            ...
            "successor_change_detection_execution_id": str(candidate.successor_change_detection_execution_id),
            "successor_change_scope_hash": candidate.successor_change_scope_hash,
            "successor_change_request_hash": candidate.successor_change_request_hash,
            "successor_change_completion_hash": candidate.successor_change_completion_hash,
            "observed_projection_hash": candidate.observed_projection_hash,
...
def _request_hash(execution: ExternalDocumentSourceCheckpointGeneration3Execution) -> str:
    return _canonical_hash(
        {
...
def _completion_hash(execution: ExternalDocumentSourceCheckpointGeneration3Execution) -> str:
    if execution.completed_at is None or execution.result_status != "checkpoint_generation_3_advanced":
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 completion facts are incomplete")
    return _canonical_hash(
        {
            "execution_id": str(execution.id),
            "scope_hash": execution.scope_hash,
...
def _receipt_hash(receipt: ExternalDocumentSourceCheckpointGeneration3Receipt) -> str:
    return _canonical_hash(
        {
...
def _receipts(db: Session, execution: ExternalDocumentSourceCheckpointGeneration3Execution):
    return list(
        db.scalars(
            select(ExternalDocumentSourceCheckpointGeneration3Receipt)
            .where(
                ExternalDocumentSourceCheckpointGeneration3Receipt.organization_id == execution.organization_id,
                ExternalDocumentSourceCheckpointGeneration3Receipt.execution_id == execution.id,
            )
            .order_by(ExternalDocumentSourceCheckpointGeneration3Receipt.sequence_number.asc())
        ).all()
    )
...
def _append_receipt(
    db: Session,
    *,
    execution: ExternalDocumentSourceCheckpointGeneration3Execution,
    event_type: str,
    actor_id: UUID,
    ...
    receipt = ExternalDocumentSourceCheckpointGeneration3Receipt(
...
def _get_candidate(
    ...
    *,
    ...
    if candidate is None:
        raise ExternalDocumentSourceNotFoundError("Phase T successor versioned-restaging execution not found")
...
def _get_predecessor(
    ...
    *,
    ...
    stmt = select(ExternalDocumentSourceCheckpointGenerationExecution).where(
        ExternalDocumentSourceCheckpointGenerationExecution.id == predecessor_id,
    ...
    if predecessor is None:
        raise ExternalDocumentSourceConflictError("Phase R generation-2 predecessor checkpoint is missing")
    _ensure_checkpoint_generation_integrity(db, predecessor)
    return predecessor


def _get_execution(db: Session, *, organization_id: UUID, profile_id: UUID, execution_id: UUID):
    execution = db.scalar(
        select(ExternalDocumentSourceCheckpointGeneration3Execution).where(
    ...
    if execution is None:
        raise ExternalDocumentSourceNotFoundError("Checkpoint-generation-3 execution not found")
    return execution


def _ensure_integrity(db: Session, execution: ExternalDocumentSourceCheckpointGeneration3Execution) -> None:
    ...
    if (
        ...
        raise ExternalDocumentSourceConflictError("Phase T candidate is not eligible for checkpoint-generation-3 advancement")

    predecessor = _get_predecessor(
        db,
        organization_id=execution.organization_id,
        profile_id=execution.profile_id,
        predecessor_id=execution.predecessor_checkpoint_generation_execution_id,
    )
    if candidate.checkpoint_generation_execution_id != predecessor.id:
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 predecessor is not the Phase T lineage checkpoint")

    if (
        execution.successor_change_detection_execution_id != candidate.successor_change_detection_execution_id
        or execution.versioned_restaging_execution_id != candidate.versioned_restaging_execution_id
        or execution.predecessor_sync_checkpoint_execution_id != candidate.predecessor_sync_checkpoint_execution_id
        or execution.change_detection_execution_id != candidate.change_detection_execution_id
        ...
        or predecessor.successor_checkpoint_generation != PREDECESSOR_CHECKPOINT_GENERATION
        or predecessor.successor_checkpoint_generation != PHASE_R_SUCCESSOR_GENERATION
        or execution.predecessor_checkpoint_state_hash != predecessor.successor_checkpoint_state_hash
        or execution.predecessor_checkpoint_completion_hash != predecessor.completion_hash
        or execution.successor_change_scope_hash != candidate.successor_change_scope_hash
        or execution.successor_change_request_hash != candidate.successor_change_request_hash
        or execution.successor_change_completion_hash != candidate.successor_change_completion_hash
        or execution.observed_projection_hash != candidate.observed_projection_hash
        ...
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 lineage drifted")
    ...
    for value, label in (
        ...
        (execution.successor_change_scope_hash, "successor change scope hash"),
        (execution.successor_change_request_hash, "successor change request hash"),
        (execution.successor_change_completion_hash, "successor change completion hash"),
        (execution.observed_projection_hash, "observed projection hash"),
        ...
        if not _HEX_64.fullmatch(value):
            raise ExternalDocumentSourceConflictError(f"Checkpoint-generation-3 {label} is invalid")
    if execution.version_token_hash is not None and not _HEX_64.fullmatch(execution.version_token_hash):
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 version-token hash is invalid")
    if not 0 <= execution.content_byte_count <= MAX_CHECKPOINT_GENERATION_3_CONTENT_BYTES:
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 content byte count is invalid")
    ...
    if (
        execution.successor_checkpoint_state_hash != expected_state
        or execution.scope_hash != expected_scope
        or execution.request_hash != _request_hash(execution)
    ):
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 request integrity failed")

    expected_events = {"requested": ["requested"], "completed": ["requested", "completed"]}
    if execution.status not in expected_events:
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 status is invalid")
    rows = _receipts(db, execution)
    if [row.event_type for row in rows] != expected_events[execution.status]:
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 receipt lifecycle drifted")
    ...
    for index, receipt in enumerate(rows, start=1):
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 receipt integrity failed")
        for field, expected in _base_safety(completed).items():
            if bool(getattr(receipt, field)) != expected:
                raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 receipt safety boundary drifted")
        prior = receipt.receipt_hash

    completed = execution.status == "completed"
    for field, expected in _base_safety(completed).items():
        if bool(getattr(execution, field)) != expected:
            raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 safety boundary drifted")
    if not completed:
        if execution.result_status is not None or execution.completed_at is not None or execution.completion_hash is not None:
            raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 request lifecycle drifted")
        return
    if (
        execution.result_status != "checkpoint_generation_3_advanced"
        ...
        raise ExternalDocumentSourceConflictError("Checkpoint-generation-3 completion integrity failed")


def execute_external_document_source_checkpoint_generation_3(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    existing = db.scalar(
        select(ExternalDocumentSourceCheckpointGeneration3Execution).where(
            ExternalDocumentSourceCheckpointGeneration3Execution.organization_id == organization_id,
            ExternalDocumentSourceCheckpointGeneration3Execution.profile_id == profile_id,
            ExternalDocumentSourceCheckpointGeneration3Execution.successor_versioned_restaging_execution_id == successor_versioned_restaging_execution_id,
        )
    )
    if existing is not None:
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Conflicting replay or second consumption for checkpoint-generation-3 advancement")
    ...
    collision = db.scalar(
        select(ExternalDocumentSourceCheckpointGeneration3Execution).where(
            ExternalDocumentSourceCheckpointGeneration3Execution.organization_id == organization_id,
            ExternalDocumentSourceCheckpointGeneration3Execution.profile_id == profile_id,
            ExternalDocumentSourceCheckpointGeneration3Execution.request_key == normalized_key,
        )
    )
    if collision is not None:
        raise ExternalDocumentSourceConflictError("Conflicting replay for checkpoint-generation-3 request_key")
    ...
    if (
        ...
        raise ExternalDocumentSourceConflictError("Only a completed integrity-valid Phase T generation-3 candidate is eligible")

    predecessor = _get_predecessor(
        db,
        organization_id=organization_id,
        profile_id=profile_id,
        predecessor_id=candidate.checkpoint_generation_execution_id,
        for_update=True,
    )
    if predecessor.successor_checkpoint_generation != PREDECESSOR_CHECKPOINT_GENERATION:
        raise ExternalDocumentSourceConflictError("Phase U requires the exact Phase R generation-2 predecessor")
    if (
        candidate.successor_checkpoint_state_hash != predecessor.successor_checkpoint_state_hash
        or candidate.successor_checkpoint_completion_hash != predecessor.completion_hash
    ):
        raise ExternalDocumentSourceConflictError("Phase T candidate does not bind to the exact Phase R predecessor")

    competing = db.scalar(
        select(ExternalDocumentSourceCheckpointGeneration3Execution).where(
            ExternalDocumentSourceCheckpointGeneration3Execution.organization_id == organization_id,
            ExternalDocumentSourceCheckpointGeneration3Execution.profile_id == profile_id,
            ExternalDocumentSourceCheckpointGeneration3Execution.predecessor_checkpoint_generation_execution_id == predecessor.id,
        )
    )
    if competing is not None:
        _ensure_integrity(db, competing)
        raise ExternalDocumentSourceConflictError("The Phase R generation-2 predecessor already has a generation-3 successor checkpoint")
    ...
    execution = ExternalDocumentSourceCheckpointGeneration3Execution(
        ...
        predecessor_checkpoint_generation_execution_id=predecessor.id,
        successor_change_detection_execution_id=candidate.successor_change_detection_execution_id,
        versioned_restaging_execution_id=candidate.versioned_restaging_execution_id,
        predecessor_sync_checkpoint_execution_id=candidate.predecessor_sync_checkpoint_execution_id,
...
def get_external_document_source_checkpoint_generation_3(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
...
def list_external_document_source_checkpoint_generation_3_receipts(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    execution = get_external_document_source_checkpoint_generation_3(
        db,
        organization_id=organization_id,
        profile_id=profile_id,
        execution_id=execution_id,
