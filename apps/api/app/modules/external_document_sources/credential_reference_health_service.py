...
class CredentialReferenceLocator:
    backend: str
    namespace: str
    name: str
    version: str | None
...
@dataclass(frozen=True)
class CredentialReferenceHealthProbeResult:
    resolvable: bool
    failure_code: str | None = None


class CredentialReferenceHealthResolver(Protocol):
    resolver_kind: str

    def check(self, locator: CredentialReferenceLocator) -> CredentialReferenceHealthProbeResult: ...


_RESOLVERS: dict[str, CredentialReferenceHealthResolver] = {}


def register_external_document_source_credential_reference_health_resolver(
    backend: str,
    resolver: CredentialReferenceHealthResolver,
) -> None:
    ...
    if not isinstance(resolver_kind, str) or not _SAFE_IDENTIFIER.fullmatch(resolver_kind):
        raise ValueError("Credential reference resolver kind is invalid")
    _RESOLVERS[normalized_backend] = resolver


def clear_external_document_source_credential_reference_health_resolvers() -> None:
    _RESOLVERS.clear()
...
def _validate_probe_result(result: CredentialReferenceHealthProbeResult) -> tuple[str, str | None]:
    if not isinstance(result, CredentialReferenceHealthProbeResult):
        raise ExternalDocumentSourceConflictError("Credential reference resolver returned an invalid health result")
    if result.resolvable:
        if result.failure_code is not None:
            raise ExternalDocumentSourceConflictError("Resolvable credential reference health result cannot include a failure code")
        return "resolvable", None
    if result.failure_code not in _ALLOWED_FAILURE_CODES:
        raise ExternalDocumentSourceConflictError("Credential reference resolver returned an unsupported failure code")
    return "unresolvable", result.failure_code


def _scope_hash(*, binding, resolver_kind: str, request_key: str) -> str:
    if binding.approval_hash is None:
        raise ExternalDocumentSourceConflictError("Credential reference binding approval lineage is incomplete")
...
def _request_hash(row: ExternalDocumentSourceCredentialReferenceHealthQualification) -> str:
    return _canonical_hash(
        {
            ...
            **{field: False for field in _NON_RESOLUTION_SAFETY_FIELDS},
        }
    )


def _result_hash(row: ExternalDocumentSourceCredentialReferenceHealthQualification) -> str:
    return _canonical_hash(
        {
            "qualification_id": str(row.id),
            "scope_hash": row.scope_hash,
...
def _receipt_hash(receipt: ExternalDocumentSourceCredentialReferenceHealthReceipt) -> str:
    return _canonical_hash(
        {
    ...
    *,
    ...
    row = db.scalar(
        select(ExternalDocumentSourceCredentialReferenceHealthQualification).where(
            ExternalDocumentSourceCredentialReferenceHealthQualification.id == qualification_id,
...
def _receipts(db: Session, row: ExternalDocumentSourceCredentialReferenceHealthQualification):
    return list(
        db.scalars(
            select(ExternalDocumentSourceCredentialReferenceHealthReceipt)
            .where(
                ExternalDocumentSourceCredentialReferenceHealthReceipt.organization_id == row.organization_id,
                ExternalDocumentSourceCredentialReferenceHealthReceipt.qualification_id == row.id,
            )
            .order_by(ExternalDocumentSourceCredentialReferenceHealthReceipt.sequence_number.asc())
        ).all()
    )


def _active_binding(db: Session, row: ExternalDocumentSourceCredentialReferenceHealthQualification):
    ...
    if (
        row.provider_kind != binding.provider_kind
        ...
        raise ExternalDocumentSourceConflictError("Credential reference health upstream lineage drifted")
    return binding
...
def _ensure_integrity(db: Session, row: ExternalDocumentSourceCredentialReferenceHealthQualification) -> None:
    ...
    if row.result_hash != _result_hash(row):
        raise ExternalDocumentSourceConflictError("Credential reference health result integrity failed")
    if not row.credential_reference_stored or not row.credential_reference_resolution_performed:
        raise ExternalDocumentSourceConflictError("Credential reference health resolution boundary drifted")
    if any(bool(getattr(row, field)) for field in _NON_RESOLUTION_SAFETY_FIELDS):
        raise ExternalDocumentSourceConflictError("Credential reference health safety boundary drifted")
    if row.result_status == "resolvable":
        if row.failure_code is not None:
            raise ExternalDocumentSourceConflictError("Credential reference health result facts drifted")
    elif row.result_status == "unresolvable":
        if row.failure_code not in _ALLOWED_FAILURE_CODES:
            raise ExternalDocumentSourceConflictError("Credential reference health failure code drifted")
    else:
        raise ExternalDocumentSourceConflictError("Credential reference health result status is invalid")
    if _aware(row.checked_at) < _aware(row.requested_at):
        raise ExternalDocumentSourceConflictError("Credential reference health timestamps drifted")

    receipts = _receipts(db, row)
    if len(receipts) != 2 or [receipt.event_type for receipt in receipts] != ["requested", "completed"]:
        raise ExternalDocumentSourceConflictError("Credential reference health receipt lifecycle is incomplete")
    expected = (
        (1, "requested", row.requested_at, "requested", row.request_hash, False),
        (2, "completed", row.checked_at, row.result_status, row.result_hash, True),
    )
    prior: str | None = None
    for receipt, facts in zip(receipts, expected, strict=True):
        sequence, event_type, occurred_at, status_after, decision_hash, resolution_performed = facts
        if (
            ...
            raise ExternalDocumentSourceConflictError("Credential reference health receipt facts drifted")
        if any(bool(getattr(receipt, field)) for field in _NON_RESOLUTION_SAFETY_FIELDS):
            raise ExternalDocumentSourceConflictError("Credential reference health receipt safety boundary drifted")
        if receipt.receipt_hash != _receipt_hash(receipt):
            raise ExternalDocumentSourceConflictError("Credential reference health receipt integrity failed")
...
def _append_receipt(
    db: Session,
    *,
    row: ExternalDocumentSourceCredentialReferenceHealthQualification,
    event_type: str,
    status_after: str,
    ...
    receipts = _receipts(db, row)
    receipt = ExternalDocumentSourceCredentialReferenceHealthReceipt(
        organization_id=row.organization_id,
...
def qualify_external_document_source_credential_reference_health(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    existing = db.scalar(
        select(ExternalDocumentSourceCredentialReferenceHealthQualification).where(
            ExternalDocumentSourceCredentialReferenceHealthQualification.credential_reference_binding_id == binding_id
        )
    )
    if existing is not None:
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Conflicting replay for credential reference health qualification")
    ...
    key_collision = db.scalar(
        select(ExternalDocumentSourceCredentialReferenceHealthQualification).where(
            ExternalDocumentSourceCredentialReferenceHealthQualification.organization_id == organization_id,
            ExternalDocumentSourceCredentialReferenceHealthQualification.profile_id == profile_id,
            ExternalDocumentSourceCredentialReferenceHealthQualification.request_key == normalized_key,
        )
    )
    if key_collision is not None:
        raise ExternalDocumentSourceConflictError("Conflicting replay for credential reference health request_key")

    resolver = _RESOLVERS.get(binding.reference_backend)
    if resolver is None:
        raise ExternalDocumentSourceConflictError("Credential reference resolver is unavailable")
    resolver_kind = _normalize_resolver_kind(resolver.resolver_kind)
    scope_hash = _scope_hash(binding=binding, resolver_kind=resolver_kind, request_key=normalized_key)
    ...
    try:
        ...
        raise ExternalDocumentSourceConflictError("Credential reference resolver health check failed") from exc
    result_status, failure_code = _validate_probe_result(probe)
...
def get_external_document_source_credential_reference_health_qualification(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
...
def list_external_document_source_credential_reference_health_receipts(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
