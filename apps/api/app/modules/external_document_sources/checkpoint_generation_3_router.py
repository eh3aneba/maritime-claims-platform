...
from app.modules.external_document_sources.checkpoint_generation_3_schemas import (
    ExternalDocumentSourceCheckpointGeneration3Read,
    ExternalDocumentSourceCheckpointGeneration3ReceiptRead,
    ExternalDocumentSourceCheckpointGeneration3Request,
)
from app.modules.external_document_sources.checkpoint_generation_3_service import (
    execute_external_document_source_checkpoint_generation_3,
    get_external_document_source_checkpoint_generation_3,
    list_external_document_source_checkpoint_generation_3_receipts,
)
...
def _execution_audit_values(execution) -> dict:
    return {
        ...
        "checkpoint_generation_3_advance_completed": True,
        "sync_executed": False,
...
@router.post(
    "/profiles/{profile_id}/successor-versioned-restaging-executions/{successor_versioned_restaging_execution_id}/checkpoint-generation-3-executions",
    response_model=ExternalDocumentSourceCheckpointGeneration3Read,
    status_code=status.HTTP_201_CREATED,
)
def execute_checkpoint_generation_3_endpoint(
    profile_id: UUID,
    successor_versioned_restaging_execution_id: UUID,
    payload: ExternalDocumentSourceCheckpointGeneration3Request,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
) -> ExternalDocumentSourceCheckpointGeneration3Read:
    try:
        execution, outcome = execute_external_document_source_checkpoint_generation_3(
            db,
            organization_id=current_user.organization_id,
            profile_id=profile_id,
            successor_versioned_restaging_execution_id=successor_versioned_restaging_execution_id,
        ...
        if outcome == "completed":
            write_audit_log(
                ...
                action="EXTERNAL_DOCUMENT_SOURCE_CHECKPOINT_GENERATION_3_ADVANCED",
                entity_type="external_document_source_checkpoint_generation_3_execution",
                entity_id=execution.id,
                new_values=_execution_audit_values(execution),
                details=(
                    "Phase 17.5-U recorded one immutable generation-3 successor checkpoint from the exact integrity-valid Phase R generation-2 predecessor and Phase T candidate. "
                    "No provider or object-storage I/O occurred; predecessor/candidate objects were not mutated, and no Document, Evidence, processing/OCR, recurring sync or Claim mutation occurred."
                ),
            )
            db.commit()
            db.refresh(execution)
        return ExternalDocumentSourceCheckpointGeneration3Read.model_validate(execution)
    except (
        ExternalDocumentSourceValidationError,
        ExternalDocumentSourceConflictError,
        ExternalDocumentSourceNotFoundError,
...
@router.get(
    "/profiles/{profile_id}/checkpoint-generation-3-executions/{execution_id}",
    response_model=ExternalDocumentSourceCheckpointGeneration3Read,
)
def get_checkpoint_generation_3_endpoint(
    profile_id: UUID,
    execution_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
...
@router.get(
    "/profiles/{profile_id}/checkpoint-generation-3-executions/{execution_id}/receipts",
    response_model=list[ExternalDocumentSourceCheckpointGeneration3ReceiptRead],
)
def list_checkpoint_generation_3_receipts_endpoint(
    profile_id: UUID,
    execution_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
) -> list[ExternalDocumentSourceCheckpointGeneration3ReceiptRead]:
    try:
        receipts = list_external_document_source_checkpoint_generation_3_receipts(
            db,
            organization_id=current_user.organization_id,
            profile_id=profile_id,
            execution_id=execution_id,
        )
        return [ExternalDocumentSourceCheckpointGeneration3ReceiptRead.model_validate(row) for row in receipts]
    except (
        ExternalDocumentSourceValidationError,
        ExternalDocumentSourceConflictError,
        ExternalDocumentSourceNotFoundError,
