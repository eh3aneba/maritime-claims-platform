...
from app.modules.external_document_sources.sftp_handshake_authorization_service import (
    _ensure_integrity as _ensure_handshake_authorization_integrity,
...
def _scope_hash(*, authorization: ExternalDocumentSourceSftpHandshakeAuthorization, request_key: str) -> str:
    if authorization.authorization_hash is None or authorization.authorization_expires_at is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization approval facts are incomplete")
    return _canonical_hash(
        {
            ...
            "authentication_kind": authorization.authentication_kind,
            ...
            "handshake_authorization_scope_hash": authorization.scope_hash,
            "handshake_authorization_request_hash": authorization.request_hash,
            "handshake_authorization_hash": authorization.authorization_hash,
            "handshake_authorization_expires_at": _iso(authorization.authorization_expires_at),
...
def _request_hash(execution: ExternalDocumentSourceSftpHandshakeExecution) -> str:
    return _canonical_hash(
        {
            ...
            "handshake_authorization_consumed": False,
            **{field: False for field in _FALSE_SAFETY_FIELDS},
        }
    )


def _completion_hash(execution: ExternalDocumentSourceSftpHandshakeExecution) -> str:
    if execution.authorization_terminal_hash is None or execution.completed_at is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake execution completion facts are incomplete")
    return _canonical_hash(
        {
            "execution_id": str(execution.id),
            "scope_hash": execution.scope_hash,
            ...
            "credential_reference_stored": True,
            "handshake_authorization_consumed": True,
            **{field: False for field in _FALSE_SAFETY_FIELDS},
        }
    )


def _receipt_hash(receipt: ExternalDocumentSourceSftpHandshakeExecutionReceipt) -> str:
    return _canonical_hash(
        {
            ...
            "credential_reference_stored": True,
            "handshake_authorization_consumed": receipt.handshake_authorization_consumed,
            **{field: False for field in _FALSE_SAFETY_FIELDS},
        }
    )
...
def _consumption_reason(execution_id: UUID) -> str:
    return f"Phase 17.6-E handshake execution {execution_id} consumed this bounded SFTP handshake authorization."
...
def _get_execution(
    ...
    *,
    ...
    row = db.scalar(
        select(ExternalDocumentSourceSftpHandshakeExecution).where(
            ExternalDocumentSourceSftpHandshakeExecution.id == execution_id,
            ExternalDocumentSourceSftpHandshakeExecution.organization_id == organization_id,
            ExternalDocumentSourceSftpHandshakeExecution.profile_id == profile_id,
        )
    )
    if row is None:
        raise ExternalDocumentSourceNotFoundError("SFTP handshake execution not found")
    return row
...
) -> list[ExternalDocumentSourceSftpHandshakeExecutionReceipt]:
    return list(
        db.scalars(
            select(ExternalDocumentSourceSftpHandshakeExecutionReceipt)
            .where(
                ExternalDocumentSourceSftpHandshakeExecutionReceipt.organization_id == execution.organization_id,
                ExternalDocumentSourceSftpHandshakeExecutionReceipt.execution_id == execution.id,
            )
            .order_by(ExternalDocumentSourceSftpHandshakeExecutionReceipt.sequence_number.asc())
        ).all()
    )
...
def _append_receipt(
    ...
    *,
    ...
    receipt = ExternalDocumentSourceSftpHandshakeExecutionReceipt(
        ...
        credential_reference_stored=True,
        handshake_authorization_consumed=consumed,
        **{field: False for field in _FALSE_SAFETY_FIELDS},
    )
...
def _authorization_for_execution(
    ...
    authorization = db.scalar(
        select(ExternalDocumentSourceSftpHandshakeAuthorization).where(
            ExternalDocumentSourceSftpHandshakeAuthorization.id == execution.authorization_id,
            ExternalDocumentSourceSftpHandshakeAuthorization.organization_id == execution.organization_id,
            ExternalDocumentSourceSftpHandshakeAuthorization.profile_id == execution.profile_id,
        )
    )
    if authorization is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake execution authorization lineage is missing")
    return authorization


def _ensure_integrity(db: Session, execution: ExternalDocumentSourceSftpHandshakeExecution) -> None:
    if execution.status != "completed" or not execution.handshake_authorization_consumed:
        raise ExternalDocumentSourceConflictError("SFTP handshake execution lifecycle is incomplete")
    if not execution.credential_reference_stored or any(bool(getattr(execution, field)) for field in _FALSE_SAFETY_FIELDS):
        raise ExternalDocumentSourceConflictError("SFTP handshake execution safety boundary drifted")

    authorization = _authorization_for_execution(db, execution)
    _ensure_handshake_authorization_integrity(db, authorization)
    if authorization.status != "expired" or authorization.sftp_handshake_authorized:
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization was not terminalized by execution")
    if authorization.authorization_hash is None or authorization.authorization_expires_at is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization approval facts are incomplete")
    if authorization.terminal_hash is None or authorization.terminal_at is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization terminal facts are incomplete")
    if authorization.terminal_reason != _consumption_reason(execution.id):
        raise ExternalDocumentSourceConflictError("SFTP handshake execution consumption lineage drifted")
    if _aware(authorization.terminal_at) != _aware(execution.completed_at):
        raise ExternalDocumentSourceConflictError("SFTP handshake execution completion timing drifted")
    if execution.completed_at is None or _aware(execution.completed_at) >= _aware(authorization.authorization_expires_at):
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization was not consumed before expiry")
    ...
    expected_bindings = (
        ...
        execution.authentication_kind == authorization.authentication_kind,
        ...
        execution.handshake_authorization_scope_hash == authorization.scope_hash,
        execution.handshake_authorization_request_hash == authorization.request_hash,
        execution.handshake_authorization_hash == authorization.authorization_hash,
        _aware(execution.handshake_authorization_expires_at) == _aware(authorization.authorization_expires_at),
    ...
    if not all(expected_bindings):
        raise ExternalDocumentSourceConflictError("SFTP handshake execution lineage drifted")
    ...
    if execution.scope_hash != expected_scope or execution.request_hash != _request_hash(execution):
        raise ExternalDocumentSourceConflictError("SFTP handshake execution request integrity failed")
    if execution.completion_hash != _completion_hash(execution):
        raise ExternalDocumentSourceConflictError("SFTP handshake execution completion integrity failed")

    rows = _receipts(db, execution)
    if len(rows) != 2 or [row.event_type for row in rows] != ["requested", "completed"]:
        raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt lifecycle is incomplete")
    prior: str | None = None
    for sequence, receipt in enumerate(rows, start=1):
        if receipt.sequence_number != sequence or receipt.prior_receipt_hash != prior:
            raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt chain linkage failed")
        if receipt.scope_hash != execution.scope_hash or receipt.actor_id != execution.requested_by_id:
            raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt scope or actor drifted")
        if not receipt.credential_reference_stored or any(bool(getattr(receipt, field)) for field in _FALSE_SAFETY_FIELDS):
            raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt safety boundary drifted")
        ...
        if expected_time is None or expected_decision is None:
            raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt facts are incomplete")
        if (
            ...
            or receipt.handshake_authorization_consumed != expected_consumed
        ):
            raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt facts drifted")
        if receipt.receipt_hash != _receipt_hash(receipt):
            raise ExternalDocumentSourceConflictError("SFTP handshake execution receipt integrity failed")
        prior = receipt.receipt_hash


def execute_external_document_source_sftp_handshake_authorization(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    existing = db.scalar(
        select(ExternalDocumentSourceSftpHandshakeExecution).where(
            ExternalDocumentSourceSftpHandshakeExecution.organization_id == organization_id,
            ExternalDocumentSourceSftpHandshakeExecution.profile_id == profile_id,
            ExternalDocumentSourceSftpHandshakeExecution.authorization_id == authorization_id,
        )
    )
    if existing is not None:
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Conflicting replay for consumed SFTP handshake authorization")
    ...
    key_collision = db.scalar(
        select(ExternalDocumentSourceSftpHandshakeExecution).where(
            ExternalDocumentSourceSftpHandshakeExecution.organization_id == organization_id,
            ExternalDocumentSourceSftpHandshakeExecution.profile_id == profile_id,
            ExternalDocumentSourceSftpHandshakeExecution.request_key == normalized_key,
        )
    )
    if key_collision is not None:
        raise ExternalDocumentSourceConflictError("Conflicting replay for SFTP handshake execution request_key")
    ...
    authorization = db.scalar(
        ...
        .where(
            ...
            ExternalDocumentSourceSftpHandshakeAuthorization.organization_id == organization_id,
            ExternalDocumentSourceSftpHandshakeAuthorization.profile_id == profile_id,
        )
        .with_for_update()
    )
    if authorization is None:
        raise ExternalDocumentSourceNotFoundError("SFTP handshake authorization not found")
    ...
    if (
        ...
        or not authorization.sftp_handshake_authorized
        ...
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization is not consumable")
    if current >= _aware(authorization.authorization_expires_at):
        _terminalize_expired(
            db,
            authorization,
            current=current,
            reason="Bounded SFTP handshake authorization expired unused.",
        )
        _ensure_handshake_authorization_integrity(db, authorization)
        return None, "expired"
    ...
    execution = ExternalDocumentSourceSftpHandshakeExecution(
        ...
        handshake_authorization_request_hash=authorization.request_hash,
        handshake_authorization_hash=authorization.authorization_hash,
        handshake_authorization_expires_at=authorization.authorization_expires_at,
        execution_limit=1,
    ...
    if authorization.terminal_hash is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization terminal hash is missing")

    execution.status = "completed"
    execution.authorization_terminal_hash = authorization.terminal_hash
    execution.completed_at = current
    execution.handshake_authorization_consumed = True
...
def get_external_document_source_sftp_handshake_execution(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
...
def list_external_document_source_sftp_handshake_execution_receipts(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
