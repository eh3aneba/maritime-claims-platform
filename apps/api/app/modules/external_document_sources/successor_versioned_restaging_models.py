...
MAX_SUCCESSOR_VERSIONED_RESTAGING_BYTES = 8 * 1024 * 1024
SUCCESSOR_VERSIONED_STORAGE_PURPOSE = "external_remote_content_versioned_quarantine_v1"
SUCCESSOR_CANDIDATE_GENERATION = 3


class _SuccessorVersionedRestagingSafetyMixin:
    ...
    successor_versioned_restaging_completed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=false())
...
class ExternalDocumentSourceSuccessorVersionedRestagingExecution(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _SuccessorVersionedRestagingSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_successor_versioned_restage_execs"
    __table_args__ = (
        UniqueConstraint("successor_change_detection_execution_id", name="uq_ext_doc_svr3_exec_successor_change"),
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_svr3_exec_request"),
        CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_svr3_exec_provider"),
        CheckConstraint(f"candidate_generation = {SUCCESSOR_CANDIDATE_GENERATION}", name="ck_ext_doc_svr3_exec_generation"),
        CheckConstraint(f"storage_purpose = '{SUCCESSOR_VERSIONED_STORAGE_PURPOSE}'", name="ck_ext_doc_svr3_exec_purpose"),
        CheckConstraint("status IN ('requested','content_verified','completed')", name="ck_ext_doc_svr3_exec_status"),
        CheckConstraint("result_status IS NULL OR result_status = 'staged_candidate_verified'", name="ck_ext_doc_svr3_exec_result"),
        CheckConstraint(
            f"content_byte_count IS NULL OR (content_byte_count >= 0 AND content_byte_count <= {MAX_SUCCESSOR_VERSIONED_RESTAGING_BYTES})",
            name="ck_ext_doc_svr3_exec_size",
        ),
        CheckConstraint(
        ...
        Index("ix_ext_doc_svr3_exec_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_svr3_exec_successor_change", "successor_change_detection_execution_id"),
        Index("ix_ext_doc_svr3_exec_org_status", "organization_id", "status"),
    ...
    candidate_generation: Mapped[int] = mapped_column(Integer, nullable=False, default=SUCCESSOR_CANDIDATE_GENERATION, server_default=str(SUCCESSOR_CANDIDATE_GENERATION))
    storage_backend_kind: Mapped[str] = mapped_column(String(128), nullable=False)
    storage_purpose: Mapped[str] = mapped_column(String(128), nullable=False, default=SUCCESSOR_VERSIONED_STORAGE_PURPOSE, server_default=SUCCESSOR_VERSIONED_STORAGE_PURPOSE)
...
class ExternalDocumentSourceSuccessorVersionedRestagingReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _SuccessorVersionedRestagingSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_successor_versioned_restage_receipts"
    __table_args__ = (
        UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_svr3_rcpt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_svr3_rcpt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_svr3_rcpt_seq"),
        CheckConstraint("event_type IN ('requested','content_verified','completed')", name="ck_ext_doc_svr3_rcpt_event"),
        CheckConstraint("(sequence_number = 1 AND prior_receipt_hash IS NULL) OR (sequence_number > 1 AND prior_receipt_hash IS NOT NULL)", name="ck_ext_doc_svr3_rcpt_chain"),
        Index("ix_ext_doc_svr3_rcpt_seq", "execution_id", "sequence_number"),
        *_safety_constraints("ext_doc_svr3_rcpt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_successor_versioned_restage_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
