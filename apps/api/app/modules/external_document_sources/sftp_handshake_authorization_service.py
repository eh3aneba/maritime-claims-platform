...
def _scope_hash(*, qualification: ExternalDocumentSourceSftpCredentialHealthQualification, request_key: str) -> str:
    return _canonical_hash(
        {
            ...
            "authentication_kind": qualification.authentication_kind,
...
def _request_hash(row: ExternalDocumentSourceSftpHandshakeAuthorization) -> str:
    return _canonical_hash(
        {
...
def _authorization_hash(row: ExternalDocumentSourceSftpHandshakeAuthorization) -> str:
    if row.approved_by_id is None or row.approved_at is None or row.approval_reason is None or row.authorization_expires_at is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization decision is incomplete")
    return _canonical_hash(
        {
            "authorization_id": str(row.id),
            "scope_hash": row.scope_hash,
...
def _terminal_hash(row: ExternalDocumentSourceSftpHandshakeAuthorization) -> str:
    if row.terminal_at is None or row.terminal_reason is None:
        raise ExternalDocumentSourceConflictError("SFTP handshake terminal decision is incomplete")
    return _canonical_hash(
        {
            "authorization_id": str(row.id),
            "scope_hash": row.scope_hash,
...
def _receipt_hash(receipt: ExternalDocumentSourceSftpHandshakeAuthorizationReceipt) -> str:
    return _canonical_hash(
        {
            ...
            "credential_reference_stored": True,
            "sftp_handshake_authorized": receipt.sftp_handshake_authorized,
            **{field: False for field in _FALSE_SAFETY_FIELDS},
        }
    )
...
def _receipts(db: Session, row: ExternalDocumentSourceSftpHandshakeAuthorization):
    return list(
        db.scalars(
            select(ExternalDocumentSourceSftpHandshakeAuthorizationReceipt)
            .where(
                ExternalDocumentSourceSftpHandshakeAuthorizationReceipt.organization_id == row.organization_id,
                ExternalDocumentSourceSftpHandshakeAuthorizationReceipt.authorization_id == row.id,
            )
            .order_by(ExternalDocumentSourceSftpHandshakeAuthorizationReceipt.sequence_number.asc())
        ).all()
    )


def _lineage(db: Session, row: ExternalDocumentSourceSftpHandshakeAuthorization):
    qualification = get_external_document_source_sftp_credential_health_qualification(
        db,
        organization_id=row.organization_id,
        profile_id=row.profile_id,
        qualification_id=row.health_qualification_id,
    )
    if qualification.result_status != "qualified" or qualification.failure_code is not None:
        raise ExternalDocumentSourceConflictError("SFTP handshake requires an exact qualified Phase 17.6-C credential-health qualification")
    if (
        ...
        or row.authentication_kind != qualification.authentication_kind
        ...
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization upstream lineage drifted")
    return qualification


def _expected_events(row: ExternalDocumentSourceSftpHandshakeAuthorization) -> list[str]:
    ...
    raise ExternalDocumentSourceConflictError("SFTP handshake authorization status is invalid")


def _expected_receipt_facts(row: ExternalDocumentSourceSftpHandshakeAuthorization, event_type: str):
    ...
    raise ExternalDocumentSourceConflictError("SFTP handshake receipt event is invalid")


def _ensure_integrity(db: Session, row: ExternalDocumentSourceSftpHandshakeAuthorization) -> None:
    ...
    if row.scope_hash != expected_scope or row.request_hash != _request_hash(row):
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization request integrity failed")
    if _aware(row.review_expires_at) != _aware(row.requested_at) + _REVIEW_TTL:
        raise ExternalDocumentSourceConflictError("SFTP handshake review TTL drifted")
    if row.execution_limit != 1:
        raise ExternalDocumentSourceConflictError("SFTP handshake execution boundary drifted")
    if not row.credential_reference_stored or any(bool(getattr(row, field)) for field in _FALSE_SAFETY_FIELDS):
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization safety boundary drifted")
    if row.sftp_handshake_authorized != (row.status == "authorized"):
        raise ExternalDocumentSourceConflictError("SFTP handshake authority mapping drifted")
    if row.authorization_hash is not None:
        if row.authorization_hash != _authorization_hash(row):
            raise ExternalDocumentSourceConflictError("SFTP handshake authorization decision integrity failed")
        if row.approved_at is None or row.authorization_expires_at is None:
            raise ExternalDocumentSourceConflictError("SFTP handshake authorization timing is incomplete")
        if _aware(row.authorization_expires_at) != _aware(row.approved_at) + _AUTHORIZATION_TTL:
            raise ExternalDocumentSourceConflictError("SFTP handshake authorization TTL drifted")
    if row.status in {"authorized", "expired"} and row.approved_by_id is not None and row.requested_by_id == row.approved_by_id:
        raise ExternalDocumentSourceConflictError("SFTP handshake four-eyes boundary drifted")
    if row.terminal_hash is not None and row.terminal_hash != _terminal_hash(row):
        raise ExternalDocumentSourceConflictError("SFTP handshake terminal integrity failed")
    ...
    if len(receipts) != len(events) or [receipt.event_type for receipt in receipts] != events:
        raise ExternalDocumentSourceConflictError("SFTP handshake receipt lifecycle is incomplete")
    prior: str | None = None
    for sequence, receipt in enumerate(receipts, start=1):
        if receipt.sequence_number != sequence or receipt.prior_receipt_hash != prior or receipt.scope_hash != row.scope_hash:
            raise ExternalDocumentSourceConflictError("SFTP handshake receipt chain linkage failed")
        if not receipt.credential_reference_stored or any(bool(getattr(receipt, field)) for field in _FALSE_SAFETY_FIELDS):
            raise ExternalDocumentSourceConflictError("SFTP handshake receipt safety boundary drifted")
        ...
        if expected_time is None or expected_reason is None or expected_decision is None:
            raise ExternalDocumentSourceConflictError("SFTP handshake receipt facts are incomplete")
        if (
            ...
            or receipt.sftp_handshake_authorized != expected_authorized
        ):
            raise ExternalDocumentSourceConflictError("SFTP handshake receipt facts drifted")
        if receipt.receipt_hash != _receipt_hash(receipt):
            raise ExternalDocumentSourceConflictError("SFTP handshake receipt integrity failed")
...
def _append_receipt(
    db: Session,
    *,
    row: ExternalDocumentSourceSftpHandshakeAuthorization,
    event_type: str,
    actor_id: UUID | None,
    ...
    receipts = _receipts(db, row)
    receipt = ExternalDocumentSourceSftpHandshakeAuthorizationReceipt(
        organization_id=row.organization_id,
        ...
        credential_reference_stored=True,
        sftp_handshake_authorized=authorized,
        **{field: False for field in _FALSE_SAFETY_FIELDS},
    )
...
def _terminalize_expired(db: Session, row: ExternalDocumentSourceSftpHandshakeAuthorization, *, current: datetime, reason: str) -> None:
    row.status = "expired"
    row.sftp_handshake_authorized = False
...
def _expire_if_needed(db: Session, row: ExternalDocumentSourceSftpHandshakeAuthorization, *, now: datetime) -> bool:
    current = _aware(now)
    ...
    if row.status == "authorized" and row.authorization_expires_at is not None and current >= _aware(row.authorization_expires_at):
        _terminalize_expired(db, row, current=current, reason="Bounded SFTP handshake authorization expired unused.")
        return True
    return False


def request_external_document_source_sftp_handshake_authorization(
    db: Session,
    *,
    ...
    locked = db.scalar(
        select(ExternalDocumentSourceSftpCredentialHealthQualification)
        .where(
            ExternalDocumentSourceSftpCredentialHealthQualification.id == health_qualification_id,
            ExternalDocumentSourceSftpCredentialHealthQualification.organization_id == organization_id,
            ExternalDocumentSourceSftpCredentialHealthQualification.profile_id == profile_id,
        )
        .with_for_update()
    )
    if locked is None:
    ...
    existing = db.scalar(
        select(ExternalDocumentSourceSftpHandshakeAuthorization).where(
            ExternalDocumentSourceSftpHandshakeAuthorization.health_qualification_id == health_qualification_id
        )
    )
    if existing is not None:
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Conflicting replay for SFTP handshake authorization")
        _ensure_integrity(db, existing)
    ...
    key_collision = db.scalar(
        select(ExternalDocumentSourceSftpHandshakeAuthorization).where(
            ExternalDocumentSourceSftpHandshakeAuthorization.organization_id == organization_id,
            ExternalDocumentSourceSftpHandshakeAuthorization.profile_id == profile_id,
            ExternalDocumentSourceSftpHandshakeAuthorization.request_key == normalized_key,
        )
    )
    if key_collision is not None:
        raise ExternalDocumentSourceConflictError("Conflicting replay for SFTP handshake request_key")
...
def approve_external_document_source_sftp_handshake_authorization(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    if row.status == "authorized":
        ...
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization is already approved")
    if row.status != "pending_second_approval":
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization is not pending approval")
    ...
    row.sftp_handshake_authorized = True
...
def reject_external_document_source_sftp_handshake_authorization(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    if row.status == "rejected":
        if row.terminal_by_id == rejected_by_id and row.terminal_reason == normalized_reason:
            return row, "unchanged"
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization is already rejected")
    if row.status != "pending_second_approval":
        raise ExternalDocumentSourceConflictError("SFTP handshake authorization is not pending rejection")
...
def get_external_document_source_sftp_handshake_authorization(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
...
def list_external_document_source_sftp_handshake_authorization_receipts(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
