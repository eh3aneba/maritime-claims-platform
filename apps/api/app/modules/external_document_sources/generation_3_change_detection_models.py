...
GENERATION_3_CHANGE_RESULTS = ("unchanged", "changed", "missing")
GENERATION_3_BASELINE_GENERATION = 3
MAX_GENERATION_3_METADATA_BYTE_SIZE = 10**15


class _Generation3ChangeDetectionSafetyMixin:
    ...
    generation_3_successor_change_detection_completed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=false())
...
class ExternalDocumentSourceGeneration3ChangeDetectionExecution(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _Generation3ChangeDetectionSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_gen3_change_detect_execs"
    __table_args__ = (
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_g3cd_exec_request"),
        CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_g3cd_exec_provider"),
        CheckConstraint(f"baseline_generation = {GENERATION_3_BASELINE_GENERATION}", name="ck_ext_doc_g3cd_exec_generation"),
        CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_g3cd_exec_status"),
        CheckConstraint("result_status IS NULL OR result_status IN ('unchanged','changed','missing')", name="ck_ext_doc_g3cd_exec_result"),
        CheckConstraint("baseline_item_kind = 'file'", name="ck_ext_doc_g3cd_exec_base_kind"),
        CheckConstraint("observed_item_kind IS NULL OR observed_item_kind = 'file'", name="ck_ext_doc_g3cd_exec_obs_kind"),
        CheckConstraint(
            f"baseline_byte_size IS NULL OR (baseline_byte_size >= 0 AND baseline_byte_size <= {MAX_GENERATION_3_METADATA_BYTE_SIZE})",
            name="ck_ext_doc_g3cd_exec_base_size",
        ),
        CheckConstraint(
            f"observed_byte_size IS NULL OR (observed_byte_size >= 0 AND observed_byte_size <= {MAX_GENERATION_3_METADATA_BYTE_SIZE})",
            name="ck_ext_doc_g3cd_exec_obs_size",
        ),
        CheckConstraint(
            ...
            "AND generation_3_successor_change_detection_completed = true)",
            name="ck_ext_doc_g3cd_exec_lifecycle",
        ),
        Index("ix_ext_doc_g3cd_exec_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_g3cd_exec_checkpoint", "checkpoint_generation_3_execution_id"),
        Index("ix_ext_doc_g3cd_exec_org_status", "organization_id", "status"),
    ...
    baseline_generation: Mapped[int] = mapped_column(Integer, nullable=False, default=GENERATION_3_BASELINE_GENERATION, server_default=str(GENERATION_3_BASELINE_GENERATION))
...
class ExternalDocumentSourceGeneration3ChangeDetectionReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _Generation3ChangeDetectionSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_gen3_change_detect_receipts"
    __table_args__ = (
        UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_g3cd_rcpt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_g3cd_rcpt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_g3cd_rcpt_seq"),
        CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_g3cd_rcpt_event"),
        CheckConstraint(
        ...
        CheckConstraint(
            ...
            name="ck_ext_doc_g3cd_rcpt_chain",
        ),
        Index("ix_ext_doc_g3cd_rcpt_exec", "execution_id", "sequence_number"),
        Index("ix_ext_doc_g3cd_rcpt_org", "organization_id"),
        *_safety_constraints("ext_doc_g3cd_rcpt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_gen3_change_detect_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
