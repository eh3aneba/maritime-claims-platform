...
from app.modules.external_document_sources.change_detection_models import ExternalDocumentSourceChangeDetectionExecution
...
from app.modules.external_document_sources.versioned_restaging_models import (
    ...
    MAX_VERSIONED_RESTAGING_BYTES,
    VERSIONED_STORAGE_PURPOSE,
...
def _base_safety(stage: str) -> dict[str, bool]:
    ...
    return {
        ...
        "versioned_restaging_completed": completed,
...
def _storage_key(execution: ExternalDocumentSourceVersionedRestagingExecution) -> str:
    return (
        f"external-remote-content-versioned-quarantine/{execution.organization_id}/"
        f"{execution.change_detection_execution_id}/generation-{CANDIDATE_GENERATION}/{execution.id}"
    )


def _scope_hash(change: ExternalDocumentSourceChangeDetectionExecution, *, read_operation_kind: str, read_adapter_kind: str,
                endpoint_policy_hash: str, storage_backend_kind: str, storage_object_key_hash: str, request_key: str) -> str:
    if change.completion_hash is None or change.observed_projection_hash is None or change.observed_provider_item_id_hash is None:
        raise ExternalDocumentSourceConflictError("Phase P changed observation completion facts are incomplete")
    return _canonical_hash({
        "organization_id": str(change.organization_id),
        "profile_id": str(change.profile_id),
        "change_detection_execution_id": str(change.id),
        "sync_checkpoint_execution_id": str(change.sync_checkpoint_execution_id),
        "original_staging_execution_id": str(change.remote_content_staging_execution_id),
        ...
        "storage_purpose": VERSIONED_STORAGE_PURPOSE,
        "storage_object_key_hash": storage_object_key_hash,
        "request_key": request_key,
    })


def _request_hash(execution: ExternalDocumentSourceVersionedRestagingExecution) -> str:
    return _canonical_hash({
...
def _content_proof_hash(execution: ExternalDocumentSourceVersionedRestagingExecution) -> str:
    if (
        ...
        raise ExternalDocumentSourceConflictError("Versioned restaging content proof is incomplete")
    return _canonical_hash({
        "execution_id": str(execution.id),
        "scope_hash": execution.scope_hash,
...
def _completion_hash(execution: ExternalDocumentSourceVersionedRestagingExecution) -> str:
    if execution.completed_at is None or execution.result_status != "staged_candidate_verified" or execution.content_proof_hash is None:
        raise ExternalDocumentSourceConflictError("Versioned restaging completion facts are incomplete")
    return _canonical_hash({
        "execution_id": str(execution.id),
        "scope_hash": execution.scope_hash,
        "request_hash": execution.request_hash,
...
def _receipt_hash(receipt: ExternalDocumentSourceVersionedRestagingReceipt) -> str:
    return _canonical_hash({
...
def _receipts(db: Session, execution: ExternalDocumentSourceVersionedRestagingExecution):
    return list(db.scalars(select(ExternalDocumentSourceVersionedRestagingReceipt).where(
        ExternalDocumentSourceVersionedRestagingReceipt.organization_id == execution.organization_id,
        ExternalDocumentSourceVersionedRestagingReceipt.execution_id == execution.id,
    ).order_by(ExternalDocumentSourceVersionedRestagingReceipt.sequence_number.asc())).all())


def _append_receipt(db: Session, *, execution: ExternalDocumentSourceVersionedRestagingExecution, event_type: str,
                    actor_id: UUID, occurred_at: datetime, decision_hash: str) -> None:
    rows = _receipts(db, execution)
    receipt = ExternalDocumentSourceVersionedRestagingReceipt(
...
def _get_change(db: Session, *, organization_id: UUID, profile_id: UUID, change_id: UUID, for_update: bool = False):
    stmt = select(ExternalDocumentSourceChangeDetectionExecution).where(
        ExternalDocumentSourceChangeDetectionExecution.id == change_id,
    ...
    row = db.scalar(stmt)
    if row is None:
        raise ExternalDocumentSourceNotFoundError("Phase P change-detection execution not found")
    _ensure_change_detection_integrity(db, row)
    return row


def _get_execution(db: Session, *, organization_id: UUID, profile_id: UUID, execution_id: UUID):
    row = db.scalar(select(ExternalDocumentSourceVersionedRestagingExecution).where(
        ExternalDocumentSourceVersionedRestagingExecution.id == execution_id,
    ...
    if row is None:
        raise ExternalDocumentSourceNotFoundError("Versioned restaging execution not found")
    return row


def _validate_content_result(result, policy, change: ExternalDocumentSourceChangeDetectionExecution):
    if getattr(result, "read", None) is not True:
        failure_code = getattr(result, "failure_code", None)
        raise ExternalDocumentSourceConflictError(f"Versioned restaging remote content read failed ({failure_code or 'invalid_result'})")
    if getattr(result, "failure_code", None) is not None or type(getattr(result, "content", None)) is not bytes:
        raise ExternalDocumentSourceConflictError("Versioned restaging content adapter returned an invalid successful result")
    payload = result.content
    if len(payload) > policy.max_content_bytes or len(payload) > MAX_VERSIONED_RESTAGING_BYTES:
        raise ExternalDocumentSourceConflictError("Versioned restaging remote content exceeded the byte bound")
    if change.observed_byte_size is not None and len(payload) != change.observed_byte_size:
        raise ExternalDocumentSourceConflictError("Versioned restaging byte count no longer matches the Phase P observation")
    latency = getattr(result, "latency_class", None)
    if latency not in _ALLOWED_LATENCY_CLASSES:
        raise ExternalDocumentSourceConflictError("Versioned restaging adapter returned an invalid latency class")
    ...
    if media is not None and expected_media is not None and media != expected_media:
        raise ExternalDocumentSourceConflictError("Versioned restaging media type no longer matches the Phase P observation")
    media = media or expected_media
    version = _normalize_hash(getattr(result, "observed_version_token_hash", None), field="Observed version-token hash")
    expected_version = _normalize_hash(change.observed_version_token_hash, field="Phase P observed version-token hash")
    if expected_version is not None and version != expected_version:
        raise ExternalDocumentSourceConflictError("Versioned restaging version no longer matches the Phase P observation")
    return payload, hashlib.sha256(payload).hexdigest(), len(payload), media, version, latency


def _verify_storage_object(store, execution: ExternalDocumentSourceVersionedRestagingExecution):
    if execution.content_sha256 is None or execution.content_byte_count is None:
        raise ExternalDocumentSourceConflictError("Versioned restaging storage proof is unavailable")
    try:
        metadata = store.head_object(storage_key=execution.storage_object_key)
        if metadata.file_hash.lower() != execution.content_sha256 or metadata.file_size_bytes != execution.content_byte_count:
            raise ExternalDocumentSourceConflictError("Versioned quarantine object does not match the persisted content proof")
        payload = store.get_bytes(storage_key=execution.storage_object_key, expected_sha256=execution.content_sha256)
        if len(payload) != execution.content_byte_count:
            raise ExternalDocumentSourceConflictError("Versioned quarantine object byte count drifted")
        del payload
        return metadata
        ...
        raise ExternalDocumentSourceConflictError("Governed versioned quarantine storage verification failed") from None


def _ensure_integrity(db: Session, execution: ExternalDocumentSourceVersionedRestagingExecution, *, verify_storage: bool = False) -> None:
    change = _get_change(
        db,
        organization_id=execution.organization_id,
        profile_id=execution.profile_id,
        change_id=execution.change_detection_execution_id,
    )
    if change.result_status != "changed" or change.completion_hash is None or change.observed_projection_hash is None or change.observed_provider_item_id_hash is None:
        raise ExternalDocumentSourceConflictError("Phase P observation is not an eligible changed item")
    ...
    if profile.status != "active" or profile.profile_hash != change.profile_hash:
        raise ExternalDocumentSourceConflictError("Versioned restaging source profile is not active")
    ...
    if (
        execution.sync_checkpoint_execution_id != change.sync_checkpoint_execution_id
        or execution.original_staging_execution_id != change.remote_content_staging_execution_id
        ...
        or execution.change_scope_hash != change.scope_hash
        or execution.change_request_hash != change.request_hash
        or execution.change_completion_hash != change.completion_hash
        ...
        or execution.candidate_generation != CANDIDATE_GENERATION
        or execution.storage_purpose != VERSIONED_STORAGE_PURPOSE
        ...
        raise ExternalDocumentSourceConflictError("Versioned restaging lineage/custody drifted")
    if not isinstance(execution.read_adapter_kind, str) or not _SAFE_IDENTIFIER.fullmatch(execution.read_adapter_kind):
        raise ExternalDocumentSourceConflictError("Versioned restaging adapter identity drifted")
    ...
    if execution.scope_hash != expected_scope or execution.request_hash != _request_hash(execution):
        raise ExternalDocumentSourceConflictError("Versioned restaging request integrity failed")

    rows = _receipts(db, execution)
    expected_events = {"requested": ["requested"], "content_verified": ["requested", "content_verified"], "completed": ["requested", "content_verified", "completed"]}
    if execution.status not in expected_events or [row.event_type for row in rows] != expected_events[execution.status]:
        raise ExternalDocumentSourceConflictError("Versioned restaging receipt lifecycle drifted")
    prior = None
    for index, receipt in enumerate(rows, start=1):
        decision = execution.request_hash if receipt.event_type == "requested" else execution.content_proof_hash if receipt.event_type == "content_verified" else execution.completion_hash
        occurred = execution.requested_at if receipt.event_type == "requested" else execution.content_verified_at if receipt.event_type == "content_verified" else execution.completed_at
        if (
            ...
            raise ExternalDocumentSourceConflictError("Versioned restaging receipt integrity failed")
        for field, expected in _base_safety(receipt.event_type).items():
            if bool(getattr(receipt, field)) != expected:
                raise ExternalDocumentSourceConflictError("Versioned restaging receipt safety boundary drifted")
    ...
    for field, expected in _base_safety(execution.status).items():
        if bool(getattr(execution, field)) != expected:
            raise ExternalDocumentSourceConflictError("Versioned restaging safety boundary drifted")
    if execution.status == "requested":
        if any(value is not None for value in (execution.content_sha256, execution.content_byte_count, execution.content_proof_hash, execution.content_verified_at, execution.completed_at, execution.completion_hash, execution.result_status, execution.stored_etag)):
            raise ExternalDocumentSourceConflictError("Versioned restaging request anchor drifted")
        return
    if (
        execution.content_sha256 is None
        or not _HEX_64.fullmatch(execution.content_sha256)
        or execution.content_byte_count is None
        or not 0 <= execution.content_byte_count <= MAX_VERSIONED_RESTAGING_BYTES
        ...
        raise ExternalDocumentSourceConflictError("Versioned restaging content proof integrity failed")
    if execution.status == "content_verified":
        if execution.completed_at is not None or execution.completion_hash is not None or execution.result_status is not None or execution.stored_etag is not None:
            raise ExternalDocumentSourceConflictError("Versioned restaging content-verified lifecycle drifted")
        return
    if (
        ...
        raise ExternalDocumentSourceConflictError("Versioned restaging completion integrity failed")
    if verify_storage:
        _verify_storage_object(_configured_store(), execution)


def _read_changed_payload(db: Session, execution: ExternalDocumentSourceVersionedRestagingExecution, change, listing, item, profile):
    policy = _read_policy(profile.provider_kind, profile.normalized_config, item)
    if _endpoint_policy_hash(policy) != execution.endpoint_policy_hash or policy.read_operation_kind != execution.read_operation_kind:
        raise ExternalDocumentSourceConflictError("Versioned restaging read policy drifted")
    adapter = _READ_ADAPTERS.get((profile.provider_kind, policy.read_operation_kind))
    if adapter is None:
        raise ExternalDocumentSourceConflictError("Remote file content read adapter is unavailable for versioned restaging")
    if getattr(adapter, "adapter_kind", None) != execution.read_adapter_kind:
        raise ExternalDocumentSourceConflictError("Versioned restaging read adapter drifted from the request anchor")
    ...
    try:
        ...
        raise ExternalDocumentSourceConflictError("Versioned restaging remote content reread failed") from None
...
def execute_external_document_source_versioned_restaging(
    db: Session, *, organization_id: UUID, profile_id: UUID, change_detection_execution_id: UUID,
    ...
    if not isinstance(backend, str) or not _SAFE_IDENTIFIER.fullmatch(backend):
        raise ExternalDocumentSourceConflictError("Governed versioned quarantine storage backend identity is invalid")

    existing = db.scalar(select(ExternalDocumentSourceVersionedRestagingExecution).where(
        ExternalDocumentSourceVersionedRestagingExecution.organization_id == organization_id,
        ExternalDocumentSourceVersionedRestagingExecution.profile_id == profile_id,
        ExternalDocumentSourceVersionedRestagingExecution.change_detection_execution_id == change_detection_execution_id,
    ))
    if existing is not None:
        ...
        if existing.request_key != normalized_key or existing.request_reason != normalized_reason or existing.requested_by_id != requested_by_id:
            raise ExternalDocumentSourceConflictError("Conflicting replay or second consumption for versioned restaging")
        if existing.status == "completed":
            return existing, "unchanged"
        execution = existing
        change = _get_change(db, organization_id=organization_id, profile_id=profile_id, change_id=change_detection_execution_id)
    else:
        collision = db.scalar(select(ExternalDocumentSourceVersionedRestagingExecution).where(
            ExternalDocumentSourceVersionedRestagingExecution.organization_id == organization_id,
            ExternalDocumentSourceVersionedRestagingExecution.profile_id == profile_id,
            ExternalDocumentSourceVersionedRestagingExecution.request_key == normalized_key,
        ))
        if collision is not None:
            raise ExternalDocumentSourceConflictError("Conflicting replay for versioned restaging request_key")
        change = _get_change(db, organization_id=organization_id, profile_id=profile_id, change_id=change_detection_execution_id, for_update=True)
        if change.result_status != "changed" or change.completion_hash is None or change.observed_projection_hash is None or change.observed_provider_item_id_hash is None:
            raise ExternalDocumentSourceConflictError("Only a completed Phase P changed observation is eligible for versioned restaging")
        listing, item = _listing_and_item(db, change)
        if item.item_kind != "file" or hashlib.sha256(item.provider_item_id.encode("utf-8")).hexdigest() != change.observed_provider_item_id_hash:
            raise ExternalDocumentSourceConflictError("Phase P exact file identity is not eligible for versioned restaging")
        ...
        if not isinstance(adapter_kind, str) or not _SAFE_IDENTIFIER.fullmatch(adapter_kind):
            raise ExternalDocumentSourceConflictError("Versioned restaging adapter identity is invalid")
        current = _aware(now or _utc_now())
        execution = ExternalDocumentSourceVersionedRestagingExecution(
            organization_id=organization_id,
            profile_id=profile_id,
            change_detection_execution_id=change.id,
            sync_checkpoint_execution_id=change.sync_checkpoint_execution_id,
            original_staging_execution_id=change.remote_content_staging_execution_id,
    ...
    payload = None
    try:
        transient_result, payload, digest, count, media, version, latency = _read_changed_payload(db, execution, change, listing, item, profile)
        if execution.status == "requested":
            ...
            _append_receipt(db, execution=execution, event_type="content_verified", actor_id=requested_by_id, occurred_at=content_verified_at, decision_hash=execution.content_proof_hash)
            db.flush()
            _ensure_integrity(db, execution, verify_storage=False)
            db.commit()
            db.refresh(execution)
            ...
            if (
                ...
                raise ExternalDocumentSourceConflictError("Versioned restaging recovery reread no longer matches the persisted content proof")
        try:
            store.put_bytes_if_absent(payload, storage_key=execution.storage_object_key, expected_sha256=execution.content_sha256)
        except ObjectStoragePreconditionFailed:
            pass
        except ObjectStorageError:
            raise ExternalDocumentSourceConflictError("Governed versioned quarantine storage write failed") from None
    finally:
...
def get_external_document_source_versioned_restaging(db: Session, *, organization_id: UUID, profile_id: UUID, execution_id: UUID):
...
def list_external_document_source_versioned_restaging_receipts(db: Session, *, organization_id: UUID, profile_id: UUID, execution_id: UUID):
