...
from app.modules.external_document_sources.successor_change_detection_schemas import (
    ExternalDocumentSourceSuccessorChangeDetectionRead,
    ExternalDocumentSourceSuccessorChangeDetectionReceiptRead,
    ExternalDocumentSourceSuccessorChangeDetectionRequest,
)
from app.modules.external_document_sources.successor_change_detection_service import (
    execute_external_document_source_successor_change_detection,
    get_external_document_source_successor_change_detection,
    list_external_document_source_successor_change_detection_receipts,
)
...
def _execution_audit_values(execution) -> dict:
    return {
        ...
        "successor_change_detection_completed": True,
...
@router.post(
    "/profiles/{profile_id}/checkpoint-generation-executions/{checkpoint_generation_execution_id}/successor-change-detection-executions",
    response_model=ExternalDocumentSourceSuccessorChangeDetectionRead,
    status_code=status.HTTP_201_CREATED,
)
def execute_successor_change_detection_endpoint(
    profile_id: UUID,
    checkpoint_generation_execution_id: UUID,
    payload: ExternalDocumentSourceSuccessorChangeDetectionRequest,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
) -> ExternalDocumentSourceSuccessorChangeDetectionRead:
    try:
        execution, outcome = execute_external_document_source_successor_change_detection(
            db,
            organization_id=current_user.organization_id,
            profile_id=profile_id,
            checkpoint_generation_execution_id=checkpoint_generation_execution_id,
        ...
        if outcome == "completed":
            write_audit_log(
                ...
                action="EXTERNAL_DOCUMENT_SOURCE_SUCCESSOR_EXACT_ITEM_CHANGE_DETECTION_COMPLETED",
                entity_type="external_document_source_successor_change_detection_execution",
                entity_id=execution.id,
                new_values=_execution_audit_values(execution),
                details=(
                    "Phase 17.5-S performed one explicit metadata-only observation of the exact provider item against the immutable generation-2 successor checkpoint. "
        ...
        return ExternalDocumentSourceSuccessorChangeDetectionRead.model_validate(execution)
    except (
        ExternalDocumentSourceValidationError,
        ExternalDocumentSourceConflictError,
        ExternalDocumentSourceNotFoundError,
...
@router.get(
    "/profiles/{profile_id}/successor-change-detection-executions/{execution_id}",
    response_model=ExternalDocumentSourceSuccessorChangeDetectionRead,
)
def get_successor_change_detection_endpoint(
    profile_id: UUID,
    execution_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
...
@router.get(
    "/profiles/{profile_id}/successor-change-detection-executions/{execution_id}/receipts",
    response_model=list[ExternalDocumentSourceSuccessorChangeDetectionReceiptRead],
)
def list_successor_change_detection_receipts_endpoint(
    profile_id: UUID,
    execution_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
) -> list[ExternalDocumentSourceSuccessorChangeDetectionReceiptRead]:
    try:
        receipts = list_external_document_source_successor_change_detection_receipts(
            db,
            organization_id=current_user.organization_id,
            profile_id=profile_id,
            execution_id=execution_id,
        )
        return [ExternalDocumentSourceSuccessorChangeDetectionReceiptRead.model_validate(row) for row in receipts]
    except (
        ExternalDocumentSourceValidationError,
        ExternalDocumentSourceConflictError,
        ExternalDocumentSourceNotFoundError,
