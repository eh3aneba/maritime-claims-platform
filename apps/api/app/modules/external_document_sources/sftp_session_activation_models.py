...
class _SftpSessionActivationSafetyMixin:
...
class ExternalDocumentSourceSftpSessionActivation(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _SftpSessionActivationSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_sftp_session_activations"
    __table_args__ = (
        UniqueConstraint("transport_verification_id", name="uq_ext_doc_sftp_session_activation_verify"),
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_sftp_session_activation_request"),
        CheckConstraint("provider_kind = 'sftp'", name="ck_ext_doc_sftp_session_activation_provider"),
        CheckConstraint("authentication_kind IN ('password','private_key')", name="ck_ext_doc_sftp_session_activation_auth_kind"),
        CheckConstraint("activation_limit = 1", name="ck_ext_doc_sftp_session_activation_limit"),
        CheckConstraint("result_status IN ('activated','failed')", name="ck_ext_doc_sftp_session_activation_result"),
        CheckConstraint(
        ...
        CheckConstraint("latency_class IS NULL OR latency_class IN ('fast','normal','slow')", name="ck_ext_doc_sftp_session_activation_latency"),
        CheckConstraint(
        ...
        CheckConstraint("checked_at >= requested_at", name="ck_ext_doc_sftp_session_activation_time"),
        Index("ix_ext_doc_sftp_session_activation_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_sftp_session_activation_org_result", "organization_id", "result_status"),
    ...
    transport_verification_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_sftp_transport_verifications.id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    transport_verification_scope_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    transport_verification_request_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    transport_verification_result_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    ...
    activation_limit: Mapped[int] = mapped_column(Integer, nullable=False, default=1, server_default="1")
...
class ExternalDocumentSourceSftpSessionActivationReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _SftpSessionActivationSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_sftp_session_activation_receipts"
    __table_args__ = (
        UniqueConstraint("activation_id", "sequence_number", name="uq_ext_doc_sftp_session_activation_rcpt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_sftp_session_activation_rcpt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_sftp_session_activation_rcpt_seq"),
        CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_sftp_session_activation_rcpt_event"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_sftp_session_activation_rcpt_seq", "activation_id", "sequence_number"),
        *_safety_constraints("ext_doc_sftp_session_activation_rcpt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    activation_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_sftp_session_activations.id", ondelete="RESTRICT"), nullable=False, index=True)
