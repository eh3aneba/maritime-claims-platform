...
class _ProviderClientHealthSafetyMixin:
    ...
    provider_network_health_performed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default=false())
...
class ExternalDocumentSourceProviderClientHealthExecution(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _ProviderClientHealthSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_provider_client_health_execs"
    __table_args__ = (
        UniqueConstraint("token_acquisition_execution_id", name="uq_ext_doc_pch_exec_token"),
        UniqueConstraint("organization_id", "profile_id", "request_key", name="uq_ext_doc_pch_exec_request"),
        CheckConstraint("provider_kind IN ('sharepoint','google_drive')", name="ck_ext_doc_pch_exec_provider"),
        CheckConstraint("status IN ('requested','completed')", name="ck_ext_doc_pch_exec_status"),
        CheckConstraint("result_status IS NULL OR result_status = 'healthy'", name="ck_ext_doc_pch_exec_result"),
        CheckConstraint("latency_class IS NULL OR latency_class IN ('fast','normal','slow','unknown')", name="ck_ext_doc_pch_exec_latency"),
        CheckConstraint(
        ...
        Index("ix_ext_doc_pch_exec_org_profile", "organization_id", "profile_id"),
        Index("ix_ext_doc_pch_exec_org_status", "organization_id", "status"),
    ...
    token_acquirer_kind: Mapped[str] = mapped_column(String(128), nullable=False)
    token_acquisition_scope_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    token_acquisition_request_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    token_acquisition_completion_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ExternalDocumentSourceProviderClientHealthReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _ProviderClientHealthSafetyMixin,
    Base,
):
    __tablename__ = "external_doc_source_provider_client_health_receipts"
    __table_args__ = (
        UniqueConstraint("execution_id", "sequence_number", name="uq_ext_doc_pch_rcpt_seq"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_ext_doc_pch_rcpt_hash"),
        CheckConstraint("sequence_number > 0", name="ck_ext_doc_pch_rcpt_seq"),
        CheckConstraint("event_type IN ('requested','completed')", name="ck_ext_doc_pch_rcpt_event"),
        CheckConstraint(
        ...
        CheckConstraint("(sequence_number = 1 AND prior_receipt_hash IS NULL) OR (sequence_number > 1 AND prior_receipt_hash IS NOT NULL)", name="ck_ext_doc_pch_rcpt_chain"),
        Index("ix_ext_doc_pch_rcpt_seq", "execution_id", "sequence_number"),
        *_safety_constraints("ext_doc_pch_rcpt"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    execution_id: Mapped[UUID] = mapped_column(ForeignKey("external_doc_source_provider_client_health_execs.id", ondelete="RESTRICT"), nullable=False, index=True)
