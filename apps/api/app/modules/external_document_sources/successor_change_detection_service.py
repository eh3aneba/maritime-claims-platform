...
from app.modules.external_document_sources.change_detection_service import (
    ...
    _ensure_integrity as _ensure_phase_p_integrity,
...
from app.modules.external_document_sources.checkpoint_generation_service import (
    _ensure_integrity as _ensure_checkpoint_generation_integrity,
)
...
from app.modules.external_document_sources.versioned_restaging_service import (
    _ensure_integrity as _ensure_versioned_restaging_integrity,
)
...
def _base_safety(completed: bool) -> dict[str, bool]:
    return {
        ...
        "successor_change_detection_completed": completed,
...
def _baseline_facts(change: ExternalDocumentSourceChangeDetectionExecution) -> dict:
    if (
        change.result_status != "changed"
        or change.observed_projection_hash is None
        or change.observed_provider_item_id_hash is None
        or change.observed_item_kind is None
        or change.observed_display_name_hash is None
    ):
        raise ExternalDocumentSourceConflictError("Phase P observed generation-2 baseline is incomplete")
    facts = {
        "provider_item_id_hash": change.observed_provider_item_id_hash,
        "item_kind": change.observed_item_kind,
        "display_name_hash": change.observed_display_name_hash,
        "parent_item_id_hash": change.observed_parent_item_id_hash,
        "mime_type_class": change.observed_mime_type_class,
        "byte_size": change.observed_byte_size,
        "modified_at": change.observed_modified_at,
        "version_token_hash": change.observed_version_token_hash,
    }
    if _projection_hash(facts) != change.observed_projection_hash:
        raise ExternalDocumentSourceConflictError("Phase P observed generation-2 projection drifted")
    return facts


def _persisted_baseline_facts(execution: ExternalDocumentSourceSuccessorChangeDetectionExecution) -> dict:
    return {
...
def _persisted_observed_facts(execution: ExternalDocumentSourceSuccessorChangeDetectionExecution) -> dict:
    return {
...
def _scope_hash(
    checkpoint: ExternalDocumentSourceCheckpointGenerationExecution,
    *,
    ...
    if checkpoint.completion_hash is None:
        raise ExternalDocumentSourceConflictError("Phase R successor checkpoint completion hash is missing")
    return _canonical_hash(
        {
            "organization_id": str(checkpoint.organization_id),
            "profile_id": str(checkpoint.profile_id),
            "checkpoint_generation_execution_id": str(checkpoint.id),
            ...
            "versioned_restaging_execution_id": str(checkpoint.versioned_restaging_execution_id),
...
def _request_hash(execution: ExternalDocumentSourceSuccessorChangeDetectionExecution) -> str:
    return _canonical_hash(
        {
...
def _completion_hash(execution: ExternalDocumentSourceSuccessorChangeDetectionExecution) -> str:
    if execution.completed_at is None or execution.result_status not in {"unchanged", "changed", "missing"}:
        raise ExternalDocumentSourceConflictError("Successor change-detection completion facts are incomplete")
    return _canonical_hash(
        {
            "execution_id": str(execution.id),
            "scope_hash": execution.scope_hash,
...
def _receipt_hash(receipt: ExternalDocumentSourceSuccessorChangeDetectionReceipt) -> str:
    return _canonical_hash(
        {
...
def _receipts(
    db: Session,
    execution: ExternalDocumentSourceSuccessorChangeDetectionExecution,
) -> list[ExternalDocumentSourceSuccessorChangeDetectionReceipt]:
    return list(
        db.scalars(
            select(ExternalDocumentSourceSuccessorChangeDetectionReceipt)
            .where(
                ExternalDocumentSourceSuccessorChangeDetectionReceipt.organization_id == execution.organization_id,
                ExternalDocumentSourceSuccessorChangeDetectionReceipt.execution_id == execution.id,
            )
            .order_by(ExternalDocumentSourceSuccessorChangeDetectionReceipt.sequence_number.asc())
        ).all()
    )
...
def _append_receipt(
    ...
    *,
    ...
    receipt = ExternalDocumentSourceSuccessorChangeDetectionReceipt(
...
def _checkpoint_generation(
    ...
    *,
    ...
    stmt = select(ExternalDocumentSourceCheckpointGenerationExecution).where(
    ...
    row = db.scalar(stmt)
    if row is None:
        raise ExternalDocumentSourceNotFoundError("Phase R checkpoint-generation execution not found")
    _ensure_checkpoint_generation_integrity(db, row)
    if (
        row.status != "completed"
        or row.result_status != "checkpoint_generation_advanced"
        ...
        raise ExternalDocumentSourceConflictError("Phase R successor checkpoint is not eligible for observation")
    return row
...
def _lineage(
    ...
    candidate = db.scalar(
        select(ExternalDocumentSourceVersionedRestagingExecution).where(
            ExternalDocumentSourceVersionedRestagingExecution.id == checkpoint.versioned_restaging_execution_id,
            ExternalDocumentSourceVersionedRestagingExecution.organization_id == checkpoint.organization_id,
            ExternalDocumentSourceVersionedRestagingExecution.profile_id == checkpoint.profile_id,
        )
    )
    if candidate is None:
        raise ExternalDocumentSourceConflictError("Phase Q candidate lineage is missing")
    ...
    change = db.scalar(
        select(ExternalDocumentSourceChangeDetectionExecution).where(
            ExternalDocumentSourceChangeDetectionExecution.id == checkpoint.change_detection_execution_id,
            ExternalDocumentSourceChangeDetectionExecution.organization_id == checkpoint.organization_id,
            ExternalDocumentSourceChangeDetectionExecution.profile_id == checkpoint.profile_id,
        )
    )
    if change is None:
        raise ExternalDocumentSourceConflictError("Phase P changed-observation lineage is missing")
    _ensure_phase_p_integrity(db, change)
    baseline = _baseline_facts(change)
    baseline_hash = _projection_hash(baseline)

    if (
        candidate.id != checkpoint.versioned_restaging_execution_id
        or candidate.change_detection_execution_id != change.id
        or checkpoint.predecessor_sync_checkpoint_execution_id != change.sync_checkpoint_execution_id
        or candidate.listing_execution_id != checkpoint.listing_execution_id
        or candidate.metadata_item_id != checkpoint.metadata_item_id
        or checkpoint.provider_kind != change.provider_kind
        or checkpoint.profile_hash != change.profile_hash
        ...
        raise ExternalDocumentSourceConflictError("Successor observation generation-2 lineage drifted")

    if checkpoint.version_token_hash is not None and checkpoint.version_token_hash != baseline["version_token_hash"]:
        raise ExternalDocumentSourceConflictError("Successor checkpoint version no longer matches the Phase P generation-2 observation")
    if checkpoint.content_byte_count != baseline["byte_size"]:
        raise ExternalDocumentSourceConflictError("Successor checkpoint byte count no longer matches the generation-2 metadata baseline")
    if checkpoint.media_type_class is not None and baseline["mime_type_class"] is not None and checkpoint.media_type_class != baseline["mime_type_class"]:
        raise ExternalDocumentSourceConflictError("Successor checkpoint media type no longer matches the generation-2 metadata baseline")
    ...
    if hashlib.sha256(item.provider_item_id.encode("utf-8")).hexdigest() != baseline["provider_item_id_hash"]:
        raise ExternalDocumentSourceConflictError("Successor exact provider-item identity drifted")
    ...
    if profile.status != "active" or profile.profile_hash != checkpoint.profile_hash:
        raise ExternalDocumentSourceConflictError("Successor observation source profile is not active")
    policy = _policy(profile.provider_kind, profile.normalized_config, item.provider_item_id)
    return candidate, change, listing, item, profile, policy, baseline, baseline_hash


def _ensure_integrity(db: Session, execution: ExternalDocumentSourceSuccessorChangeDetectionExecution) -> None:
    checkpoint = _checkpoint_generation(
        db,
        organization_id=execution.organization_id,
        profile_id=execution.profile_id,
        execution_id=execution.checkpoint_generation_execution_id,
    )
    candidate, change, _listing, _item, _profile, policy, baseline, baseline_hash = _lineage(db, checkpoint)
    expected_policy_hash = _endpoint_policy_hash(policy)

    if (
        execution.versioned_restaging_execution_id != checkpoint.versioned_restaging_execution_id
        ...
        or execution.baseline_generation != SUCCESSOR_CHECKPOINT_GENERATION
        ...
        raise ExternalDocumentSourceConflictError("Successor change-detection baseline lineage drifted")

    adapter_kind = _normalize_identifier(execution.observation_adapter_kind, field="Successor change-detection adapter kind")
    ...
    if execution.scope_hash != expected_scope or execution.request_hash != _request_hash(execution):
        raise ExternalDocumentSourceConflictError("Successor change-detection request integrity failed")

    if execution.status != "completed" or execution.result_status not in {"unchanged", "changed", "missing"}:
        raise ExternalDocumentSourceConflictError("Successor change-detection lifecycle is incomplete")
    if execution.completed_at is None or _aware(execution.completed_at) < _aware(execution.requested_at):
        raise ExternalDocumentSourceConflictError("Successor change-detection timestamps drifted")
    for field, expected in _base_safety(True).items():
        if bool(getattr(execution, field)) != expected:
            raise ExternalDocumentSourceConflictError("Successor change-detection safety boundary drifted")
    ...
    if execution.result_status == "missing":
        ...
        if any(value is not None for value in observed_fields) or execution.changed_dimensions != "missing":
            raise ExternalDocumentSourceConflictError("Successor missing-item observation facts drifted")
    else:
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Successor observed exact-item identity drifted")
        if execution.observed_projection_hash != _projection_hash(observed):
            raise ExternalDocumentSourceConflictError("Successor observed metadata projection integrity failed")
        ...
        if execution.result_status != expected_result or execution.changed_dimensions != ",".join(expected_dimensions):
            raise ExternalDocumentSourceConflictError("Successor change-detection comparison facts drifted")

    if execution.completion_hash != _completion_hash(execution):
        raise ExternalDocumentSourceConflictError("Successor change-detection completion integrity failed")

    rows = _receipts(db, execution)
    if len(rows) != 2 or [row.event_type for row in rows] != ["requested", "completed"]:
        raise ExternalDocumentSourceConflictError("Successor change-detection receipt lifecycle is incomplete")
    expected_rows = (
    ...
    for receipt, facts in zip(rows, expected_rows, strict=True):
        ...
        if (
            ...
            raise ExternalDocumentSourceConflictError("Successor change-detection receipt integrity failed")
        for field, expected in _base_safety(completed).items():
            if bool(getattr(receipt, field)) != expected:
                raise ExternalDocumentSourceConflictError("Successor change-detection receipt safety boundary drifted")
        prior = receipt.receipt_hash


def execute_external_document_source_successor_change_detection(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    checkpoint_generation_execution_id: UUID,
    ...
    existing = db.scalar(
        select(ExternalDocumentSourceSuccessorChangeDetectionExecution).where(
            ...
            ExternalDocumentSourceSuccessorChangeDetectionExecution.request_key == normalized_key,
        )
    )
    if existing is not None:
        _ensure_integrity(db, existing)
        if (
            existing.checkpoint_generation_execution_id != checkpoint_generation_execution_id
            or existing.request_reason != normalized_reason
            or existing.requested_by_id != requested_by_id
        ):
            raise ExternalDocumentSourceConflictError("Conflicting replay for successor change-detection request_key")
    ...
    checkpoint = _checkpoint_generation(
        ...
        execution_id=checkpoint_generation_execution_id,
        for_update=True,
    )
    candidate, change, listing, item, profile, policy, baseline, baseline_hash = _lineage(db, checkpoint)
    adapter = _OBSERVATION_ADAPTERS.get((profile.provider_kind, policy.observation_operation_kind))
    if adapter is None:
        raise ExternalDocumentSourceConflictError("Exact-item successor change-detection adapter is unavailable")
    adapter_kind = _normalize_identifier(adapter.adapter_kind, field="Successor change-detection adapter kind")
    if (
        ...
        raise ExternalDocumentSourceConflictError("Successor change-detection adapter policy drifted")
    ...
    execution = ExternalDocumentSourceSuccessorChangeDetectionExecution(
        organization_id=organization_id,
        profile_id=profile_id,
        checkpoint_generation_execution_id=checkpoint.id,
        ...
        baseline_generation=SUCCESSOR_CHECKPOINT_GENERATION,
        successor_checkpoint_kind=checkpoint.successor_checkpoint_kind,
        successor_checkpoint_state_hash=checkpoint.successor_checkpoint_state_hash,
        successor_checkpoint_completion_hash=checkpoint.completion_hash,
    ...
    try:
        ...
        raise ExternalDocumentSourceConflictError("Exact-item successor metadata observation failed") from None
    ...
    if observed_projection is None:
        ...
        if observed["provider_item_id_hash"] != baseline["provider_item_id_hash"]:
            raise ExternalDocumentSourceConflictError("Successor exact-item endpoint returned an unexpected provider item identity")
    ...
    execution.successor_change_detection_completed = True
...
def get_external_document_source_successor_change_detection(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    if execution is None:
        raise ExternalDocumentSourceNotFoundError("Successor change-detection execution not found")
    _ensure_integrity(db, execution)
    return execution


def list_external_document_source_successor_change_detection_receipts(
    db: Session,
    *,
    organization_id: UUID,
    profile_id: UUID,
    ...
    execution = get_external_document_source_successor_change_detection(
        db,
        organization_id=organization_id,
        profile_id=profile_id,
