...
from app.modules.external_document_sources.change_detection_schemas import (
    ExternalDocumentSourceChangeDetectionRead,
    ExternalDocumentSourceChangeDetectionReceiptRead,
    ExternalDocumentSourceChangeDetectionRequest,
)
from app.modules.external_document_sources.change_detection_service import (
    execute_external_document_source_change_detection,
    get_external_document_source_change_detection,
    list_external_document_source_change_detection_receipts,
)
...
def _execution_audit_values(execution) -> dict:
    return {
        ...
        "checkpoint_completion_hash": execution.checkpoint_completion_hash,
        "baseline_metadata_item_hash": execution.baseline_metadata_item_hash,
...
@router.post(
    "/profiles/{profile_id}/sync-checkpoint-executions/{sync_checkpoint_execution_id}/change-detection-executions",
    response_model=ExternalDocumentSourceChangeDetectionRead,
    status_code=status.HTTP_201_CREATED,
)
def execute_change_detection_endpoint(
    profile_id: UUID,
    sync_checkpoint_execution_id: UUID,
    payload: ExternalDocumentSourceChangeDetectionRequest,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
) -> ExternalDocumentSourceChangeDetectionRead:
    try:
        execution, outcome = execute_external_document_source_change_detection(
            db,
            organization_id=current_user.organization_id,
            profile_id=profile_id,
            sync_checkpoint_execution_id=sync_checkpoint_execution_id,
        ...
        if outcome == "completed":
            write_audit_log(
                ...
                action="EXTERNAL_DOCUMENT_SOURCE_EXACT_ITEM_CHANGE_DETECTION_COMPLETED",
                entity_type="external_document_source_change_detection_execution",
                entity_id=execution.id,
                new_values=_execution_audit_values(execution),
                details=(
                    "Phase 17.5-P performed one explicit metadata-only observation of the exact Phase O checkpointed provider item. "
                    "No folder listing, file-body read, object-store access, checkpoint advance, recurring sync/subscription, Document creation, Evidence admission, parsing/OCR or claim mutation was performed."
                ),
            )
            db.commit()
            db.refresh(execution)
        return ExternalDocumentSourceChangeDetectionRead.model_validate(execution)
    except (
        ExternalDocumentSourceValidationError,
        ExternalDocumentSourceConflictError,
        ExternalDocumentSourceNotFoundError,
...
@router.get(
    "/profiles/{profile_id}/change-detection-executions/{execution_id}",
    response_model=ExternalDocumentSourceChangeDetectionRead,
)
def get_change_detection_endpoint(
    profile_id: UUID,
    execution_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
...
@router.get(
    "/profiles/{profile_id}/change-detection-executions/{execution_id}/receipts",
    response_model=list[ExternalDocumentSourceChangeDetectionReceiptRead],
)
def list_change_detection_receipts_endpoint(
    profile_id: UUID,
    execution_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    current_user: ConnectionAuthorizationAdminMfa,
) -> list[ExternalDocumentSourceChangeDetectionReceiptRead]:
    try:
        receipts = list_external_document_source_change_detection_receipts(
            db,
            organization_id=current_user.organization_id,
            profile_id=profile_id,
            execution_id=execution_id,
        )
        return [ExternalDocumentSourceChangeDetectionReceiptRead.model_validate(row) for row in receipts]
    except (
        ExternalDocumentSourceValidationError,
        ExternalDocumentSourceConflictError,
        ExternalDocumentSourceNotFoundError,
