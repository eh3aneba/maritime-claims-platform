...
class EvidenceRecoveryDurableReadPromotionLease(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """Bounded long-lived read-routing lease backed by one approved Phase L authorization."""

    __tablename__ = "evidence_recovery_durable_read_promotion_leases"
    __table_args__ = (
        CheckConstraint(
            "status IN ('prepared', 'activated', 'rolled_back', 'expired', 'invalidated')",
            name="ck_recovery_durable_route_lease_status",
        ),
        CheckConstraint(
            "prepared_by_id <> activated_by_id OR activated_by_id IS NULL",
            name="ck_recovery_durable_route_lease_four_eyes",
        ),
        CheckConstraint(
            "authorization_approved_by_id <> activated_by_id OR activated_by_id IS NULL",
            name="ck_recovery_durable_route_lease_approver_split",
        ),
        CheckConstraint(
            ...
            name="ck_recovery_durable_route_lease_state_flags",
        ),
        CheckConstraint("source_file_size_bytes >= 0", name="ck_recovery_durable_route_lease_source_size"),
        CheckConstraint("write_path_switched = false", name="ck_recovery_durable_route_lease_no_write"),
        CheckConstraint("document_storage_key_mutated = false", name="ck_recovery_durable_route_lease_no_key"),
        CheckConstraint("authoritative_storage_changed = false", name="ck_recovery_durable_route_lease_no_authority"),
        CheckConstraint("destructive_action_performed = false", name="ck_recovery_durable_route_lease_no_destructive"),
        CheckConstraint("s3_delete_performed = false", name="ck_recovery_durable_route_lease_no_s3_delete"),
        CheckConstraint("local_delete_performed = false", name="ck_recovery_durable_route_lease_no_local_delete"),
        UniqueConstraint("authorization_id", name="uq_recovery_durable_route_lease_authorization"),
        UniqueConstraint("organization_id", "lease_hash", name="uq_recovery_durable_route_lease_org_hash"),
        Index("ix_recovery_durable_route_lease_org_claim_status", "organization_id", "claim_id", "status"),
        Index("ix_recovery_durable_route_lease_org_doc_status", "organization_id", "document_id", "status"),
    )
    ...
    authorization_approval_receipt_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_durable_read_promotion_authorization_receipts.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class EvidenceRecoveryDurableReadPromotionReceipt(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """Append-only transition receipt for a durable recovery read-routing lease."""

    __tablename__ = "evidence_recovery_durable_read_promotion_receipts"
    __table_args__ = (
        CheckConstraint(
            "phase IN ('prepared', 'activated', 'rolled_back', 'expired', 'invalidated')",
            name="ck_recovery_durable_route_receipt_phase",
        ),
        CheckConstraint(
            "from_route_class IN ('local_source', 'recovery_replica') AND to_route_class IN ('local_source', 'recovery_replica')",
            name="ck_recovery_durable_route_receipt_route_class",
        ),
        CheckConstraint("route_version >= 1", name="ck_recovery_durable_route_receipt_version"),
        CheckConstraint("write_path_switched = false", name="ck_recovery_durable_route_receipt_no_write"),
        CheckConstraint("document_storage_key_mutated = false", name="ck_recovery_durable_route_receipt_no_key"),
        CheckConstraint("authoritative_storage_changed = false", name="ck_recovery_durable_route_receipt_no_authority"),
        CheckConstraint("destructive_action_performed = false", name="ck_recovery_durable_route_receipt_no_destructive"),
        CheckConstraint("s3_delete_performed = false", name="ck_recovery_durable_route_receipt_no_s3_delete"),
        CheckConstraint("local_delete_performed = false", name="ck_recovery_durable_route_receipt_no_local_delete"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_recovery_durable_route_receipt_org_hash"),
        Index("ix_recovery_durable_route_receipt_time", "durable_lease_id", "transitioned_at"),
    )
