...
class EvidenceRecoveryDurableReadReauthorizedRenewalHealthQualification(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """Non-routable health evidence for one completed Phase S renewal window."""

    __tablename__ = "evidence_recovery_drr_reauth_renewal_health_qualifications"
    __table_args__ = (
        CheckConstraint(
            "status IN ('pending_second_approval', 'qualified', 'degraded', 'rejected', 'expired', 'invalidated')",
            name="ck_drr_reauth_health_status",
        ),
        CheckConstraint("health_state IN ('healthy', 'degraded', 'failed')", name="ck_drr_reauth_health_state"),
        CheckConstraint("terminal_phase IN ('rolled_back', 'expired')", name="ck_drr_reauth_health_terminal"),
        CheckConstraint("requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_reauth_health_four_eyes"),
        CheckConstraint("reauthorized_renewal_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_reauth_health_s_split"),
        CheckConstraint("reauthorization_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_reauth_health_r_split"),
        CheckConstraint("phase_q_qualified_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_reauth_health_q_split"),
        CheckConstraint("prior_renewal_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_reauth_health_p_split"),
        CheckConstraint("verified_durable_read_count >= 1", name="ck_drr_reauth_health_verified"),
        CheckConstraint("integrity_failure_count >= 0", name="ck_drr_reauth_health_integrity"),
        CheckConstraint("storage_unavailable_count >= 0", name="ck_drr_reauth_health_storage"),
        CheckConstraint("route_expired_attempt_count >= 0", name="ck_drr_reauth_health_expired"),
        CheckConstraint("operational_event_count >= verified_durable_read_count", name="ck_drr_reauth_health_events"),
        CheckConstraint("route_version_at_request >= 1", name="ck_drr_reauth_health_routever"),
        CheckConstraint("status <> 'qualified' OR health_state = 'healthy'", name="ck_drr_reauth_health_qualified"),
        CheckConstraint("status <> 'degraded' OR health_state <> 'healthy'", name="ck_drr_reauth_health_degraded"),
        CheckConstraint("routable_authority_created = false", name="ck_drr_reauth_health_no_route"),
        CheckConstraint("durable_read_route_created = false", name="ck_drr_reauth_health_no_durable"),
        CheckConstraint("read_path_switched = false", name="ck_drr_reauth_health_no_read"),
        CheckConstraint("write_path_switched = false", name="ck_drr_reauth_health_no_write"),
        CheckConstraint("document_storage_key_mutated = false", name="ck_drr_reauth_health_no_key"),
        CheckConstraint("authoritative_storage_changed = false", name="ck_drr_reauth_health_no_auth"),
        CheckConstraint("destructive_action_performed = false", name="ck_drr_reauth_health_no_dest"),
        CheckConstraint("s3_delete_performed = false", name="ck_drr_reauth_health_no_s3del"),
        CheckConstraint("local_delete_performed = false", name="ck_drr_reauth_health_no_localdel"),
        UniqueConstraint("reauthorized_renewal_lease_id", name="uq_drr_reauth_health_s_lease"),
        UniqueConstraint("organization_id", "health_qualification_hash", name="uq_drr_reauth_health_org_hash"),
        Index("ix_drr_reauth_health_org_claim", "organization_id", "claim_id", "status"),
        Index("ix_drr_reauth_health_org_doc", "organization_id", "document_id", "status"),
    )
...
class EvidenceRecoveryDurableReadReauthorizedRenewalHealthQualificationReceipt(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """Append-only Phase T governance receipt."""

    __tablename__ = "evidence_recovery_drr_reauth_renewal_health_receipts"
    __table_args__ = (
        CheckConstraint("phase IN ('requested', 'qualified', 'degraded', 'rejected', 'expired', 'invalidated')", name="ck_drr_reauth_health_rec_phase"),
        CheckConstraint("routable_authority_created = false", name="ck_drr_reauth_health_rec_route"),
        CheckConstraint("durable_read_route_created = false", name="ck_drr_reauth_health_rec_durable"),
        CheckConstraint("read_path_switched = false", name="ck_drr_reauth_health_rec_read"),
        CheckConstraint("write_path_switched = false", name="ck_drr_reauth_health_rec_write"),
        CheckConstraint("document_storage_key_mutated = false", name="ck_drr_reauth_health_rec_key"),
        CheckConstraint("authoritative_storage_changed = false", name="ck_drr_reauth_health_rec_auth"),
        CheckConstraint("destructive_action_performed = false", name="ck_drr_reauth_health_rec_dest"),
        CheckConstraint("s3_delete_performed = false", name="ck_drr_reauth_health_rec_s3del"),
        CheckConstraint("local_delete_performed = false", name="ck_drr_reauth_health_rec_localdel"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_drr_reauth_health_rec_org_hash"),
        Index("ix_drr_reauth_health_rec_time", "health_qualification_id", "transitioned_at"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    claim_id: Mapped[UUID] = mapped_column(ForeignKey("claims.id", ondelete="RESTRICT"), nullable=False, index=True)
    document_id: Mapped[UUID] = mapped_column(ForeignKey("documents.id", ondelete="RESTRICT"), nullable=False, index=True)
    health_qualification_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_drr_reauth_renewal_health_qualifications.id", ondelete="RESTRICT"), nullable=False, index=True)
    reauthorized_renewal_lease_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_drr_reauthorized_renewal_leases.id", ondelete="RESTRICT"), nullable=False, index=True)
