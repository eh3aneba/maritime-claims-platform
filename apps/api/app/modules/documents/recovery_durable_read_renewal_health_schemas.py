...
class RecoveryDurableReadRenewalHealthQualificationRequest(BaseModel):
    renewal_lease_id: UUID
    reason: str = Field(min_length=8, max_length=2000)


class RecoveryDurableReadRenewalHealthQualificationReason(BaseModel):
    reason: str = Field(min_length=8, max_length=2000)


class RecoveryDurableReadRenewalHealthQualificationRead(BaseModel):
    ...
    renewal_authorization_approved_by_id: UUID
...
class RecoveryDurableReadRenewalHealthQualificationReceiptRead(BaseModel):
    model_config = ConfigDict(from_attributes=True)
...
class RecoveryDurableReadRenewalHealthQualificationOperationRead(BaseModel):
    qualification: RecoveryDurableReadRenewalHealthQualificationRead
    receipt: RecoveryDurableReadRenewalHealthQualificationReceiptRead | None
    outcome: str
