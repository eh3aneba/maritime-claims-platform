...
class _DurableAuthoritativeStorageHealthSafetyMixin:
...
def _safety_constraints(prefix: str):
    return (
        CheckConstraint("local_evidence_preserved = true", name=f"ck_{prefix}_local"),
...
class EvidenceRecoveryDurableAuthoritativeStorageHealthQualification(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _DurableAuthoritativeStorageHealthSafetyMixin,
    Base,
):
    """Phase AO independent health qualification for durable recovery-storage authority."""

    __tablename__ = "evidence_recovery_durable_auth_storage_health_qualifications"
    __table_args__ = (
        CheckConstraint(
            "status IN ('pending_second_approval','qualified','rejected','expired','invalidated')",
            name="ck_daso_health_status",
        ),
        CheckConstraint("health_state = 'healthy'", name="ck_daso_health_healthy"),
        CheckConstraint("observed_authority_kind = 'recovery_storage'", name="ck_daso_health_kind"),
        CheckConstraint("observed_authority_tenure = 'durable_recovery'", name="ck_daso_health_tenure"),
        CheckConstraint("observed_ratification_active = true", name="ck_daso_health_active"),
        CheckConstraint("observed_durable_authority_created = true", name="ck_daso_health_durable"),
        CheckConstraint("observed_local_authoritative = false", name="ck_daso_health_not_local"),
        CheckConstraint("observed_recovery_authoritative = true", name="ck_daso_health_recovery"),
        CheckConstraint("observed_authoritative_storage_changed = true", name="ck_daso_health_changed"),
        CheckConstraint("requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_four_eyes"),
        CheckConstraint("phase_an_executed_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_an_split"),
        CheckConstraint("phase_am_requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_amr_split"),
        CheckConstraint("phase_am_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_ama_split"),
        CheckConstraint("phase_al_requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_alr_split"),
        CheckConstraint("phase_al_qualified_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_alq_split"),
        CheckConstraint("phase_ak_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_ak_split"),
        CheckConstraint("phase_aj_requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_ajr_split"),
        CheckConstraint("phase_aj_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_aja_split"),
        CheckConstraint("phase_ai_requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_air_split"),
        CheckConstraint("phase_ai_qualified_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_aiq_split"),
        CheckConstraint("phase_ah_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_daso_health_ah_split"),
        CheckConstraint("source_file_size_bytes >= 0", name="ck_daso_health_size"),
        CheckConstraint("observed_local_size_bytes = source_file_size_bytes", name="ck_daso_health_local_size"),
        CheckConstraint("observed_recovery_size_bytes = source_file_size_bytes", name="ck_daso_health_recovery_size"),
        CheckConstraint("observed_local_hash = source_file_hash", name="ck_daso_health_local_hash"),
        CheckConstraint("observed_recovery_hash = source_file_hash", name="ck_daso_health_recovery_hash"),
        CheckConstraint("authority_route_version_at_request >= 1", name="ck_daso_health_route_ver"),
        CheckConstraint("read_route_version_at_request >= 1", name="ck_daso_health_read_ver"),
        CheckConstraint("experimental_write_route_version_at_request >= 1", name="ck_daso_health_exp_ver"),
        CheckConstraint("durable_route_version_at_request >= 1", name="ck_daso_health_durable_ver"),
        UniqueConstraint("ratification_id", name="uq_daso_health_ratification"),
        UniqueConstraint("organization_id", "health_qualification_hash", name="uq_daso_health_org_hash"),
        Index("ix_daso_health_org_claim", "organization_id", "claim_id", "status"),
        Index("ix_daso_health_org_doc", "organization_id", "document_id", "status"),
    ...
    ratification_receipt_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_storage_ratification_receipts.id", ondelete="RESTRICT"), nullable=False)
    phase_am_authorization_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_storage_ratification_auths.id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    ratification_receipt_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    phase_am_authorization_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    phase_am_approval_receipt_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    ...
    observed_ratification_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default=true())
    observed_durable_authority_created: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default=true())
    ...
    phase_an_executed_by_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
    phase_am_requested_by_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
    phase_am_approved_by_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class EvidenceRecoveryDurableAuthoritativeStorageHealthReceipt(
    UUIDPrimaryKeyMixin,
    TimestampMixin,
    _DurableAuthoritativeStorageHealthSafetyMixin,
    Base,
):
    __tablename__ = "evidence_recovery_durable_auth_storage_health_receipts"
    __table_args__ = (
        CheckConstraint("phase IN ('requested','qualified','rejected','expired','invalidated')", name="ck_daso_health_rec_phase"),
        CheckConstraint("health_state = 'healthy'", name="ck_daso_health_rec_healthy"),
        CheckConstraint("observed_authority_kind = 'recovery_storage'", name="ck_daso_health_rec_kind"),
        CheckConstraint("observed_authority_tenure = 'durable_recovery'", name="ck_daso_health_rec_tenure"),
        CheckConstraint("observed_ratification_active = true", name="ck_daso_health_rec_active"),
        CheckConstraint("observed_durable_authority_created = true", name="ck_daso_health_rec_durable"),
        CheckConstraint("observed_local_authoritative = false", name="ck_daso_health_rec_not_local"),
        CheckConstraint("observed_recovery_authoritative = true", name="ck_daso_health_rec_recovery"),
        CheckConstraint("observed_authoritative_storage_changed = true", name="ck_daso_health_rec_changed"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_daso_health_rec_org_hash"),
        Index("ix_daso_health_rec_time", "health_qualification_id", "transitioned_at"),
    ...
    claim_id: Mapped[UUID] = mapped_column(ForeignKey("claims.id", ondelete="RESTRICT"), nullable=False, index=True)
    document_id: Mapped[UUID] = mapped_column(ForeignKey("documents.id", ondelete="RESTRICT"), nullable=False, index=True)
    health_qualification_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_durable_auth_storage_health_qualifications.id", ondelete="RESTRICT"), nullable=False, index=True)
    ratification_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_storage_ratifications.id", ondelete="RESTRICT"), nullable=False, index=True)
