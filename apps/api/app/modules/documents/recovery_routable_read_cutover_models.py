...
class EvidenceRecoveryReadPathCutoverLease(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_recovery_route_lease_status",
        ),
        CheckConstraint(
            "prepared_by_id <> activated_by_id OR activated_by_id IS NULL",
            name="ck_recovery_route_lease_four_eyes",
        ),
        CheckConstraint(
            "authorization_approved_by_id <> activated_by_id OR activated_by_id IS NULL",
            name="ck_recovery_route_lease_approver_split",
        ),
        CheckConstraint(
            "((status = 'activated' AND read_path_switched = true AND routable_authority_created = true) OR "
            "(status <> 'activated' AND read_path_switched = false AND routable_authority_created = false))",
            name="ck_recovery_route_lease_state_flags",
        ),
...
class EvidenceRecoveryReadPathRoute(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "route_class IN ('local_source', 'recovery_replica')",
