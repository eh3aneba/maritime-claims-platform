...
class EvidenceRecoveryDurableReadRenewalHealthQualification(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """Non-routable evidence that one completed Phase P renewal window was operationally healthy."""

    __tablename__ = "evidence_recovery_durable_read_renewal_health_qualifications"
    __table_args__ = (
        CheckConstraint(
            "status IN ('pending_second_approval', 'qualified', 'degraded', 'rejected', 'expired', 'invalidated')",
            name="ck_drr_health_status",
        ),
        CheckConstraint("health_state IN ('healthy', 'degraded', 'failed')", name="ck_drr_health_state"),
        CheckConstraint("terminal_phase IN ('rolled_back', 'expired')", name="ck_drr_health_terminal_phase"),
        CheckConstraint("requested_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_health_four_eyes"),
        CheckConstraint("renewal_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_health_renewal_activator_split"),
        CheckConstraint("renewal_authorization_approved_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_health_auth_approver_split"),
        CheckConstraint("prior_durable_activated_by_id <> qualified_by_id OR qualified_by_id IS NULL", name="ck_drr_health_prior_activator_split"),
        CheckConstraint("verified_durable_read_count >= 1", name="ck_drr_health_verified_reads"),
        CheckConstraint("integrity_failure_count >= 0", name="ck_drr_health_integrity_failures"),
        CheckConstraint("storage_unavailable_count >= 0", name="ck_drr_health_storage_failures"),
        CheckConstraint("route_expired_attempt_count >= 0", name="ck_drr_health_expired_attempts"),
        CheckConstraint("operational_event_count >= verified_durable_read_count", name="ck_drr_health_event_count"),
        CheckConstraint("route_version_at_request >= 1", name="ck_drr_health_route_version"),
        CheckConstraint("status <> 'qualified' OR health_state = 'healthy'", name="ck_drr_health_qualified_only_healthy"),
        CheckConstraint("status <> 'degraded' OR health_state <> 'healthy'", name="ck_drr_health_degraded_not_healthy"),
        CheckConstraint("routable_authority_created = false", name="ck_drr_health_no_route_auth"),
        CheckConstraint("durable_read_route_created = false", name="ck_drr_health_no_durable_route"),
        CheckConstraint("read_path_switched = false", name="ck_drr_health_no_read_switch"),
        CheckConstraint("write_path_switched = false", name="ck_drr_health_no_write_switch"),
        CheckConstraint("document_storage_key_mutated = false", name="ck_drr_health_no_key_mut"),
        CheckConstraint("authoritative_storage_changed = false", name="ck_drr_health_no_authority"),
        CheckConstraint("destructive_action_performed = false", name="ck_drr_health_no_destructive"),
        CheckConstraint("s3_delete_performed = false", name="ck_drr_health_no_s3_delete"),
        CheckConstraint("local_delete_performed = false", name="ck_drr_health_no_local_delete"),
        UniqueConstraint("renewal_lease_id", name="uq_drr_health_renewal_lease"),
        UniqueConstraint("organization_id", "health_qualification_hash", name="uq_drr_health_org_hash"),
        Index("ix_drr_health_org_claim", "organization_id", "claim_id", "status"),
        Index("ix_drr_health_org_doc", "organization_id", "document_id", "status"),
    )
    ...
    renewal_authorization_approved_by_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class EvidenceRecoveryDurableReadRenewalHealthQualificationReceipt(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("durable_read_route_created = false", name="ck_drr_health_rec_no_durable"),
        CheckConstraint("read_path_switched = false", name="ck_drr_health_rec_no_read"),
        CheckConstraint("write_path_switched = false", name="ck_drr_health_rec_no_write"),
        CheckConstraint("document_storage_key_mutated = false", name="ck_drr_health_rec_no_key"),
        CheckConstraint("authoritative_storage_changed = false", name="ck_drr_health_rec_no_auth"),
        CheckConstraint("destructive_action_performed = false", name="ck_drr_health_rec_no_dest"),
        CheckConstraint("s3_delete_performed = false", name="ck_drr_health_rec_no_s3del"),
        CheckConstraint("local_delete_performed = false", name="ck_drr_health_rec_no_localdel"),
        UniqueConstraint("organization_id", "receipt_hash", name="uq_drr_health_rec_org_hash"),
        Index("ix_drr_health_receipt_time", "health_qualification_id", "transitioned_at"),
    )

    organization_id: Mapped[UUID] = mapped_column(ForeignKey("organizations.id", ondelete="RESTRICT"), nullable=False, index=True)
    claim_id: Mapped[UUID] = mapped_column(ForeignKey("claims.id", ondelete="RESTRICT"), nullable=False, index=True)
    document_id: Mapped[UUID] = mapped_column(ForeignKey("documents.id", ondelete="RESTRICT"), nullable=False, index=True)
    health_qualification_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_durable_read_renewal_health_qualifications.id", ondelete="RESTRICT"), nullable=False, index=True)
    renewal_lease_id: Mapped[UUID] = mapped_column(ForeignKey("evidence_recovery_durable_read_renewal_leases.id", ondelete="RESTRICT"), nullable=False, index=True)
